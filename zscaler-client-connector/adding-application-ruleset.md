...
PDF: https://help.zscaler.com/pdf/com/en/1532885.pdf

You can add an application to use when adding a ruleset to manage endpoint firewall rules to use with location-based policies. To learn more, see [About Location-Based Policies](/zscaler-client-connector/about-location-based-policies).
...
- Click **Save**.
You can enter a maximum of 300 applications to use in rulesets. After you create an application, you can select it in [inbound](/zscaler-client-connector/adding-ruleset#inbound-rule) and [outbound](/zscaler-client-connector/adding-ruleset#outbound-rule) firewall rules when adding a ruleset. You must use the application in a ruleset, and you cannot use it for application-based bypasses directly in the app profile.
