...
PDF: https://help.zscaler.com/pdf/com/en/1296756.pdf

[Watch a video on using PAC files with Zscaler Client Connector.](https://fast.wistia.net/embed/iframe/txkrnumofy)
...
A PAC file is a text file used for directing traffic to a proxy server. A PAC file can also be used to bypass traffic or direct traffic to a specific proxy (i.e., traffic splitting). To learn more about PAC files, see [Understanding PAC Files](/zia/understanding-pac-file).
Zscaler Client Connector uses PAC files in the [Forwarding Profile](/zscaler-client-connector/configuring-forwarding-profiles-zscaler-app) and in the [App Profile](/zscaler-client-connector/configuring-zscaler-app-profiles). The Forwarding Profile PAC file is used to direct system, browser, and application traffic to Zscaler Client Connector. When Zscaler Client Connector receives traffic, the App Profile is used to direct that traffic to the Zscaler cloud. Zscaler Client Connector directs traffic to the closest data center according to the App Profile PAC file by changing the proxy return statement to the IP address of the geographically closest data center.
...
In an IPv6 environment, IPv6 bypasses are required for Zscaler Private Access (ZPA) apps in the App Profile PAC file when the Forwarding PAC is configured to send traffic to the Zscaler Client Connector listener.
...
In an IPv6 environment, IPv6 bypasses are required for ZPA apps in the App Profile PAC file when the Forwarding PAC is configured to send traffic to the Zscaler Client Connector listener.
