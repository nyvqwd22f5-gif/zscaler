...
Source: https://help.zscaler.com/zscaler-client-connector/best-practices-adding-bypasses-z-tunnel-2.0
PDF: https://help.zscaler.com/pdf/com/en/1349806.pdf
...
Add network bypasses to the **Hostname or IP Address Bypass for VPN Gateway** field in the [Zscaler Client Connector Portal app profile](/zscaler-client-connector/configuring-zscaler-app-profiles).[]
...
[]You can add specific subnets to the **Destination Exclusions** and **Destination Inclusion** fields in the [Zscaler Client Connector Portal app profile](/zscaler-client-connector/configuring-zscaler-app-profiles), allowing you to send a specific subset of your traffic to the ZIA Public Service Edge through Z-Tunnel 2.0.
...
To learn more about adding specific subnets, see [Configuring Zscaler Client Connector Profiles](/zscaler-client-connector/configuring-zscaler-app-profiles).
...
To add a port-based bypass, add the port to the network bypass in the **Destination Exclusions** field in the [Zscaler Client Connector Portal app profile](/zscaler-client-connector/configuring-zscaler-app-profiles). You must add the port to the end of the network bypass.
...
You can configure domain-based bypasses with custom [PAC](/zscaler-client-connector/best-practices-using-pac-files-zscaler-app) files for the forwarding profile and the app profile. For Zscaler Client Connector version 3.8 and later for Windows, you can enable Forwarding Profile options that don’t require configuring a system proxy PAC in the forwarding profile. This allows you to create bypasses for domain-based applications (i.e., applications that can’t be resolved to a single IP address).
...
[]
Configure the [app profile PAC file](#app-profile-pac-file) and select from the following [Forwarding Profile](/zscaler-client-connector/configuring-forwarding-profiles-zscaler-client-connector) options:
...
The following table describes how traffic is handled depending on how you enable these options in the [Forwarding Profile](/zscaler-client-connector/configuring-forwarding-profiles-zscaler-client-connector):
-
-
...
[]Configure the [forwarding profile PAC file](/zscaler-client-connector/configuring-forwarding-profiles-zscaler-app) to include the Z-Tunnel 2.0 bypass return statement for any destinations you want to send directly.
...
[]Configure the [app profile PAC file](/zscaler-client-connector/configuring-zscaler-app-profiles) to include a return direct statement for the domain:
function FindProxyForURL(url, host) {
