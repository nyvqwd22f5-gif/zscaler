# Enabling ZPA Partner Logins
Source: https://help.zscaler.com/zscaler-client-connector/enabling-zpa-partner-logins
PDF: https://help.zscaler.com/pdf/com/en/1417541.pdf

A partner is a separate entity or company that owns a ZPA tenant. You can give partner organizations access to your organization's tenant. You can also allow your users to access a partner organization's tenant. A partner organization must have these same settings configured for their own tenant as well to ensure users from your organization and their organization can access each other's tenants.
To enable partner logins for ZPA:
- In the Zscaler Client Connector Portal, go to **Administration**.
- On the **Administration** page, from the left-side navigation, select **ZPA Partner Logins**.
- In **Allow Partner Login to This Tenant**:
...
If you use Zscaler Client Connector version 4.6 and later for Windows, you can configure access to a partner’s tenant on the [app profile](/zscaler-client-connector/configuring-zscaler-client-connector-app-profiles). Partner login settings that are enabled on the app profile override these settings.
