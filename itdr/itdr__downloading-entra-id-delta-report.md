...
Source: https://help.zscaler.com/itdr/downloading-entra-id-delta-report
PDF: https://help.zscaler.com/pdf/com/en/1531842.pdf
...
-
On the **Entra ID Posture **dashboard, click **Downloads** and select one of the following options from the drop-down menu:
[See image.](#itdr-entra-id-delta-report-pdf-excel-download-option-image)
- [Delta Report PDF](#itdr-entra-id-delta-report-pdf)
- [Delta Report Excel](#itdr-entra-id-delta-report-excel)
[]In the **Delta Report PDF **window, select the Entra ID scans that you want to compare.
-
Select the first Entra ID scan timestamp from the drop-down menu, and click **Next**.
[See image.](#itdr-entra-id-delta-report-pdf-first-scan-image)
-
Select the second Entra ID scan timestamp (an older date) from the drop-down menu, and click **Submit**.
[See image.](#itdr-entra-id-delta-report-pdf-second-scan-image)
The delta report is downloaded to your system in the PDF. The report includes the following information in a structured and concise format with clear insights into the security posture changes between the two Entra ID scans:
...
- **Key Highlights**: The key highlights on the security posture of the Entra ID tenant, such as top new issues, major improvements, and remaining critical issues that require attention.
...
[See image.](#itdr-entra-id-delta-pdf-report-image)
-
[]In the **Delta Report Excel** window, select the Entra ID scans that you want to compare.
-
Select the first Entra ID scan timestamp from the drop-down menu, and click **Next**.
[See image.](#itdr-entra-id-delta-excel-first-scan-image)
-
Select the second Entra ID scan timestamp (an older date) from the drop-down menu, and click **Submit**.
[See image.](#itdr-entra-id-delta-excel-second-scan-image)
...
- **New Issues**: The total number of new issues introduced in Scan B, including issue name, severity levels (**High**, **Medium**, or **Low**), and affected Entra ID identities (**User**, **Service Principal**, etc.) details.
...
[See image.](#itdr-entra-id-delta-excel-report-details-image)
[]![Download Entra ID delta report PDF and Excel options](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-download-entraID-delta-report.png)
[]![Select the first Entra ID scan](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-entra-id-delta-pdf-select-first-scan.png)
[]![Select the second Entra ID scan](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-entra-id-delta-pdf-select-second-scan.png)
[]![Displaying Entra ID Delta PDF Report](/downloads/itdr/dashboard/entra-id-posture/entra-id-posture-reports/downloading-entra-id-delta-report/Entra-Delta-Report-1.png)
[]![Select the first Entra ID scan](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-entra-id-delta-excel-select-first-scan.png)
[]![Select the second Entra ID scan](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-entra-id-delta-excel-select-second-scan.png)
[]![View the Entra ID delta Excel report](/downloads/itdr/dashboard/entra-id/downloading-entra-id-delta-report/itdr-entra-id-delta-excel-report-details-1.png)