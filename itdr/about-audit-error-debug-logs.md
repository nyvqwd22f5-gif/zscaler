...
Source: https://help.zscaler.com/itdr/about-audit-error-debug-logs
PDF: https://help.zscaler.com/pdf/com/en/1531703.pdf

You can view and manage audit, error, and debug logs.
The Logs page provides the following benefits and enables you to:
- View or export audit logs in the Zscaler ITDR Admin Portal.
...
[]Audit logs are chronological records of user activities that occur in the ITDR Admin Portal. These logs record activities such as login attempts, user profile updates, etc. You can use the audit logs to trace back to the source of a change. Audit logs allow you to evaluate the events and check for any anomalies in user behavior. You can export audit logs as a CSV file.
...
- **Module**: The ITDR module (Login, Retention Policy, etc.) on which the activity occurred.
...
- **Module**: The ITDR entity (ZPA, Email, etc.) on which the error occurred.
...
[]Debug logs provide information on the errors that occurred in the ITDR Admin Portal. The debug logs help Zscaler Support to identify the root cause of a problem and provide a resolution. You can download debug logs for the last 2 hours, 24 hours, or 7 days.
...
Click the **Delete** icon to delete the file containing the debug logs from the ITDR Admin Portal.
- Click **Close**.
...
[]![View error messages](/downloads/itdr/settings/logs/about-audit-error-debug-logs/zscaler-deception-logs-error-message-4.png)
...
[]![View the Actions menu](/downloads/itdr/settings/logs/about-audit-error-debug-logs/zscaler-deception-error-logs-action-menu-3.png)
