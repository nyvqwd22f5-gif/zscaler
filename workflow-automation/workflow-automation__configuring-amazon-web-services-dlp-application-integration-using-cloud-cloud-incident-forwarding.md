...
PDF: https://help.zscaler.com/pdf/com/en/1531125.pdf
...
To configure the AWS DLP application integration using Cloud-to-Cloud Incident Forwarding:
- [1. Create resources for AWS.](#c2c-create-resources-AWS)
- [2. Add a DLP AWS application integration in Workflow Automation using the output from the CloudFormation stack or the manually created resources.](#c2c-Add-DLP-Integration-WA)
- [3. Set Up the DLP Cloud-to-Cloud Incident Forwarding.](#c2c-setup)
...
- [Creating AWS Resources Using a CloudFormation Stack in AWS](#c2c-create-aws-resources-using-cloudformation-stack-aws)
- [Creating AWS Resources Manually Using the AWS Console](#c2c-create-aws-resources-manually)
- [][a. Download the Zscaler template file from the Workflow Automation Admin Portal.](#c2c-Download-Zscaler-Template-File)
- [b. Create the CloudFormation stack in AWS using the template file that Zscaler provides.](#c2c-setup-cloudformation-stack)
-
...
- **ziaRoleARN**: Enter the Cloud-to-Cloud Role Amazon Resource Name (ARN), which is the only role allowed to write S3 objects for the S3 bucket policy. A ziaRoleARN is a string of characters. You can find this ARN in the AWS account where the Cloud-to-Cloud Incident Forwarding is hosted.
...
-
Click the **Outputs** tab to view the IAM cross-account role ARN and the external ID, SNS topic ARN, support role ARN (if created), and S3 bucket name prefix (copied from input) outputs. Copy these values. They are required when [adding a DLP Application Integration in Workflow Automation](#c2c-Add-DLP-Integration-WA).
[See image.](#cloudformation-stack-outputs-tab)
Different S3 buckets that share a common name prefix are used for storing DLP incident data and metadata files. The names of the data and metadata buckets are set to `<bucketNamePrefix>-data` and `<bucketNamePrefix>-metadata`, respectively. The Zscaler DLP Incident Management service needs to read and store the incident metadata file (with trigger data stripped), but it never reads and stores the incident data files.
...
- [][a. Create the AWS resources manually.](#c2c-create-resources-manually-procedure)
- [b. (Optional) Perform additional steps for customer-managed keys on the S3 buckets and SNS topic.](#c2c-additional-steps-cmkeys)
- [c. (Optional) Create a support role manually.](#c2c-Create-Support-Role-Manually)
[]AWS integration requires three AWS resources:
- **S3 Bucket**: The S3 bucket names share a common prefix. The names of the data and metadata buckets are `<bucketNamePrefix>-data` and `<bucketNamePrefix>-metadata`, respectively.
...
The bucket name prefix is also used as a property when [adding a DLP Application Integration in Workflow Automation](#c2c-Add-DLP-Integration-WA).
-
...
-
On the topic overview page, in the **Details** section, copy the **ARN** field value for the SNS topic. You use SNS Topic ARN as a property when [adding a DLP Application Integration in Workflow Automation](#c2c-Add-DLP-Integration-WA).
...
-
On the role overview page, in the **Summary** section, copy the **ARN** field value for the IAM role. This IAM cross-account role ARN is used as a property when [adding a DLP Application Integration in Workflow Automation](#c2c-Add-DLP-Integration-WA).
...
- Attach the following policy to the Cloud-to-Cloud role for uploading data to S3 buckets:
- Create a policy on the IAM dashboard:
- On the AWS console, search for `**IAM**`.
- In the search results, select **IAM**. The IAM dashboard** **appears.
-
On the IAM dashboard, in the left-side navigation, click **Policies**. The **Policies** page appears, listing all the existing policies.
[See image.](#Policies-Page-ZIA-Role)
- On the **Policies** page, click **Create Policy**. The **Specify permissions** page appears.
-
...
[See image.](#Specify-Permissions-Page-ZIA-Role)
- Click **Next**. The **Review and create** page appears.
- On the **Review and create** page, in the **Policy Details** section, enter a name for the policy in the **Policy name** field.
- Click **Create Policy**.
- Attach the policy to the Cloud-to-Cloud role:
- On the IAM dashboard, in the left-side navigation, click **Roles**. The **Roles** page appears, listing all the existing roles.
- On the **Roles **page, in the list of roles, select the Cloud-to-Cloud role. The **Zscaler Cloud-to-Cloud role overview** page appears.
-
On the **Zscaler Cloud-to-Cloud role overview **page, in the **Permissions policies** section, from the **Add permissions** drop-down menu, select **Attach policies**. The **Add Permissions **page appears, listing all the permission policies.
[See image.](#ZIA-Role-Overview-Page)
- On the **Add Permissions** page, select the KMS policy that you just created.
- Click **Add Permissions.**
...
To learn more, see [Configuring DLP Cloud-to-Cloud Incident Forwarding](/zia/configure-dlp-cloud-cloud-incident-forwarding).
...
[]
![Viewing the Zscaler Specific Configuration Section on the Specify Stack Details Page in AWS](/downloads/workflow-automation/data-protection/setup/configuring-amazon-web-services-dlp-application-integration-using-cloud-cloud-incident-forwarding/WA-C2C-AWS-CloudFormation-Zscaler-Specific-Configuration.png)
...
[]![Create Bucket Page](/downloads/workflow-automation/data-protection/setup/configuring-amazon-web-services-dlp-application-integration-using-cloud-cloud-incident-forwarding/WA-AWS-Create-Bucket-PG-data.png)
[]![Viewing the Bucket Overview Page in the AWS Console](/downloads/workflow-automation/data-protection/setup/configuring-amazon-web-services-dlp-application-integration-using-cloud-cloud-incident-forwarding/ZIA-WA-AWS-Bucket-Overview-PG-metadata.png)
...
[]![Specifying permissions on the Specify Permissions page in the AWS Console](/downloads/workflow-automation/data-protection/setup/configuring-amazon-web-services-dlp-application-integration-using-cloud-cloud-incident-forwarding/ZIA-WA-AWS-IAM-Specify-Permissions-Image-ZIA.png)
[]![Adding a policy to the ZIA Role on the ZIA role overview page in the AWS Console](/downloads/workflow-automation/data-protection/setup/configuring-amazon-web-services-dlp-application-integration-using-cloud-cloud-incident-forwarding/ZIA-WA-AWS-ZIA-Role-Overview-Page.png)
