...
PDF: https://help.zscaler.com/pdf/com/en/1420711.pdf

The Traffic Flow page provides information about the Traffic and Domain Name System (DNS) Overview. You can access the Traffic Flow page by clicking the **View** icon (![Traffic Monitoring View Icon in the Cloud & Branch Connector Table](/downloads/cloud-connector/analytics-monitoring/about-traffic-flow/View%20icon%20C%26B%20Connector.jpg)
) in the [Traffic Monitoring](/cloud-branch-connector/analyzing-traffic-monitoring)'s Cloud & Branch Connector table, where you can see the throughput and session count forwarded via ZIA, ZPA, Direct, or Log & Control for the selected Cloud or Branch Connector.
...
- **Log & Control GW**: The session count data forwarded through the [Log and Control ](/cloud-branch-connector/brnch/about-log-and-control-forwarding)gateway.
...
Select a point on the graph to see the value for the specified time period. **Click for more info** displays the [session logs](/cloud-branch-connector/brnch/session-insights-logs-columns), where you can view real-time log consolidation for each transaction.
...
DNS Monitor
The DNS Monitor table displays data about the [DNS](/cloud-branch-connector/brnch/dns-insights-logs-filters) traffic that is configured by the [DNS Filtering Rule](/cloud-branch-connector/brnch/configuring-dns-filtering-rule). The DNS Monitor demonstrates the total transactions under the following network traffic actions:
