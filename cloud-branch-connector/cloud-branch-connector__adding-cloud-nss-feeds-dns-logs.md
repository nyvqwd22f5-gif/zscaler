...
PDF: https://help.zscaler.com/pdf/com/en/1452746.pdf
...
- **Feed Output Format**: These are the fields that are displayed in the output. See [NSS Feed Output Format: DNS Logs](/cloud-branch-connector/nss-feed-output-format-dns-logs) for information about the available fields and their syntax.
...
- **Redirect to ZPA**: Use this filter to limit the logs to transactions wherein the service redirected the DNS request or response to Zscaler Private Access (ZPA).
- **Rule Name**: Use this filter to limit the logs based on specific rules in the [DNS Filtering Rule](/cloud-branch-connector/configuring-dns-filtering-rule). Choose the rules from the list.
