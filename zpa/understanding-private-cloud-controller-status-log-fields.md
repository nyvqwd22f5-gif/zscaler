...
PDF: https://help.zscaler.com/pdf/com/en/1510696.pdf
...
| SessionType | The type of session. The expected value for this field is:ZPN_TUNNEL_CONTROL: Session from the Private Cloud Controller towards the closest ZPA Public Service Edge for management | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
| SessionStatus | The status of the session. The expected values for this field are:ZPN_STATUS_AUTHENTICATED: Private Cloud Controller successfully authenticated in ZPAZPN_STATUS_AUTH_FAILED: Private Cloud Controller failed to authenticate in ZPAZPN_STATUS_DISCONNECTED: Private Cloud Controller disconnected from a ZPA Service Edge | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
