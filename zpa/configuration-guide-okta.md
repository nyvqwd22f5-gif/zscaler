...
Source: https://help.zscaler.com/zpa/configuration-guide-okta
PDF: https://help.zscaler.com/pdf/com/en/1483681.pdf
...
- Repeat step b for all users and groups you want to assign to the ZPA application, then click **Done**.
...
- Click the **Identity Provider metadata** hyperlink to download the IdP's metadata file. You will need this file later in order to [complete the configuration](/zpa/about-idp/new#IdP_BothSSO) within the ZPA Admin Portal.
[See image.](#Step7_UserImg)
- For **Service Provider URL**, the URL that is provided for you when you [configure a new IdP configuration](/zpa/about-idp/new#IdP_BothSSO) in ZPA Admin Portal. This URL is specific to your IdP.
- For **Service Provider Entity ID**, enter the ID that is provided for you when you configure a new IdP configuration in ZPA Admin Portal. This ID is specific to your IdP.
[See image.](#SP_URLandID)
- Click **Save**.
- Go to the ZPA Admin Portal and [complete the IdP configuration set up](/zpa/about-idp/new#IdP_BothSSO).
- (Optional) If you are configuring Okta for user SSO and want to use [SCIM](/zpa/about-scim), proceed to the [SCIM Configuration Guide for Okta](/zpa/scim-configuration-guide-okta).
