...
PDF: https://help.zscaler.com/pdf/com/en/1485146.pdf

Using SCIM operations allows you to fetch, create, update, and delete users and groups. To perform these operations, you must enable SCIM-based provisioning and have your SCIM Service Provider Endpoint and Bearer Token. You must provide the Bearer Token to authenticate the SCIM API calls. To learn more, see [Enabling SCIM for Identity Management](/zpa/enabling-scim-identity-management).
...
For a full list of operations supported by the Zscaler SCIM servers, as well as attribute mappings, see [About SCIM APIs](/zpa/about-scim-apis).
...
Provide the `{{userId}}`, the Internal User ID found on the SCIM Users (**Administration** > **SCIM Users**) page in the ZPA Admin Portal, in the request endpoint.
- [Curl example](#Curl3)
...
Provide the `{{groupId}}`, the Internal Group ID found on the SCIM Groups (**Administration** > **SCIM Groups**) page in the ZPA Admin Portal, in the request endpoint.
- [Curl example](#Curl11)
