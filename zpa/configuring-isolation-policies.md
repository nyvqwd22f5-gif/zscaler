...
PDF: https://help.zscaler.com/pdf/com/en/1484886.pdf

Isolation policy rules allow you to implement controls based on [isolation profiles](/isolation/creating-isolation-profiles-zpa). For a complete list of ranges and limitations for isolation policy rules, see [Ranges & Limitations](/zpa/ranges-limitations).
To configure an access policy rule:
- Go to **Policy** > **Isolation Policy**.
- Click **Add Rule**.
...
[]The **Web Browser** client type is required for isolation policy rules, and it is the only client type available. It is preselected for the rule. To learn more, [About Browser Access](/zpa/about-browser-access).
