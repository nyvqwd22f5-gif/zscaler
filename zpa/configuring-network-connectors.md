...
PDF: https://help.zscaler.com/pdf/com/en/1529069.pdf

Within the ZPA Admin Portal, you can create [Network Connectors](/zpa/about-vpn-connectors), [Network Connector groups](/zpa/about-vpn-connector-groups), and [Network Connector provisioning keys](/zpa/about-vpn-connector-provisioning-keys). For a complete list of ranges and limits, see [Ranges & Limitations](/zpa/ranges-limitations).
To add a new Network Connector:
- Go to **VPN (for Legacy Apps)** > **Network Connectors** > **Network Connectors**.
- Click **Add** **Network Connector**.
The **Add Network Connector** window appears.
...
- []For the **Signing Certificate** step, from the drop-down menu, select the certificate that ZPA uses to sign certificates it issues to the Network Connector. If you need to generate a new enrollment certificate, see [Generating Zscaler-Issued Enrollment (CA) Certificates](/zpa/generating-zscaler-issued-enrollment-ca-certificates).
[See image.](#signimage)
- Click **Next**.
To learn more about certificates, see [Understanding Certificates](/zpa/understanding-certificates).
[]![Add Network Connector Window within ZPA Admin Portal](/downloads/zpa/configuring-vpn-connectors/Config%20Net%20Conn%201.png)
...
- **Version Profile**: Displays the current version profile. The default value is set to Default. To learn more, see [Configuring a Network Version Profile](/zpa/configuring-vpn-version-profile).
