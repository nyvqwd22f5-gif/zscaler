...
PDF: https://help.zscaler.com/pdf/com/en/1483706.pdf

You can [configure access policies](/zpa/about-accesspolicy/new) to meet the specific needs of your organization. To learn more about the different policy types and the order in which ZPA enforces policies, see [About Policies](/zpa/about-policies).
...
- [][Create a segment group](/zpa/about-applicationgroup/new) that includes the applications you want to configure policies for.
- In the** Add Access Policy **window, for **Rule Action**,** **select **Allow Access**.
- Under **Criteria**:
- For **Segment Groups**, select the segment group you created in step 1 (e.g., HR App Group).
- For **SAML Attributes**, leave the field set to **Any SAML attribute from an IdP** for this example. This will apply the policy rule to any user from any IdP you have [configured for user single sign-on (SSO)](/zpa/about-idp/new#IdP_BothSSO).
...
-  [][Create an segment group](/zpa/about-applicationgroup/new) that includes the applications you only want users in the Sales department to have access to.
- In the** Add Access Policy **window, for **Rule Action**,** **select **Allow Access**.
- Under **Criteria**:
- For **Segment Groups**, select the segment group you created in step 1 (e.g., HR App Group).
- For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use (e.g., Department) from the drop-down menu and provide the proper entry for it (e.g., Sales).
- Click **Save**.
...
- For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use (e.g., Group) from the drop-down menu and provide the proper entry for it (e.g., IT Staff).
- Click **Save**.
...
- For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use (e.g., Group) from the drop-down menu and provide the proper entry for it (e.g., Marketing Dept).
- Click **Save**.
...
- For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use (e.g., Group) from the drop-down menu and provide the proper entry for it (e.g., Sales).
- Click **Save**.
...
![Add Access Policy window within the ZPA Admin Portal](/downloads/zpa/documentation-knowledgebase/policies/access-policy/policy-configuration-examples/zpa-accesspolicy-allowanysamlandappsegorseggrp.png)
For this example, the Rule Order within the [Access Policy](/zpa/about-accesspolicy) page would appear as follows:
...
- []For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use (e.g., Group) from the drop-down menu and provide the proper entry for it, in this case the user group you want to block (e.g., Temp Staff).
- Click **Save**.
...
- []For **SAML Attributes**:
- Click **Select IdP** and choose the IdP configuration you want include in the policy rule. The IdP must be [configured for user SSO](/zpa/about-idp/new#IdP_BothSSO).
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use, in this case the user in the group you want to block (e.g., Email address), and provide the proper entry for it (e.g., jane.doe@example.com).
- Click **Save**.
...
- For **SAML Attributes**:
- Click **Select IdP** and choose the same IdP configuration you included in step 2b above.
- Select the [SAML attribute](/zpa/about-samlattributes) you want to use, in this case the group you want to allow (e.g., Group), and provide the proper entry for it (e.g., Sales).
- Click **Save**.
