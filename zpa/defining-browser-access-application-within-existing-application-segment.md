...
PDF: https://help.zscaler.com/pdf/com/en/1484091.pdf

You can define applications for Browser Access when [configuring a new application segment](/zpa/about-application/onboard). However, you can also modify existing application segments to enable Browser Access for an application. To learn more, see [About Browser Access](/zpa/about-BrowserAccess).
...
- Select a Browser Access (i.e., web server) certificate from the drop-down menu. You can click **Clear All** to remove any selections. To learn more, see [About Browser Access (Web Server) Certificates](/zpa/about-BrowserAccessCertificates).
- From the drop-down menu, select the protocol you want ZPA to use when a request is made to access the application. If your web server supports **HTTPS**, select this protocol from the drop-down menu. Otherwise, select **HTTP**. You can click **Clear All** to remove any selections.
...
- Make sure that the rest of the configuration is correct, then click **Save**. To learn more, see [Configuring Application Segments](/zpa/about-application/onboard).
After saving, you are returned to the **Application Segments** page.
- Copy the Canonical Name (CNAME).
- Go to ** Resource Management **>** Application Management **> [**Browser Access**](/zpa/about-BrowserAccess#BAPage).
...
[]![Application Segment page with Canonical Name (CNAME) within the ZPA Admin Portal](/downloads/zpa/browser-access/defining-browser-access-application-within-existing-application-segment/zpa-appsegment-copycname.png)