...
PDF: https://help.zscaler.com/pdf/com/en/1483836.pdf

This deployment guide provides information on prerequisites, how to deploy an App Connector on Microsoft Azure, and post-deployment verification checks. For general information regarding App Connector deployment for ZPA, see [About Deploying App Connectors](/zpa/about-deploying-connectors).
To learn more about ZPA for Microsoft Azure, see the [Solution Brief](https://www.zscaler.com/resources/solution-briefs/zpa-for-azure.pdf).
...
- Step 5: Verify that the deployed App Connector is [running and healthy](/zpa/managing-deployed-connectors#Status). Also, check that it [meets your sizing requirements](/zpa/managing-deployed-connectors#VerifySizing).
...
[See image.](#SeeImage1)
- Click the **Create** drop-down menu, and then click **Virtual machine**.
[See image.](#seeimage2)
- In the **Basics** tab, enter general information for the VM:
...
[See image.](#seeimage8)
- Select a size that supports the App Connector's [sizing requirements](/zpa/connector-deployment-microsoft-azure#Prereqs), and then configure the rest of the general information for the VM.
[See image.](#seeimage9)
- In the **Disks** tab, configure any additional features for the VM.
...
[See image.](#eeimage9c)
By default, the VM allows inbound SSH connections for port 22. Zscaler recommends updating the inbound rule to be more restricted. To learn more about configuring the security group, see the [Firewall Requirements and Interoperability Guidelines](/zpa/connector-deployment-guide-microsoft-azure#SecurityAndFirewallReqs) section above.
...
- In the **Virtual machines** window, take note of the **Public IP Address** for the newly configured VM. You will need this information in the next step.
...
After the initial boot of an App Connector instance, it might take up to 15 minutes for the admin credentials to apply. If you receive an invalid credentials error after the initial boot, wait a few minutes and try again.
...
****![Terminal.png](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/connector-deployment-guide-microsoft-azure/zpa-appc-azure-login.png)
****
...
In Azure, be sure to remove the inbound access rule on port 22 within the network security group** **associated with the App Connector.
After the App Connector provisioning key is applied, and you have made any necessary [network configuration changes](/zpa/connector-deployment-microsoft-azure#Networking), be sure to verify that the deployed App Connector is [running and healthy](/zpa/managing-deployed-connectors#Status).
...
[]![Azure Virtual Machine networking configuration](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/connector-deployment-guide-microsoft-azure/zpa-azure-vm-networking-panel_1.png)
...
[]![Configuring the Tags in Azure](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/connector-deployment-guide-microsoft-azure/zpa-azure-vm-tags_0.png)
