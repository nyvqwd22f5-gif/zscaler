...
PDF: https://help.zscaler.com/pdf/com/en/1484571.pdf

This deployment guide provides information on prerequisites, how to deploy a ZPA Private Service Edge on Microsoft Azure, and post-deployment verification checks. For general information regarding ZPA Private Service Edge deployment, see [About Deploying ZPA Private Service Edges](/zpa/about-deploying-service-edges).
To learn more about ZPA for Microsoft Azure, see the [Solution Brief](https://www.zscaler.com/resources/solution-briefs/zpa-for-azure.pdf?_ga=2.25739572.886694454.1582561349-1894002721.1568064596).
- [Step 1: Make sure that you have met all of the ZPA Private Service Edge Deployment Prerequisites](#step1Prereqs)
- [Step 2: Deploy the ZPA Private Service Edge on Microsoft Azure](#step2deploy)
- [Step 3: Disable the Password Expiration for the STIG-Hardened Private Service Edge Image](#step3disablepassword)
- [Step 4: Configure the networking for the deployed ZPA Private Service Edge](#step3network)
...
[]To deploy a ZPA Private Service Edge on Azure:
...
- Search for CentOS and select a CentOS 7.x that meets the requirements in the [prerequisites](/zpa/service-edge-deployment-guide-microsoft-azure#step1Prereqs). To learn more about support for CentOS 7.x, see [End-of-Support for CentOS 7.x, RHEL 7.x, and Oracle Linux 7.x](/eos-eol/end-support-centos-7.x-rhel-7.x-and-oracle-linux-7.x).
- Select a size that supports the ZPA Private Service Edge's sizing requirements.
...
- Under **Configure network security group**, complete the following:
- Click **Create new**.
...
In the **Overview** page for the VM, take note of the **Public IP address**. You will need this information for the following step.
- Complete the following steps to connect to the instance. SSH access is required in order to configure the provisioning key for the ZPA Private Service Edge.
-
Log in to the ZPA Private Service Edge console using your admin credentials.
After the initial boot of a ZPA Private Service Edge instance, it might take up to 15 minutes for the admin credentials to apply. If you receive an invalid credentials error after the initial boot, wait a few minutes and try again.
-
...
In the following example, for the Azure instance, the ZPA Private Service Edge IP address is `13.88.177.20`:
`ssh admin@13.88.177.20`
- When you are asked if you want to continue connecting, enter `yes`.
- Complete the deployment of the ZPA Private Service Edge for Azure by following one of the [CentOS deployment options](/zpa/service-edge-deployment-guide-centos-oracle-and-redhat) within step 3. To learn more about support for CentOS 7.x, see [End-of-Support for CentOS 7.x, RHEL 7.x, and Oracle Linux 7.x](/eos-eol/end-support-centos-7.x-rhel-7.x-and-oracle-linux-7.x).
