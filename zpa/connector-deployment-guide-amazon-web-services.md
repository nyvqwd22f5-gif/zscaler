...
PDF: https://help.zscaler.com/pdf/com/en/1483831.pdf

This deployment guide provides information on prerequisites, how to deploy an App Connector on Amazon Web Services (AWS), and post-deployment verification checks. For general information regarding App Connector deployment for ZPA, see [About Deploying App Connectors](/zpa/about-deploying-connectors).
Zscaler and AWS are technology partners. To learn more about integrating Zscaler and AWS, see the following:
- [Zscaler and Amazon CloudWatch Agent Deployment Guide](/zscaler-technology-partners/zscaler-and-amazon-cloudwatch-agent-deployment-guide)
- [Zscaler and AWS Deployment Guide](/zscaler-technology-partners/zscaler-and-aws-deployment-guide)
- [Zscaler and Amazon Q Generative AI Data Protection Deployment Guide](/zscaler-technology-partners/zscaler-and-amazon-q-genai-deployment-guide)
...
- [Zscaler and AWS Snowball Deployment Guide](/zscaler-technology-partners/zscaler-and-aws-snowball-deployment-guide)
To learn more about ZPA for AWS, see the [Solution Brief](https://www.zscaler.com/resources/solution-briefs/zscaler-private-access-and-amazon-web-services-partner.pdf).
...
If Instance Metadata Service Version 2 (IMDSv2) is enabled, App Connectors will fetch the AWS VM ID.
...
[]This procedure describes how you can deploy ZPA in AWS using the instance wizard. If you require a more scalable solution, Zscaler recommends [deploying the App Connector using a Launch Template with Auto Scaling](/zpa/connector-deployment-amazon-web-services-aws#Deployment2).
...
- In **Step 1: Choose AMI**, under **AWS Marketplace**, search for zscaler.
[See image.](#Step1_SearchAnAMI)
- When the **Zscaler Private Access App Connector** AMI appears, click **Select**.
- In **Step 2: Choose Instance Type**, select an instance type that supports the App Connector's [sizing requirements](#Prereqs).
...
- In **Step 7: Review**, verify the instance configuration.
An App Connector only needs outbound access on TCP port 443 to reach ZPA. So, ensure that the security group is configured accordingly.
...
In the **Connect To Your Instance** window that appears, take note of the private key name (i.e., a .pem file) and the public hostname or IP address for your instance. You need this information for the next step.
If the proper columns are shown, you can also see this information within the table on the **Instances** page.
- Update the security group associated with the App Connector to temporarily allow inbound access on port 22, then complete the following steps to connect to the instance. SSH access is required to configure the provisioning key for the App Connector.
- [See instructions.](#security)
- Apply the App Connector provisioning key.
- [See instructions.](#pkeyreal)
After the App Connector provisioning key is applied, verify that the deployed App Connector is [running and healthy](/zpa/managing-deployed-connectors#Status).
...
- []Stop running the zpa-connector service. If the provisioning key is not detected when the App Connector first starts, the App Connector is in a sleep cycle and looks for the key again every 24 hours.
...
In AWS, make sure you disable inbound access on port 22 for the security group associated with the App Connector.
...
[]This procedure describes how you can deploy ZPA in AWS through Launch Templates and Auto Scaling configurations in order to create a scalable and supportable infrastructure.
...
- On the **Create launch template** page:
...
- (Optional) You can add **Tags** to the launch template, if necessary. To learn more, refer to the [AWS documentation](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/Using_Tags.html).
...
systemctl stop zpa-connector
systemctl start zpa-connector
In the echo statement above, make sure that you have included the App Connector provisioning key you copied from the ZPA Admin Portal, for example:
The domain (e.g., api.private.com) in the echo statement depends on what ZPA cloud you are on. To learn more, see [What Is My Cloud Name for ZPA?](/zpa/what-my-cloud-name-zpa)
...
- **Group size**: Enter** **2.
- **Subnet**: Select a subnet from the drop-down menu. This is where your App Connectors are deployed. As stated previously, for each VPC there should be a pair of App Connectors deployed for resilience and scalability, so make sure that you select 2 subnets from the drop-down menu.
...
Your scaling policy should start with the initial deployment of 2 App Connectors; each App Connector is capable of passing 400 to 500 Mbps traffic. An additional App Connector can be added when this limit is reached, which occurs when the App Connector runs at about 50% CPU utilization. To learn more about App Connector throughput, see [App Connector Specifications and Sizing Requirements](#Prereqs).
- Click **Next: Configure Notifications**.
[See image.](#deploy2-CreateAutoScalingGrp2)
- In **Step 3: Configure Notifications**, set up your notifications as needed and click **Next: Configure Tags**. To learn more, refer to the [AWS documentation](https://docs.aws.amazon.com/autoscaling/ec2/userguide/ASGettingNotifications.html#as-configure-asg-for-sns).
- In **Step 4: Configure Tags**, set up your tags as needed and click **Review**. To learn more, refer to the [AWS documentation](https://docs.aws.amazon.com/autoscaling/ec2/userguide/autoscaling-tagging.html).
...
After the App Connectors are initialized, they are provisioned into the App Connector group you specified in the ZPA Admin Portal. The provisioning is based on the provisioning key included in the [script you entered](#user-data-script) in the **User data** field for the Launch Template and Auto Scaling group configuration.
- Zscaler highly recommends [updating the App Connector system software](/zpa/managing-deployed-connectors#Updating) before proceeding.
