...
PDF: https://help.zscaler.com/pdf/com/en/1532560.pdf

When you bring new application environments online, you need to validate them at a reduced scale and capacity until a certain confidence level is achieved. When proven, you can gradually redirect traffic to the new environments without large architectural shifts or cutovers. This allows a non-conflicting way to mitigate any issues that arise during a migration in a controlled way. Using load balance to rank your [server groups](/zpa/about-server-groups) by weight is a way to ensure application access requests are sent to the [App Connectors](/zpa/about-connectors) with server groups that can support their traffic.
This feature is only available for ZPA Public Service Edges. Any configuration done for application load balancing will not be honored for the transactions originating from Private Service Edges.
...
Transactions routed via load balancing do not consider application health. For on-access health monitoring applications routed via load balancing, application health is not generated, and those applications are not shown in the Health dashboard. To learn more, see [About the Health Dashboard](/zpa/about-health-dashboard).
Prerequisites
...
Application access requests should be routed to the App Connectors based on the weights defined for the server groups for an [application segment](/zpa/configuring-application-segments). All other Geo-IP and health-based path selection modes are ignored for the application segment configured with this option. Path Selection using Closer to User and Closer to App are not applicable.
...
- Go to **Application Management** > **Application Segments**.
...
- Click **Save.**
If you move a server group to a new Microtenant, be aware that you have to reassign the load balancing weight for those server groups. To learn more, see [About Microtenants](https://help.zscaler.com/zpa/about-microtenants).
[]![A view of the Defined Application Segments table. ](/downloads/zpa/documentation-knowledgebase/servers/server-groups/configuring-load-balance-server-groups/application-segments_load-balance-icon.png)
