...
Source: https://help.zscaler.com/zpa/managing-deployed-connectors
PDF: https://help.zscaler.com/pdf/com/en/1484116.pdf

After you deploy an App Connector and complete the proper networking configurations, perform the following procedures to verify that the App Connector is running and healthy, and that the [sizing and scalability specifications](/zpa/connector-deployment-prerequisites) you decided upon before deployment are still adequate for your organization's needs:
...
To learn more, see [App Connector Specifications and Sizing Requirements](/zpa/connector-deployment-prerequisites).
...
- [Log in to the App Connector console](/zpa/managing-deployed-connectors#Login) using the default admin username and password.
...
-
Using the App Connectors page and Health dashboard - You can check the status by accessing the [App Connectors page](/zpa/about-connectors#AboutConnectorsPage) and using the [Health dashboard](/zpa/about-dashboard/health) within the ZPA Admin Portal.
To check the App Connector status using the ZPA Admin Portal:
...
-
Go to **Dashboard** > **Health**. Check that the App Connector you deployed appears in the **App Connector** widget. The App Connector must have enrolled successfully at least once in order for it to appear within the Health dashboard.
...
![App Connectors page within the ZPA Admin Portal](/downloads/zpa/documentation-knowledgebase/connectors-and-connector-groups/connector-deployment-guides-supported-platforms/managing-deployed-connectors/zpa-connectordeployguides-connectorlistexample.png)
-
...
- RPM package for Red Hat Enterprise Linux 8-based deployments ([zpa-connector.rpm](https://yum.private.zscaler.com/yum/el8/zpa-connector-25.49.4-1.el8.x86_64.rpm))
- RPM package for Red Hat Enterprise Linux 9-based deployments ([zpa-connector.rpm](https://yum.private.zscaler.com/yum/el9/zpa-connector-25.49.4-1.el9.x86_64.rpm))
-
Use the `scp` command to copy the RPM package to the App Connector, for example:
$ scp <RPM Version>
- Log in to the App Connector console using your admin credentials.
- Update the App Connector software package using the following command:
`[admin@zpa-connector ~]$ sudo rpm -Uvh zpa-connector-25.49.4-1.el9.x86_64.rpm`
- Make sure that the update completes successfully, for example:
`[admin@zpa-connector ~]$ sudo rpm -Uvh zpa-connector-25.49.4-1.el9.x86_64.rpm
[sudo] password for admin:
Preparing... ################################# [100%]
Updating / installing...
1:zpa-connector-25.49.4-1.el9 ################################# [ 50%]
Warning: zpa-connector.service changed on disk. Run 'systemctl daemon-reload' to reload units.
Cleaning up / removing...
2:zpa-connector-25.49.4-1.el9 ################################# [100%]`
-
...
You must reconfigure your proxy settings in the App Connector after creating a new provisioning key file because the proxy settings are removed along with the old provisioning key file in a [previous step](#remove-pk).
...
[]If you need to replace a deployed App Connector using a new provisioning key on an existing VM image, complete the [Replace an App Connector provisioning key](/zpa/managing-deployed-connectors#ReplaceProvisioningKey) procedure above. If you need to replace a deployed App Connector using an existing provisioning key on a new VM image, complete the following steps:
- Log into the ZPA Admin Portal and go to **Configuration & Control > Private Infrastructure > App Connector Management > App Connector Provisioning Keys**.
- On the **App Connector Provisioning Keys** page, locate the existing key you want to use and click the **Edit icon**.
- In the **Edit App Connector Provisioning Key** window, increase the **Maximum Reuse of Provisioning Key** value to accommodate the number of App Connectors you are going to replace. To learn more, see [Editing App Connector Provisioning Keys.](/zpa/about-connectorprovisioning/edit)
- Deploy the new replacement App Connector VM images using the existing provisioning key from step 3 above. By doing so, these new App Connectors will automatically join the same App Connector Group as the App Connectors you are replacing. To learn more, see the [Deployment Guide for your App Connector](/zpa/documentation-knowledgebase/connectors-and-connector-groups/connector-deployment-guides-supported-platforms).
- When the new App Connectors are deployed, they will appear within the [Health dashboard](/zpa/about-dashboard/health). Verify that the new App Connectors in the App Connector Group are enabled and showing an **Up Time**. To learn more, see [Viewing App Connector Details](/zpa/about-dashboard/health#cdetails).
...
- In the **Edit App Connectors** window, set the **Status **to **Disabled**. To learn more, see [Editing a Deployed App Connector](/zpa/about-connector/edit).
...
When the user sessions have expired on the disabled App Connectors, delete them within the ZPA Admin Portal and remove any old VM instances from your environment.