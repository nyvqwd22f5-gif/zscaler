...
Source: https://help.zscaler.com/zpa/updating-app-connector-host-os-and-software-packages
PDF: https://help.zscaler.com/pdf/com/en/1484916.pdf

In this article, software components refer to App Connectors, Private Service Edges, and Private Cloud Controllers. Software components are licensed so that Zscaler can periodically update their software. However, updates to the host operating system (OS) and software packages are the organization's responsibility. Zscaler ensures that the virtual machine software is the latest version. The software component is designed to be compatible with updates to the host OS. To learn more, see [Managing Deployed App Connectors](/zpa/managing-deployed-connectors), [Managing Deployed Private Service Edges](/zpa/managing-deployed-zpa-private-service-edges), [Managing Deployed Private Cloud Controllers](/zpa/managing-deployed-private-cloud-controllers).
...
- For prebuilt images, software components must be provisioned to a ZPA tenant to receive software updates via the OS package manager.
...
- Enter the admin credentials for [App Connectors](/zpa/managing-deployed-connectors#Default), [ZPA Private Service Edges](/zpa/managing-deployed-zpa-private-service-edges#Default), or [Private Cloud Controllers](/zpa/managing-deployed-private-cloud-controllers#Default):
login: admin
Password: *******
- Test connectivity by running the following command:
curl https://yum.private.zscaler.com
This validates that connectivity to the specific update URL is working. If you are waiting for the command to time out, and you don't get any errors, you must troubleshoot connectivity. To learn more, see [Troubleshooting App Connectors](/zpa/troubleshooting-app-connectors) or [Troubleshooting ZPA Private Service Edges](/zpa/troubleshooting-zpa-private-service-edges).
Updating the Host OS and Software Packages
...
[See image.](#See3)
- Access the [App Connector Status](/zpa/about-connector-diagnostics) log type in the ZPA Admin Portal to view the list of currently active App Connectors to make sure any existing or long-lived critical transactions have ended or stopped.
- Log in to the App Connector console using your admin credentials.
- Stop the ZPA service using the following command:
$ sudo systemctl stop zpa-connector
The command to stop the ZPA service for a ZPA Private Service Edge is `sudo systemctl stop-zpa-service-edge`.
...
- After stopping the ZPA service and removing the old packages, log in and enter the following command to update the local system software:
$ sudo yum update -y
- After completing the update, reboot the App Connector using the following command:
$ sudo reboot
- After the reboot, go to the ZPA Admin Portal and click the **Edit **icon for the App Connector.
...
- Access the [Private Service Edge Status log type](/zpa/accessing-zpa-service-edge-status-diagnostics) in the ZPA Admin Portal to view the list of currently active Private Service Edges to make sure any existing or long-lived critical transactions have ended or stopped.
...
$ sudo reboot
- After the reboot, go to the ZPA Admin Portal and click the **Edit **icon to edit the Private Service Edge.
...
- Access the [Private Cloud Controller Status log type](/zpa/accessing-private-cloud-controller-status-diagnostics) in the ZPA Admin Portal to view the list of currently active Private Cloud Controllers to make sure any existing or long-lived critical transactions have ended or stopped.
...
$ sudo reboot
- After the reboot, go to the ZPA Admin Portal and click the **Edit **icon for the Private Cloud Controller.
- Select **Enabled** to enable the Private Cloud Controller.
[See image.](#pcc-enabled)
- Click **Save**.
[]![Disabling App Connector in the ZPA Admin Portal](/downloads/zpa/app-connector-management/app-connector-managing-troubleshooting/updating-app-connector-host-os-and-software-packages/zpa-app-connector-disabled_0.png)
...
[]![Disabling a Private Service Edge in the ZPA Admin Portal](/downloads/tech-pubs-drafts/zpa-draft-articles/updating-host-os-and-software-packages-draft-doc-60000/pse-disable.png)
...
[]![Disabling a Private Cloud Controller in the ZPA Admin Portal](/downloads/tech-pubs-drafts/zpa-draft-articles/updating-host-os-and-software-packages-draft-doc-60000/pcc-disable_0.png)
