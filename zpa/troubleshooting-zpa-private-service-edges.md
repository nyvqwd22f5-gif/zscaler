# Troubleshooting ZPA Private Service Edges
Source: https://help.zscaler.com/zpa/troubleshooting-zpa-private-service-edges
PDF: https://help.zscaler.com/pdf/com/en/1485476.pdf

This article provides troubleshooting information and guidelines about ZPA Private Service Edges. To learn more about ZPA Private Service Edges, see [About ZPA Private Service Edges](/zpa/about-zpa-private-service-edges). To configure ZPA Private Service Edges, see [Configuring ZPA Private Service Edges](/zpa/configuring-service-edges).
When troubleshooting ZPA Private Service Edges, consider the following scenarios:
- [ZPA Private Service Edges Not Connected to the Cloud](#notConnectedCloud)
- [ZPA Private Service Edges Connectivity to App Connectors](#connectivityConnectors)
- [DNS Failure, but ZPA Private Service Edges Successful](#dnsFailure)
...
- [Frequent Disconnections to the ZPA Cloud](#frequentCloudDisconnects)
- [ZPA Private Service Edges Load Not Balanced](#loadBalance)
- [ZPA Private Service Edges Pause State](#pauseState)
- [ZPA Private Service Edges Upgrade Failure](#upgradeFailure)
- [ZPA Private Service Edges Upgrade Partial Failure](#partialFailure)
...
[]If the ZPA Private Service Edge was previously working and now shows an error about not being connected to the cloud, you might see an error similar to the following:
...
You might see this error if a ZPA Private Service Edge was deleted from the ZPA Admin Portal accidently. ZPA immediately recognizes that it should not communicate with the ZPA Private Service Edge, and it breaks the active connection. There is no way to recover the ZPA Private Service Edge.
To resolve this issue:
- Remove the ZPA Private Service Edge on the deployed platform.
...
- Enter the following command to delete the ZPA Private Service Edge:
...
If you attempt to delete the ZPA Private Service Edge and the ZPA Private Service Edge still reports an ID number, then the ZPA Private Service Edge was not fully deleted.
- Restart the process for [configuring](/zpa/configuring-service-edges) a ZPA Private Service Edge and [deploying](/zpa/about-deploying-service-edges) on a platform.
[]If the ZPA Private Service Edge does not have a direct connection to the App Connector that is providing access to the application, the ZPA Private Service Edge forwards traffic to the nearest ZPA Public Service Edge. Traffic is not routed automatically to App Connectors.
If no App Connector for the requested application can establish a data connection to the required ZPA Private Service Edge where the user is connected, then the ZPA Private Service Edge relays the Zscaler Client Connector request to a ZPA Public Service Edge. The ZPA Public Service Edge then connects to the closest App Connector that it can reach for that application request and completes the application request via the ZPA Private Service Edge through an additional hop.
Use the following command to confirm which App Connectors your ZPA Private Service Edges are connected to:
...
[]If you have a DNS failure and the ZPA Private Service Edge `root` process is successful, you might see an error similar to the following:
...
[]If you have an error similar to the following, a device between the ZPA Private Service Edge and ZPA cloud service is attempting a SSL interception:
...
ZPA does not support SSL interceptions. ZPA uses TLS connections and pinned certificates, so a device performing SSL interception causes the connection to fail between the ZPA Private Service Edge and the Zscaler cloud.
To make sure traffic is not SSL intercepted, you need to allowlist all the ZPA domains or IP addresses in the SSL interception device that are listed on [config.zscaler.com/private.zscaler.com/zpa](https://config.zscaler.com/private.zscaler.com/zpa) (for the private.zscaler.com cloud) or [config.zscaler.com/zpatwo.net/zpa](https://config.zscaler.com/zpatwo.net/zpa) (for the zpatwo.net cloud). To learn more, see [What Is My Cloud Name for ZPA?](/zpa/what-my-cloud-name-zpa). This ensures ZPA Private Service Edges and the Zscaler URLs (e.g., *.zpath.net, *.zscaler.com) are bypassed from a SSL inspection.
[]You should routinely monitor and address memory leaks or high memory usage in your ZPA Private Service Edges. For more information, see [Monitoring ZPA Private Service Edge Performance](/zpa/monitoring-connector-performance).
...
[]You should routinely monitor and address high CPU for your ZPA Private Service Edges. For more information, see [Monitoring ZPA Private Service Edge Performance](/zpa/monitoring-connector-performance).
...
[]If you have issues with the ZPA Private Service Edge connecting to the ZPA cloud, the problem can come from one of a couple areas. There are a few things to check when narrowing down what is preventing the ZPA Private Service Edge to connect:
...
[]Make sure all the IP ranges allowed by the firewall match those in the ZPA firewall policies: refer to [config.zscaler.com/private.zscaler.com/zpa](https://config.zscaler.com/private.zscaler.com/zpa) (for the private.zscaler.com cloud) or [config.zscaler.com/zpatwo.net/zpa](https://config.zscaler.com/zpatwo.net/zpa) (for the zpatwo.net cloud). To learn more, see [What Is My Cloud Name for ZPA?](/zpa/what-my-cloud-name-zpa).
[]Test connectivity to the upstream ZPA Public Service Edge by querying the DNS address `pb2br.prod.zpath.net` from the server that is running the ZPA Private Service Edge software.
The endpoint for ZPA Private Service Edges to locate ZPA Public Service Edges is pb2br.
...
[]Test if the ZPA Private Service Edge can do a TLS connection to a ZPA Public Service Edge using the `openssl` command.
...
If the `openssl` command shows an error, an empty response, or no response, then there is a problem with your network. You can try running the openssl command in a loop to see if there is a regular pattern to the success or failure rate. For example, the firewall rule may not allow the ZPA Private Service Edge to open a connection to a ZPA Public Service Edge. Sometimes the TCP connection goes through, but TLS fails. This is a sign that the firewall is blocking the application layer, but it’s allowing the transport layer.
...
[]This error sometimes occurs intermittently, which indicates a networking issue either in your organization’s environment or in the internet path from the ZPA Private Service Edge to the ZPA Public Service Edge. In those cases, perform the following actions:
...
[]Do an ICMP ping to the ZPA Public Service Edge.
[]This is expected behavior. Each application requires a TCP port and has its health monitored. Many of these ports can reside in a `TIME_WAIT` state. To learn more, see [Monitoring ZPA Private Service Edge Performance](/zpa/monitoring-connector-performance).
[]If you see frequent disconnections between a ZPA Private Service Edge to the ZPA cloud, the problem can come from one of a couple areas.
Questions to address:
- Is there a ZPA Private Service Edge restart involved?
...
- Could there be a routing issue? Is only a specific set of ZPA Public Service Edge IPs (with common prefix) having the problem?
...
- Go to **Analytics **> **Diagnostics**. For more information, see [About ZPA Private Service Edge Status Diagnostics](/zpa/about-zpa-service-edge-status-diagnostics).
...
- Click **Apply**.
- Check the times in the **Disconnect Time** column to identify the correct ZPA Private Service Edge.
...
[]Sometimes the load (i.e., the interface stats) is not equally split between the ZPA Private Service Edges in the Private Service Edge group. Each ZPA Private Service Edge shares its load utilization with the Zscaler cloud once per minute. The ZPA Public Service Edges redirect the load across different ZPA Private Service Edges in a group by accounting for the relative CPU and memory utilization of each ZPA Private Service Edge. This redirection steers new connections over a period of time towards less loaded ZPA Private Service Edges over ZPA Private Service Edges that are heavily loaded.
To address the load not balancing, Zscaler recommends that the CPU and memory utilization of your ZPA Private Service Edges do not exceed the threshold. To learn more, see [Monitoring ZPA Private Service Edge Performance](/zpa/monitoring-private-service-edge-performance).
[]A ZPA Private Service Edge can be in a paused state for one of the following reasons:
...
A ZPA Private Service Edge provides the following log when it enters the pause state:
...
Check the journalctl logs for the reason the ZPA Private Service Edge is in a pause state using the following command:
sudo journalctl | grep "entering PAUSE mode because of"
For [remote troubleshooting](/zpa/accessing-and-viewing-support-information#Status), ZPA Private Service Edges may reject additional session commands when they are currently in a paused state. The following command is returned in the ZPA Private Service Edge console when they are currently in a paused state and reject a session command:
ADMIN_PROBE_LOG(AL_NOTICE, "entity is currently in paused mode, not taking any probe commands");
[]There are different possible reasons for ZPA Private Service Edge upgrade failures:
- [Upgrade is in a failed state for more than 24 hours.](#upgradeFailure_24hours)
- [ZPA Private Service Edge cannot download the image since there is no disk space left.](#upgradeFailure_noDiskSpace)
- [ZPA Private Service Edge cannot download the image due to inconsistent connection between the ZPA Private Service Edge and pb2br (ZPA Private Service Edge to Public Service Edge endpoint).](#upgradeFailure_inconsistentConnection)
- [Provisioning Key was deleted in the ZPA Admin Portal.](#upgradeFailure_provisioningKey)
...
- Restart the ZPA Private Service Edge. To learn more, see [Start, Stop, or Restart a ZPA Private Service Edge](/zpa/managing-deployed-zpa-private-service-edges#Starting).
- Stop the ZPA Private Service Edge.
- Verify no processes are running.
[admin@localhost ~]$ sudo ps wU zscaler
PID TTY      STAT   TIME COMMAND
[admin@localhost ~]$
- Start the ZPA Private Service Edge.
- Return to the [ZPA Private Service Edges page](/zpa/about-zpa-private-service-edges) to see if the Update Status changes from **Failure **to **Success**. If the status still says **Failure**, proceed to the next step.
- Check the logs and network connectivity by addressing any issues if identified.
- Test connectivity to the upstream ZPA Public Service Edge by querying the DNS address `pb2br.prod.zpath.net` from the server that is running the ZPA Private Service Edge software.
The endpoint for ZPA Private Service Edge to locate ZPA Public Service Edges is pb2br.
Successful connectivity results in two IPs. For example:
[admin@localhost ~]$ dig +short pb2br.prod.zpath.net
13.60.119.37
42.68.244.163
[admin@localhost ~]$
- Check if the ZPA Private Service Edge can start a TLS connection using the openssl command. You should receive a certificate subject string returned from the ZPA Public Service Edges.
In the following example, repeat the test and replace the domain name `safemarch.com` highlighted in red with your own domain name:
[admin@localhost ~]$ openssl s_client -servername safemarch.com.c2.prod.zpath.net -connect pb2br.prod.zpath.net:443 2>&1 | grep subject
subject=/C=US/ST=California/L=San Jose/O=Zscaler/OU=Emerging Technologies/CN=broker1.was1.prod.zpath.net
...
- Revert to the default ZPA Private Service Edge version.
- Stop the ZPA Private Service Edge. To learn more, see [Start, Stop, or Restart a ZPA Private Service Edge](/zpa/managing-deployed-zpa-private-service-edges#Starting).
- Remove the ZPA Private Service Edge binary image, the version identifier file, and the image metadata file.
...
- Start the ZPA Private Service Edge. The ZPA Private Service Edge reaches out to the Zscaler cloud and re-downloads the default ZPA Private Service Edge software version. The ZPA Private Service Edge attempts to upgrade at a later time.
...
If the software downloads, return to the [ZPA Private Service Edges page](/zpa/about-zpa-private-service-edges) to see if the Update Status changes from **Failure** to **Scheduled**.
If the software did not download or the ZPA Private Service Edge still does not update, proceed to the next step.
- Wipe and rebuild the ZPA Private Service Edge configuration.
Before you wipe and rebuild the ZPA Private Service Edge, you must:
- Identify the ZPA Private Service Edge group for the ZPA Private Service Edge.
- Identify the Provisioning Key of the ZPA Private Service Edge group.
...
If needed, you can raise the maximum enrollment count. To learn more, see [Editing ZPA Private Service Edge Provisioning Keys](/zpa/edit-service-edge-provisioning-key).
- Stop the ZPA Private Service Edge. To learn more, see [Start, Stop, or Restart a ZPA Private Service Edge](/zpa/managing-deployed-zpa-private-service-edges#Starting).
- Wipe the configuration directory.
[admin@zpa-serivce-edge ~]$ sudo rm /opt/zscaler/var/service-edge/*
...
- Copy the provisioning key from the ZPA Admin Portal and use an editor to paste it into the file. To learn more, see [About the ZPA Private Service Edge Provisioning Keys](/zpa/about-zpa-service-edge-provisioning-keys).
- Save the provisioning key file.
- Start the ZPA Private Service Edge. The ZPA Private Service Edge service should pick up the provisioning key and re-enroll the ZPA Private Service Edge under the appropriate ZPA Private Service Edge group.
...
- []Verify the ZPA Private Service Edge has a stable connection to the ZPA Public Service Edge.
- Run the following command:
journalctl -n1000 | grep zscaler-update
[]Go to the [ZPA Private Service Edge page](/zpa/about-zpa-private-service-edges) and identify the ZPA Private Service Edge’s group.
- Go to the [provisioning key page](/zpa/about-zpa-service-edge-provisioning-keys) and find the ZPA Private Service Edge group. If the group is not listed in the ZPA Private Service Edge group column, the key is no longer in the ZPA Admin Portal.
- Delete the ZPA Private Service Edge and re-enroll it, which allows you to create a new provisioning key for the ZPA Private Service Edge.
[]If there is a failure when bringing up a new ZPA Private Service Edge, check the following:
...
If enrollment failure is still an issue, there are different possible reasons for ZPA Private Service Edge enrollment failures:
- [Have Less Than 100 Active ZPA Private Service Edges, But Can't Enroll More](#enrollmentFailure_amtServiceEdge)
- [Certificate Is Not Valid Yet](#enrollmentFailure_certificateNotValid)
[]You might experience issues where you cannot enroll a new ZPA Private Service Edge in the ZPA Admin Portal, but you have less than 100 active ZPA Private Service Edge.
Within the ZPA Admin Portal, you can have up to 100 ZPA Private Service Edges. This total includes both ZPA Private Service Edges enrolled that are active and ZPA Private Service Edges that are disabled, but not deleted. If you need to add more ZPA Private Service Edges, [delete](/zpa/about-zpa-private-service-edges#AbourSvcEdgePage) the disabled ZPA Private Service Edges and try enrolling your new ZPA Private Service Edge. To learn more, see [Configuring ZPA Private Service Edges](/zpa/configuring-service-edges).
[]If a ZPA Private Service Edges is not enrolling, you might see an error signifying that the certificate is not valid yet, similar to the following:
...
The ZPA Private Service Edges enrollment process involves the creation of certificates. If the ZPA Private Service Edge’s timestamp does not match the timestamp on the ZPA back-end servers, there can be a delay before the certificate becomes valid and enrolls the ZPA Private Service Edge.
To resolve this issue, verify that the time on the ZPA Private Service Edges is properly set and maintained. To learn more, see [Configure Network Time Protocol (NTP) Servers for a ZPA Private Service Edge](/zpa/service-edge-deployment-guide-amazon-web-services).
[]If the disk space utilization of the ZPA Private Service Edge exceeds 90%, the [configuration and binary snapshots](/zpa/managing-disaster-recovery-configuration-and-binary-snapshots) used for disaster recovery can no longer be captured until the low disk space condition is resolved.
...
%
[]When one of the supporting files fails to upgrade, the Upgrade Status of the ZPA Private Service Edge results in a Partial Failure. There are different reasons why the ZPA Private Service Edge can fail to install the supporting file:
- [ZPA Private Service Edge is not connected to a ZPA Public Service Edge.](#partialFailure_notConnectedToPublicSe)
- [ZPA Private Service Edge does not have enough bandwidth.](#partialFailure_notEnoughBw)
- [ZPA Private Service Edge does not have enough disk space.](#partialFailure_notEnoughDiskSpace)
If none of the above reasons are causing the Partial Failure, contact Zscaler Support.
[]View the [event logs](/zpa/about-events-diagnostics#Event) to confirm the connection to the ZPA Public Service Edge.
[]Monitor and reduce the [data throughput](/zpa/monitoring-private-service-edge-performance#throughput) of the ZPA Private Service Edge.
[]Installing the supporting files requires 150 MB of disk space for the initial installation, and then an additional 150 MB for the upgrade. If there is not adequate disk space for the upgrade, reduce the [disk space](/zpa/monitoring-private-service-edge-performance#diskspace) of the ZPA Private Service Edge.