...
| AppLearnTime | The time in microseconds taken for App Connectors to learn about the requested application and report the learned information to the central authority | %[OPT]s%[OPT]j%[OPT]J |
...
| CAProcessingTime | The time in microseconds taken for processing in the central authority | %[OPT]s%[OPT]j%[OPT]J |
...
| ClientZEN | The ZPA Public Service Edge that received the request from the Zscaler Client Connector | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
| ConnectionID | The application connection ID | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
| ConnectionSetupTime | The time taken by the App Connector to process a notification from the App Connector selection microservice and set up the connection to the application server | %[OPT]s%[OPT]j%[OPT]J |
...
| ConnectorZEN | The ZPA Public Service Edge that sent the request from the App Connector | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
| ConnectorZENSetupTime | The time in microseconds taken for setting up connection between the App Connector and ZPA Public Service Edge | %[OPT]s%[OPT]j%[OPT]J |
...
| Host | The destination host domain or IP address | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
...
| PolicyProcessingTime | The time in microseconds taken for processing the access policy associated with the application | %[OPT]s%[OPT]j%[OPT]J |
...
| ServerSetupTime | The time in microseconds taken for setting up connection at server | %[OPT]s%[OPT]j%[OPT]J |
...
| TimestampConnectionStart | The timestamp in microseconds when the ZPA Public Service Edge or ZPA Private Service Edge received the initial request from Zscaler Client Connector to start the connection | %[OPT]s%[OPT]j%[OPT]J |
| TimestampConnectionEnd | The timestamp in microseconds when the ZPA Public Service Edge or ZPA Private Service Edge terminated the connection | %[OPT]s%[OPT]j%[OPT]J |
| TimestampCATx | The timestamp in microseconds when the central authority sent a request to the ZPA Public Service Edge or ZPA Private Service Edge | %[OPT]s%[OPT]j%[OPT]J |
| TimestampCARx | The timestamp in microseconds when the central authority received a request from the ZPA Public Service Edge or ZPA Private Service Edge | %[OPT]s%[OPT]j%[OPT]J |
| TimestampAppLearnStart | The timestamp in microseconds when ZPA services start the process to learn about an application | %[OPT]s%[OPT]j%[OPT]J |
| TimestampZENFirstRxClient | The timestamp in microseconds when the ZPA Public Service Edge received the first byte from the Zscaler Client Connector | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| TimestampZENFirstTxClient | The timestamp in microseconds when the ZPA Public Service Edge sent the first byte to the Zscaler Client Connector | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| TimestampZENLastRxClient | The timestamp in microseconds when the ZPA Public Service Edge received the last byte from the Zscaler Client Connector | %[OPT]s%[OPT]j%[OPT]J |
| TimestampZENLastTxClient | The timestamp in microseconds when the ZPA Public Service Edge sent the last byte to the Zscaler Client Connector | %[OPT]s%[OPT]j%[OPT]J |
| TimestampConnectorZENSetupComplete | The timestamp in microseconds when the ZPA Public Service Edge received a request from the App Connector to set up a data connection. The request from the App Connector is triggered by the initial request for a specific application from Zscaler Client Connector. | %[OPT]s%[OPT]j%[OPT]J |
| TimestampZENFirstRxConnector | The timestamp in microseconds when the ZPA Public Service Edge received the first byte from the App Connector | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| TimestampZENFirstTxConnector | The timestamp in microseconds when the ZPA Public Service Edge sent the first byte to the App Connector | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| TimestampZENLastRxConnector | The timestamp in microseconds when the ZPA Public Service Edge received the last byte from the App Connector | %[OPT]s%[OPT]j%[OPT]J |
| TimestampZENLastTxConnector | The timestamp in microseconds when the ZPA Public Service Edge sent the last byte to the App Connector | %[OPT]s%[OPT]j%[OPT]J |
| Username | The username as entered into Zscaler Client Connector | %[OPT]s%[OPT]j%[OPT]J%[OPT]o |
| ZENBytesRxClient | The additional bytes received from Zscaler Client Connector since the last transaction log | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| ZENBytesTxClient | The additional bytes transmitted to Zscaler Client Connector since the last transaction log | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
...
| ZENTotalBytesRxClient | The total bytes received from Zscaler Client Connector by the ZPA Public Service Edge | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| ZENTotalBytesTxClient | The total bytes transmitted to Zscaler Client Connector from the ZPA Public Service Edge | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| ZENTotalBytesRxConnector | The total bytes received from the App Connector by the ZPA Public Service Edge | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |
| ZENTotalBytesTxConnector | The total bytes transmitted to the App Connector from the ZPA Public Service Edge | %[OPT]d%[OPT]x%[OPT]f%[OPT]o |