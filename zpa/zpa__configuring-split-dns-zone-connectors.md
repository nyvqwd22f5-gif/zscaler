...
Source: https://help.zscaler.com/zpa/configuring-split-dns-zone-connectors
PDF: https://help.zscaler.com/pdf/com/en/1484351.pdf

You can implement a split DNS configuration on your [App Connectors](/zpa/about-connectors) by using the Linux based service Unbound. Unbound is a validating, recursive, and caching DNS server and it enables you to build a cache-only or forwarding DNS server. However, it cannot be used as an authoritative DNS server, so it cannot be used to host custom domain name records. To learn more, see the [Unbound documentation](https://nlnetlabs.nl/documentation/unbound/unbound/).
Prerequisites
...
- Configure DNS resolution on a ZPA App Connector by editing /etc/resolv.conf and making sure the line nameserver has a value of 127.0.0.1
