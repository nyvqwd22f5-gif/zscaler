...
Source: https://help.zscaler.com/isolation/creating-isolation-profiles-zia
PDF: https://help.zscaler.com/pdf/com/en/1447211.pdf

[Watch a video about creating a ZIA Profile for Isolation.](https://fast.wistia.net/embed/iframe/qjllwn7ytu)
...
You can use [ZPA isolation profiles](/isolation/creating-isolation-profiles-zpa) to create policies in Zscaler Private Access (ZPA) to isolate specific web applications. To learn more, see [About Isolation Policy](/zpa/about-isolation-policy).
For any organization that is using Isolation, ZIA and ZPA automatically create default isolation profiles. You can use the default isolation profiles or manually create isolation profiles to use in ZIA and ZPA policies. To learn more, see [Default Isolation Profiles in Isolation](/isolation/default-isolation-profiles-isolation).
For certain levels of ZIA and Isolation integration access, admins are provided with preconfigured profiles that are only partially editable. To learn more, see [Understanding Isolation Miscellaneous and Unknown Category in ZIA](/isolation/understanding-isolation-miscellaneous-and-unknown-categories-zia) and [Editing Your Isolation Profile for ZIA](/isolation/editing-your-isolation-profile-zia).
Prerequisites
...
- Go to **Administration** > **Secure Browsing** > **Browser Isolation**.
...
[See image.](#Manage-profiles-dashboard)
...
- From the **Root Certificate** drop-down menu, select at least one file. The **Zscaler Root Certificate** that ZIA uses for SSL inspection appears by default in the drop-down menu. If your organization uses custom root certificates for SSL inspection, you can add them before creating isolation profiles. You can [add](/isolation/adding-root-certificates-isolation-zia) up to 10 root certificates for your organization. To learn more, see [About ZIA Root Certificates for Isolation](/isolation/about-root-certificates-isolation-zia).
...
- **Votiro Policy Name**: Select the Votiro policy to enforce the sanitization. If you do not select a policy, a default Votiro policy is applied. To learn more, see [Configuring Votiro Integration for Isolation](/isolation/configuring-votiro-integration-isolation) and [About Partner Integrations](/zia/about-partner-integrations).
...
- From the drop-down menu, select an **Isolation Banner**. The option you choose shows a preview banner in the window. Choose from existing banners, or create custom isolation banners to use for your isolation profiles. To learn more, see [Adding a Banner Theme for the Isolation End User Notification in ZIA](/isolation/adding-banner-theme-isolation-end-user-notification-zia).
...
- **Browser-in-browser experience**: This mode provides the user with the complete look and feel of an isolated session experience. To learn more, see [User Experience Modes in Isolation](/zia/user-experience-modes-cloud-browser-isolation).
...
- (Optional) Enable **Language Translation**. This option allows the user to translate any text from isolated web pages to the language of the user's choice. To learn more, see [Using the Isolation Bar in Native Browser Experience](/isolation/using-isolation-bar-native-browser-experience).
[See image.](#Enable-Language-Translation)
- Click **Save**.
After you save your new isolation profile, it appears in the list of ZIA isolation profiles. To edit a profile directly from the list, click the **Edit** icon. To learn more, see [Editing Your ZIA Isolation Profile](/isolation/editing-your-zia-isolation-profile-0) and [Deleting Your ZIA Isolation Profile](/isolation/deleting-your-zia-isolation-profile-0).
[See image.](#Administration_Isolation-Profiles_New-Profile)
You can use this isolation profile to create a policy in ZIA to allow traffic forwarding through browser isolation. To learn more, see [Configuring ZIA for Isolation](/isolation/configuring-zia-isolation).
[]![Go to Administration, then Secure Browsing, then Browser Isolation.](/downloads/isolation/profiles/zia-profiles/creating-zia-isolation-profile-isolation/ZIA_Admin_Secure-Browsing_Browser-Isolation_squircles.png)
[]![ZIA Isolation Profiles Dashboard](/downloads/isolation/profiles/zia-profiles/creating-zia-isolation-profile-isolation/Admin_Browser-Isolation_Isolation-Profiles_Add-Profile_squircle.png)
...
[]![View your new isolation profile in the list after saving your changes. ](/downloads/isolation/profiles/zia-profiles/creating-zia-isolation-profile-isolation/Admin_Browser-Isolation_Isolation-Profiles_1-new-profile_squircle.png)
