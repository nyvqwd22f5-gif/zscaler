...
PDF: https://help.zscaler.com/pdf/com/en/1477756.pdf

[Watch a video on Data Posture Policies.](https://fast.wistia.net/embed/iframe/pwx62y9unc)
DSPM supports creating custom security policies to expand security coverage to your cloud infrastructure beyond the predefined security policies.
You can do the following:
- Add a new custom security policy to an existing protected cloud resource. For example, you can create a custom security policy to ensure that the Amazon S3 buckets containing sensitive data are encrypted.
- Add a new custom security policy to resources that currently do not have predefined security policies. For example, you can create custom security policies to protect a new datastore in your cloud infrastructure.
To create a custom security policy:
- Go to **Policy** > **Data Posture Policies**.
-
On the **Policy** page, click **Create Policy**.
[See image.](#ds-newpolicy)
- Select your **Cloud Type**.
- For **Primary Resource Type**, select the required resource (EC2 instance, RDS instance, etc.).
-
Build the query by selecting the required predicates.
- [View the list of predicates and operators](#ds-query)
[See image.](#ds-buildquery)
- Click **Test Run**.
-
The query result is displayed.
[See image.](#ds-queryresult)
- Click **Accept and Proceed**.
-
On the **Remediation Procedure** page, click **Add Remediation Procedure**.
[See image.](#ds-remedysteps)
- Enter the remediation procedure in Markdown format, then click **Save**.
- Click **Next**.
-
On the **Severity and Description** page, enter the following details:
- **Policy Name**: The name of the security policy.
- **Policy Description (Optional)**: The description of the policy.
- **Policy Severity**: Static value signaling the severity of policy failure. The value can be Critical, High, Medium, or Low.
- **Policy Category (Optional)**: Select the threat category from the drop-down menu.
- **MITRE ATT&CK (Optional)**: Select the MITRE technique ID that must be associated with this policy.
[See image.](#ds-severity)
- Click **Finish**.
[]
...
[]![Create a custom policy](/downloads/dspm/data-posture-policies/creating-custom-policies/ds-policypage1.png)
[]![Build the policy query](/downloads/dspm/data-posture-policies/creating-custom-policies/ds-querybuilder1.png)
[]![Query result](/downloads/dspm/data-posture-policies/creating-custom-policies/ds-policytestrun.png)
[]![Add the remediation steps](/downloads/dspm/policies/creating-custom-policies/ds-add-remediation.png)
[]![Adding the severity details](/downloads/dspm/data-posture-policies/creating-custom-policies/Adding%20the%20Severity%20Details_new.png)