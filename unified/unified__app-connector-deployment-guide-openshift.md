...
Source: https://help.zscaler.com/unified/app-connector-deployment-guide-openshift
PDF: https://help.zscaler.com/pdf/com/en/1496161.pdf
...
- Pointing the Helm Chart to [the latest Docker tag](/unified/app-connector-deployment-guide-openshift#image-version) and upgrading regularly.
- Protecting App Connectors using firewall policies.
Some organizations choose to firewall or otherwise restrict outbound traffic to the internet from the data center. It is possible to deploy an App Connector in such an environment as long as the App Connector is able to reach all Zscaler data centers containing Public Service Edges. To learn more, see [firewall configuration information](https://config.zscaler.com/private.zscaler.com/zpa) for your deployment and [App Connector Security Guidance and Firewall Requirements](/unified/app-connector-deployment-guide-openshift#SecurityAndFirewallReqs).
...
Only one Helm installation can be done with the default Helm Chart. If you need to install a new Helm Chart, delete or upgrade the current Helm Chart. You can also use the multiple App Connector groups deployment option if you want to deploy new App Connectors with new provisioning keys. To learn more, see [Deployment of Multiple App Connector Groups](/unified/app-connector-deployment-guide-openshift#multiple-connector-groups).
[]
...
[]
- To add the app-connector-openshift Helm repository, use the following command:
...
You can retrieve the provisioning key from the [Admin Portal](https://admin.private.zscaler.com/). To learn more, see [About App Connector Provisioning Keys](/unified/about-app-connector-provisioning-keys).
...
- Check that the deployment is [meeting your sizing requirements](/unified/managing-deployed-app-connectors#VerifySizing).
[]
...
- [Create application segments](/unified/configuring-defined-application-segments) and [access policy rules](/unified/about-policies) to pass application traffic through.
...
[]
The threshold for App Connectors using the provisioning keys in an App Connector group is 100. The pods are ephemeral and consume a provisioning key utilization count each time they run. To avoid exceeding the App Connector threshold, Zscaler recommends configuring Auto Delete in your App Connector settings to remove App Connectors with a disconnected status after a set number of days. To learn more, see [Configuring App Connectors Page Settings](/unified/configuring-app-connectors-settings).
[]
...
These Helm Chart values only need to be changed if you need to customize the Helm Chart. The domain (e.g., api.private.com) in the echo statement will depend on what cloud you are on.
- Open `values.yaml`.
...
Provide a meaningful identifier to identify the new provision key under `secretIdentifier`, for example, an identifier matching the one in the Admin Portal. The domain (e.g., api.private.com) in the echo statement will depend on what cloud you are on.
...
Use a name for the release that does not conflict with release names for other App Connector deployments. The same namespace will be used for all App Connectors when deployed using this method. [Horizontal Pod Autoscaling](/unified/app-connector-deployment-guide-openshift#horizontal-pod-autoscaler) occurs in each App Connector group per deployment.
