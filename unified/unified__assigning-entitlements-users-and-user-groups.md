...
Source: https://help.zscaler.com/unified/assigning-entitlements-users-and-user-groups
PDF: https://help.zscaler.com/pdf/com/en/1517831.pdf

You can assign an individual ZIdentity user or user group to a Zscaler service. You must [add users](/unified/adding-users-0) or [user groups](/unified/adding-user-groups) before assigning them to a service. You can assign users to perform administrative tasks via [administrative entitlements](/unified/about-administrative-entitlements) or assign them as end users via [service entitlements](/unified/about-service-entitlements). To view the list of entitlements for a specific user, see [Viewing Entitlements to Assigned to Users](/unified/viewing-entitlements-assigned-users).
[]Administrative Entitlements
To assign users to a Zscaler service for performing administrative tasks:
-
Go to **Administration **> **Admin Management **> **Role Based Access Control** > **Administrative Entitlements **> service's name.
...
-
Go to **Administration **> **Entitlements **> **End User Entitlements **> service's name.
...
- On the **Select Groups & Roles** tab, select the user groups you want assigned to the Zscaler service.
-
In the **Role **column, the drop-down menu includes a list of roles configured in the Admin Portal (e.g., the [Role Management](/unified/about-role-management) page in the Internet & SaaS service). Select the role that must be assigned to each user group.
[See image.](#select-groups-and-roles-admin)
If you want to apply the same role for all groups, enable the **Set same role for all selected groups** toggle, and select the preferred role from the drop-down menu.
[See image](#select-groups-roles-common-admin)
-
The **Microtenant **column is visible only for the Private Applications service. Select the Microtenant for the admins within the Private Applications service in the Admin Portal. To learn more, see [About Private App Microtenants](/unified/about-private-app-microtenants).
[See image](#Assign-groups-page).
If no Microtenant is configured in the Private Applications, you must select **Default** in the **Microtenant **column.
- Click **Next**.
-
...
- On the **Select Users & Roles** tab, select the users that you want assigned to the Zscaler service.
-
In the **Role **column, the drop-down menu includes a list of roles configured in the Admin Portal (e.g., the [Role Management](/unified/about-role-management) page in the Internet & SaaS service). Select the role that must be assigned to each user.
[See image](#users-and-roles-admin)
If you want to apply the same role for all groups, enable the **Set same role for all selected users **toggle, and select the preferred role from the drop-down menu.
[See image](#users-and-roles-admin-common)
-
The **Microtenant **column is visible only for the Private Applications service. This is the [Microtenant](/unified/configuring-microtenants) that you can assign to other admins:
...
- If you have Restricted Full permission to manage administrative entitlements in ZIdentity, and are assigned a **Default** Microtenant in Private Applications, you can assign any Microtenant to other admins. You can also edit or delete any Microtenant.
-
...
[See image.](#admin-individual-user)
If no Microtenant is configured in the Private Applications, you must select **Default** in the **Microtenant **column.
- Click **Next**.
-
On the **Summary **tab, review the assignment details and click **Assign**.
...
In the **Assign Groups** wizard:
-
On the **Select Groups **tab, select the user groups that you want assigned to the Zscaler service.
...
In the **Assign Users **wizard:
-
On the **Select Users **tab, select the users that you want assigned to the Zscaler service.
