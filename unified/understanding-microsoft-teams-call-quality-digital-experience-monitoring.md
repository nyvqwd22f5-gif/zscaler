...
Source: https://help.zscaler.com/unified/understanding-microsoft-teams-call-quality-digital-experience-monitoring
PDF: https://help.zscaler.com/pdf/com/en/1494466.pdf
...
When a call has ended, Call Quality data is retrieved using the Microsoft Graph API. Currently, the Microsoft Graph API only provides data for the entire call after the call has completed, and does not provide incremental data while a call is in progress. User and device information garnered from Microsoft for call participants is mapped to Digital Experience Monitoring users and devices.
To monitor calls or meetings with Microsoft Teams Call Quality, you first must configure the application and meet the feature prerequisites. To learn more, see [Configuring Microsoft Teams Call Quality for Digital Experience Monitoring](/unified/configuring-microsoft-teams-call-quality-digital-experience-monitoring).
To access Microsoft Teams Call Quality monitoring, do one of the following from the Admin Portal:
- Go to **Applications **and click **Microsoft Teams** **Web App** in the Applications  page.
- Click the **Microsoft Teams Call Quality** application card.
[See image.](#cqm-card)
Viewing Microsoft Teams Call Quality Data
From the Microsoft Teams Call Quality page, you can set filters for time range, Departments, Zscaler Locations, User Groups, Geolocations, Location Groups, Last Mile ISPs, and Operating System to help assess Call Quality data. For time range, select a period between 2 Hours and 48 Hours, or a custom range to specify a date and time. The default time range is 2 Hours. To learn more about Department and Location filters, see [Monitoring the Applications Overview](/unified/monitoring-applications-overview).
...
- **ZDX Score**: The ZDX Score for Call Quality reflects the entire duration of a call for all users. The score is based on the MOS or metric thresholds. To learn more, see [Calculating the ZDX Score](#Calculating).
...
You can click usernames within the page to view Digital Experience Monitoring details for user device, application, and metrics. However, details are not available for users and devices identified as follows:
