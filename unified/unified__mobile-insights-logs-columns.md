...
Source: https://help.zscaler.com/unified/mobile-insights-logs-columns
PDF: https://help.zscaler.com/pdf/com/en/1497366.pdf

You can customize your web logs by using column fields. To learn more about logs, see [About Insights Logs](/unified/about-insights-logs).
...
- **Client Connection Cipher**: The cipher suite agreed upon during the SSL handshake between the client and the Internet & SaaS Public Service Edge. This filter applies to SSL-inspected traffic.
- **Client Connection TLS Version**: The version of TLS used for communication between the client and the Internet & SaaS Public Service Edge. This filter applies to SSL-inspected traffic.
...
- **MD5**: Displays the hash of suspicious files. Click to view the [Sandbox Detail Report](/unified/viewing-sandbox-reports-and-data#about-sandbox-detail-report) (requires Advanced Sandbox) or [CrowdStrike Endpoint Hits report](/unified/viewing-crowdstrike-endpoint-hits-report) (if you're [integrated with CrowdStrike](/unified/integrating-crowdstrike)).
...
- **Consumer App Control**: This refers to the [Consumer rule](/unified/adding-consumer-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
...
- **Productivity & CRM Tools App Control**: This refers to the [Productivity & CRM tools rule](/unified/adding-productivity-crm-tools-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
- **File Sharing App Control**: This refers to the [File Sharing rule](/unified/adding-file-sharing-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
...
- **Instant Messaging App Control**: This refers to the [Instant Messaging rule](/unified/adding-instant-messaging-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
...
- **Sales & Marketing App Control**: This refers to the [Sales & Marketing rule](/unified/adding-sales-marketing-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
- **Sandbox**: This refers to the [Sandbox policy](/unified/about-sandbox).
- **Social Networking/Blogging App Control**: This refers to the [Social Networking & Blogging rule](/unified/adding-social-networking-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
...
- **System & Development App Control**: This refers to the [System & Development rule](/unified/adding-system-development-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
- **URL Filtering**: This refers to the [URL filtering policy](/unified/about-url-filtering).
- **Webmail App Control**: This refers to the [Webmail rule](/unified/adding-webmail-rule-cloud-app-control) for the [Cloud App Control policy](/unified/about-cloud-app-control).
...
- **Proxy Latency (ms)**: The time, in milliseconds, added to the transaction by the Internet & SaaS Public Service Edge.
...
- **Server Certificate Self Signed**: If the certificate presented by the Origin Content Server (web server) to the Internet & SaaS Public Service Edge was self-signed.
- **Server Certificate Validation Type**: Validation type for the certificate presented by the server to the Internet & SaaS Public Service Edge (EV, OV, or DV).
- **Server Certificate Validity Period**: The validity duration of the certificate presented by the server to the Internet & SaaS Public Service Edge (i.e., how long is the certificate valid for?).
- **Server Connection Cert Chain Validity**: Whether the certificate presented by the server passed the validation check by the Internet & SaaS Public Service Edge or not. The certificate is deemed valid if the certificate is not expired, the signing authority is trusted by Zscaler, OCSP check is passed, the domain name matches the CN/SAN, etc.
- **Server Connection Cert Expiry**: If the certificate presented by the server to the Internet & SaaS Public Service Edge expired or not (Y/N).
- **Server Connection Cipher**: The cipher suite agreed upon during the SSL handshake between the Internet & SaaS Public Service Edge and the server.
- **Server Connection OCSP Result**: If the OCSP check done by the Internet & SaaS Public Service Edge passed or failed.
- **Server Connection TLS Version**: The version of TLS used for communication between the Internet & SaaS Public Service Edge and the server.
...
- **Server Wildcard Certificate**: If the certificate presented by the server to the Internet & SaaS Public Service Edge a wildcard certificate or not.
- **SSL Inspected**: Displays **Yes** if the SSL transaction was decrypted. Otherwise, it displays **No**.
- **Suspicious Content**: This field provides the “raw” Page Risk Index score of a URL. To learn more about the Suspicious Content Protection (Page RiskTM), see [About Advanced Threat Protection](/unified/configuring-advanced-threat-protection-policy).
...
- **Traffic Forwarding**: Type of traffic forwarding mechanism for this session. For aggregated sessions, this is the traffic forwarding type of the last session in the aggregate. To learn more about traffic forwarding, see [Best Practices for Traffic Forwarding](/unified/best-practices-traffic-forwarding).
