# Configuring a Probe
Source: https://help.zscaler.com/unified/configuring-probe
PDF: https://help.zscaler.com/pdf/com/en/1492841.pdf

To configure a probe for an application:
- Go to one of the following:
- **Policies **>** Digital Experience Monitoring **>** Configuration** > **Applications**, under the **Predefined Applications** or **Custom Applications** sections, expand an application** **row within the table and click **Add New Probe**.
- **Policies **>** Digital Experience Monitoring **>** Configuration** > **Probes**, and click **Add New Probe**.
The **Add New Probe** window appears.
...
- In the **Add New Probe** window:
- [a. Configure Probe](#Configure_Monitor)
...
- **Run Frequency (minutes)**: Enter the number of minutes for how frequently your probe should run. The default probe frequency varies based on your subscription plan. To learn more, see [Ranges & Limitations](/unified/ranges-limitations#digital-experience#applications-probes).
- **Probe Class**: Defines the application type as either Predefined or Custom.
-
For **Probing Criteria**: Select the criteria that you want to probe for the application. To learn more, see [Understanding Probing Criteria Logic](/unified/understanding-probing-criteria-logic).
...
By default, all user groups, users, Zscaler locations, location groups, departments, and devices in Internet & SaaS are applied. However, the maximum number of Zscaler locations cannot exceed 100 locations. If your organization has an Internet & SaaS account, each field defaults to your organization's information, such as the following examples:
...
- **Departments**: Dev Ops
- **Devices**: Desktop
When configuring Web probes for internal applications through Private Applications, probe only for users, user groups, and departments that use the application.
-
For **Exclusion Criteria**: Select the criteria that you do not want to monitor for the application. To learn more, see [Understanding Probing Criteria Logic](/unified/understanding-probing-criteria-logic).
...
By default, no user groups, users, Zscaler locations, location groups, departments, or devices are excluded. The following are examples of excluded criteria:
...
[See image.](#Protocol-Types-List)
If you choose Adaptive Mode, the best protocol for each leg in the cloud is selected via an auto-discovery process. If you choose any of the other protocols, they are used for the path of the Cloud Path probe. To learn more, see [Using Adaptive Mode](/unified/using-adaptive-mode).
...
For Private Applications, the recommended packet count is 3 packets, and the maximum is 6 packets.
...
- **Timeout (ms)**: The time to wait for a response to a probe packet before considering loss. The default is 1000, the minimum is 500, and the maximum is 5000. The recommended setting for Private Applications is 500.
...
Cloud Path probes are sent every 15 minutes by default for Standard Digital Experience Monitoring subscriptions, and every 5 minutes for Advanced Digital Experience Monitoring subscriptions.
...
[]![Review probe configuration](/downloads/zdx/configuration/probes/configuring-probe/zdx-config-probe-review3.png)
[]![Select a Protocol type when configuring a probe](/downloads/zdx/configuration/probes/configuring-probe/zdx-config-probe-protocol3.png)
[]![Example of Cloud Path probe configuration fields](/downloads/zdx/configuration/probes/configuring-probe/zdx-config-probe-cp-fields6.png)
[]![Add New Probe](/downloads/zdx/configuration/probes/configuring-probe/zdx-config-probe-add-new-probe3.png)