...
PDF: https://help.zscaler.com/pdf/com/en/1401686.pdf
...
This article illustrates how to configure two IPsec VPN tunnels from a SonicWall TZ 350 firewall to two ZIA Public Service Edges. As shown in the following figure, the corporate office sends its internal traffic to LAN port X0 in the internal network. It sends the outbound traffic to the WAN interface X1.
...
Zscaler IPSec tunnels support a limit of 400 Mbps for each public source IP address. If your organization wants to forward more than 400 Mbps of traffic, Zscalerrecommends using one of the following configurations:
...
Prerequisites
Ensure you have the [IP addresses of the ZIA Public Service Edges](/zia/locating-the-hostnames-and-ip-addresses-your-zens).
...
To configure the IPSec VPN tunnels:
-
...
This article covers only the configuration details of IPSec VPN tunnels between the SonicWall TZ 350 firewall and the ZIA Public Service Edges. For any other specific information about SonicWall TZ 350, refer to the SonicWall documentation.
...
- Verify that the default route is configured properly, so traffic can reach the ZIA Public Service Edges.
-
...
Zscalerrecommends that you select the **Policy Type** as **Site to Site** to forward all traffic to Zscaler. However, you can also select any other policy type based on your policy requirements.
- **Authentication Method**: Choose **IKE using Preshared Secret.**
- **Name**: Enter `ZscalerTunnel`.
- **IPsec Primary Gateway Name or Address**: Enter `199.168.148.132` (See the *Prerequisites* to learn how to locate ZIA Public Service Edge IP addresses for your organization.)
- **IPsec Secondary Gateway Name or Address**: Enter `104.129.194.39` (See the *Prerequisites* to learn how to locate ZIA Public Service Edge IP addresses for your organization.)
...
Zscalerrecommends that you select **LAN Subnets** in the **Local Networks** field to forward network traffic through Zscaler. However, you can select any subnet based on your traffic forwarding requirements.
...
- **Protocol**: Choose **ESP**.
-
**Encryption**: Choose **AES-GCM**-based encryption.
...
[]
![Screenshot of Configuring the Proposal tab for IKEv2 settings in SonicWall 350 UI](/downloads/zia/traffic-forwarding/ipsec/ipsec-vpn-configuration-guide-sonicwall-tz-350/sonicwall_TZ_350_proposals_tab_ikev2_proposal_page-phase%202.png)
[]
