...
Source: https://help.zscaler.com/unified/blocking-lan-access
PDF: https://help.zscaler.com/pdf/com/en/1492341.pdf
...
- Zscaler Client Connector versions 4.8 and later: Block inbound and outbound traffic in Z-Tunnel 1.0, Z-Tunnel 2.0, and Tunnel with Local Proxy using the [Endpoint Firewall Management](/unified/configuring-zscaler-client-connector-app-profiles#firewall) feature.
...
- [macOS](#mac)
[]To block LAN access for Windows devices:
...
- **Persistent**: If true, filtering remains on even when the user exits or logs out of Zscaler Client Connector, or turns off Internet & SaaS. The feature is disabled only after the user logs out. If the persistent key is false, filtering stops when the user exits Zscaler Client Connector, turns off Internet & SaaS, or logs out.
- **detectAltInterfaceTraffic**: When set to true, Zscaler Client Connector detects traffic that attempts to bypass the routing table.
-
...
- [Step 4: Enable Firewall Settings in the Admin Portal](#EnableFirewall)
Jamf Pro
...
- [Step 3: Enable Firewall Settings in the Admin Portal](#enable-firewall-settings-client-connector-portal)
...
- Click **Next**.
- In the **Review + create** section, review the summary, and click **Create**.
[]To create a property list file, see [Configuring a Custom Settings Profile](/unified/deploying-zscaler-client-connector-microsoft-intune-macos#configure-custom-profile).
- []In the Admin Portal, go to **Infrastructure > Connectors > Client**.
-
Click **Add macOS Policy**.
The **Add macOS Policy** window appears.
- In the **Add macOS Policy **window, under **Traffic Steering**, go to **App and IP Bypass **>** Firewall.**
...
- From the left-hand navigation, click **System Extensions** and select **Configure**.
...
- From the left-hand navigation, select **Content Filter**.
...
[]![Content filter section continued](/downloads/tech-pubs-drafts/client-connector-draft-articles/blocking-lan-access-mo-40040-doc-56696/Jamf-Configure-Firewall-Rules-Content-Filter-2.png)
...
If you want to change firewall parameters or rules, edit the Configuration Profile and redeploy it to macOS devices using Jamf Pro. No change is required to the App Profiles in the Admin Portal when changing existing firewall rules.
