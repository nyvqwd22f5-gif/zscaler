...
Source: https://help.zscaler.com/unified/configuring-auth0-external-idp
PDF: https://help.zscaler.com/pdf/com/en/1505481.pdf
...
- [4. Enable step-up authentication.](#enabling-step-up-authentication)
...
- Expand the **Advanced Settings **section, go to the **Endpoints **tab, and copy the **OpenID Configuration **value.
...
- Add the required scopes to the **Requested Scopes** field. By default, when the OIDC protocol is selected, the `openid`, `email`, and `profile` scopes are added.
...
- Paste the **Redirect URI** value copied from the Admin Portal to the **Application Login URI **and **Allowed Callback URLs **fields,
- Click **Save Changes**.
[See image.](#redirect-uri-auth0)
- []In the Admin Portal, go to **Administration > Identity > ZIdentity > External Identities**.
- Locate the IdP entry created for Auth0 under the **Primary Identity Provider **(or **Secondary Identity Providers**) tab and click the **Edit **icon.
...
- While mapping attributes, ensure that the attributes you enter in the **Just-in-time Attribute** field match exactly with the attributes received in the ID tokens.
- Click **Update**.
...
-
In the **Levels to Authentication context mapping** section, enter the **ACR Claim** value for each authentication level. To learn more about the supported ACR Claims in Auth0, refer to the [Auth0 Technical documentation](https://auth0.com/docs/secure/multi-factor-authentication/step-up-authentication/configure-step-up-authentication-for-web-apps).
...
[]
![Creating an application in Auth0 with the otpion to create a regular application highlighted](/downloads/zidentity/integration/external-idp-configuration/configuring-auth0-external-idp/zidentity-app-selection_0.jpg)
[]
![Configuring a cleint application in Auth0 showing the basic information](/downloads/zidentity/integration/external-idp-configuration/configuring-auth0-external-idp/zidentity-app-creation_0.jpg)
[]
![List of OIDC enpoints in Auth0 with the Well-Known Configuration URL highlighted](/downloads/zidentity/integration/external-idp-configuration/configuring-auth0-external-idp/zslogin-discovery-endpoint-auth0.jpg)
[]
![Configuring SSO in Autho for an application with the Application URIs highlighted](/downloads/zidentity/integration/external-idp-configuration/configuring-auth0-external-idp/zslogin-redirect-uri-auth0.jpg)
[]
...
[]
![Configuring JIT provioning for an IdP in ZIdentity Admin Portal](/downloads/zidentity/integration/external-idp-configuration/configuring-auth0-external-idp/zidentity-jit-mapping-no-groups%20(1).png)
[]
![Click the link in the banner to go to the Experience Center](/downloads/unified/administration/zidentity/idp-configuration/external-idp-configuration/configuring-auth0-external-idp/zid-landing-page-withbanner.png)