...
Source: https://help.zscaler.com/unified/about-network-segments
PDF: https://help.zscaler.com/pdf/com/en/1532364.pdf

A Network segment is a local area network (LAN) IP subnet that is assigned to a [Network Connector group](/unified/about-network-connector-groups) to define access to servers that are accessed via the VPN tunnel. Network segments define which traffic flows through the VPN tunnel from Zscaler Client Connector. Each Network segment has a LAN IP address range for a data center with running servers. You can also assign an FQDN and domain name server to the Network segment. You can create multiple Network segments to define which traffic flows over the VPN tunnel. When you assign a Network segment to a Network Connector group, ensure that the Network Connector and the server associated with the LAN IP address are from the same data center.
...
On the Network Segments page (Infrastructure > Private Access > Component > VPN (For Legacy Apps) > Network Segments), you can do the following:
...
- [Add a Network segment.](/unified/configuring-network-segments)
...
- [Edit an existing Network segment](/unified/editing-network-segments).
- Delete a Network segment.
- Display more rows or a different page of the table.
![Network Segments page in the Admin Portal](/downloads/zpa/about-vpn-segments/Network%20Segments%20page%20w%20steps_0.png)