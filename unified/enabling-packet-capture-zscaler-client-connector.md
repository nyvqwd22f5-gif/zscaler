...
PDF: https://help.zscaler.com/pdf/com/en/1334736.pdf
...
If you use Zscaler Client Connector version 4.8 and later for Windows, you can configure access to local packet capture on the [app profile](/zscaler-client-connector/configuring-zscaler-client-connector-app-profiles#notification). Packet capture access that is enabled on the app profile overrides this setting.
...
Packet capture files are exported to the archive that is generated when using the **Export Logs** option from Zscaler Client Connector. To learn more, see [Using Zscaler Client Connector](/zscaler-client-connector/using-zscaler-client-connector).
