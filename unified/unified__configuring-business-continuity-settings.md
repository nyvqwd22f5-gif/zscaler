...
Source: https://help.zscaler.com/unified/configuring-business-continuity-settings
PDF: https://help.zscaler.com/pdf/com/en/1528646.pdf

Business Continuity provides access to your applications during Private Applications-related cloud outages or Internet Service Provider (ISP) outages. Configuring the [Business Continuity settings](#bcsettings) in Private Applications is the first step in enabling Business Continuity for your organization. To learn more about additional prerequisites and an overview of the feature, see [Understanding Business Continuity](/unified/understanding-business-continuity).
...
If you are using [Zscaler-managed Business Continuity](/unified/understanding-zscaler-managed-business-continuity-cloud), this section can be skipped as Zscaler provides a Business Continuity domain (e.g., tenant_name.zpa.zsbcs.net).
...
| Private Service Edges for Private Applications and App Connectors | The DNS request to get the Private Cloud Controller's IP address is `co2bcp.<business_continuity_domain>` for App Connectors and `pb2bcp.<business_continuity_domain>` for Private Service Edges for Private Applications.Based on the DNS resolution, the Private Service Edge for Private Applications or App Connectors establish a TLS connection to the Private Cloud Controller. |
...
| Zscaler Client Connector TLS connections to the Private Service Edge for Private Applications | Enrolled users' SNI is `c2.<business_continuity_domain>`New users' SNI is `c2site.<business_continuity_domain>` |
[]Configuring Business Continuity Settings
To configure Business Continuity settings for Private Applications:
- Go to **Infrastructure **>** Private Access **>** Business Continuity **>** Settings**.
...
- Zscaler Client Connector to contact the Private Cloud Controller and Private Service Edges for Private Applications (e.g., any2bcp.<business_continuity_domain>, c2site.<business_continuity_domain> for new users, and c2.<business_continuity_domain> for enrolled users)
- App Connectors for control channel to Private Cloud Controller (e.g., co2bcp.<business_continuity_domain>)
- Private Service Edges for control channel to Private Cloud Controller (e.g., pb2bcp.<business_continuity_domain>)
...
Mutual TLS authentication between users and Private Service Edges for Private Applications is not supported for new user enrollment during Business Continuity.
...
To learn more about restoring Private Cloud Controller backups, see [Restoring Private Cloud Controller Backups](/unified/restoring-private-cloud-controller-backups).
- []**Maximum Wait Time to Enter Business Continuity Mode (App Connectors and Private Service Edges)**: Enter an integer to specify the duration (in minutes or hours) that App Connectors and Private Service Edges for Private Applications must wait before switching to Business Continuity. You can select the duration as **Minute(s)** or **Hour(s) **from the drop-down menu. By default, **2 Minute(s)** is selected. The minimum limit for this field is 1 minute. The maximum limit for this field is 8 hours, or 480 minutes.
...
- **Maximum Wait Time to Exit Business Continuity Mode (Users)**: Enter an integer to specify the duration (in minutes, hours, or days) that Private Service Edges for Private Applications must wait before sending a message to Zscaler Client Connector to try and re-establish the control and data channel to the ZTE. You can select the duration as **Minute(s)**, **Hour(s)**, or **Day(s) **from the drop-down menu. By default, **60 Minute(s)** is selected. The minimum supported value is 10 minutes. The maximum limit for this field is 7 days, 168 hours, or 10,080 minutes. This field is visible only after **Switch Users to Public Cloud** is enabled.
- []**Existing IdP for Authentication**: Select this option to use the same IdP configuration that is configured for user authentication for normal operations under **Administration > Identity **> **Private Access **>** IDP Configuration**. To learn more, see [About IdP Configuration](/unified/about-idp-configuration).
