...
Source: https://help.zscaler.com/unified/adding-custom-dlp-dictionaries
PDF: https://help.zscaler.com/pdf/com/en/1493106.pdf

Adding a custom Data Loss Prevention (DLP) dictionary is one of the tasks you can complete when configuring DLP policy rules. To learn more, see [Configuring DLP Policy Rules with Content Inspection](/unified/configuring-dlp-policy-rules-content-inspection).
...
To learn more about the ranges and limitations for custom DLP dictionaries, see [Ranges & Limitations](/unified/ranges-limitations).
...
You can also [edit](/unified/editing-predefined-dlp-dictionaries) or [clone](/unified/cloning-predefined-dlp-dictionaries) the predefined dictionaries Zscaler provides. Any combination of cloned, predefined, and custom dictionaries can be added to a DLP engine, which is what you must reference when creating DLP policies. To learn more, see [About DLP Engines](/unified/about-dlp-engines).
[]
-
...
-
**Exact Data Match**: If selected, the **Exact Data Match** section appears below, where you can select existing EDM templates and add data fields from those templates. To learn more, see [Creating an Exact Data Match Template](/unified/creating-exact-data-match-template) and [Defining Exact Data Match Fields for Custom DLP Dictionaries](/unified/defining-exact-data-match-fields-custom-dlp-dictionaries).
[See image.](#EDM)
-
**Indexed Document Match**: If selected, the **Indexed Document Match** section appears below, where you can select existing IDM templates and choose match accuracy levels for those templates. To learn more, see [Creating an Indexed Document Match Template](/unified/creating-indexed-document-match-template) and [Defining IDM Match Accuracy for Custom DLP Dictionaries](/unified/defining-idm-match-accuracy-custom-dlp-dictionaries).
[See image.](#IDM)
-
**Microsoft Information Protection (MIP)**: If selected, the MIP labels appear in the table below, where you can select the MIP labels. To learn more, see [Adding a MIP Account](/unified/adding-mip-account) and [Defining Microsoft Protection Labels for Custom DLP Dictionaries](/unified/defining-microsoft-information-protection-labels-custom-dlp-dictionaries).
[See image.](#MIPs)
- **Patterns & Phrases**: If selected, the **Patterns **and **Phrases **sections appear below, where you can add patterns, phrases, and apply actions to them. To enable Unicode phrases defined in the Phrases list to be matched if they are adjacent to other characters without any delimiter in between, select the **Non-delimited Unicode Phrase matching **checkbox. For example, the phrase “クレジットカード” matches “クレジットカードのコピーをください”. Matches happen even when no spaces are detected. To learn more, see [Defining Patterns for Custom DLP Dictionaries](/unified/defining-patterns-custom-dlp-dictionaries) and [Defining Phrases for Custom DLP Dictionaries](/unified/defining-phrases-custom-dlp-dictionaries).
