...
PDF: https://help.zscaler.com/pdf/com/en/1443106.pdf

This article describes how to configure the Client Certificate for Linux. You must perform these steps to successfully use the Client Certificate posture check for Linux in the Zscaler Client Connector Portal.
The Client Certificate posture check verifies the following conditions:
- The certificate authority (CA) certificate uploaded to the Zscaler Client Connector Portal can be trusted by the Linux client system.
...
- In the Zscaler Client Connector Portal, upload a CA certificate issued by an internal root CA trusted by the organization’s users. You can upload one of the following certificates:
...
- Upload the root certificate `ca.crt` to the Zscaler Client Connector Portal.
