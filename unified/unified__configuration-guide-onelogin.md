...
Source: https://help.zscaler.com/unified/configuration-guide-onelogin
PDF: https://help.zscaler.com/pdf/com/en/1491236.pdf

This guide provides information on how to set up Onelogin as an IdP for Private Applications.
Prerequisites
Ensure that you have the following:
- A Onelogin account with admin privileges.
- A Private Applicatoins account with an administrator role that allows you to add an IdP Configuration.
Configuring Onelogin for SSO
[]To configure Onelogin as the IdP for a Private Applications user and admin SSO:
...
- From the **More Actions** drop-down menu, select **SAML Metadata** to download the IdP's metadata file. You will need this file later in order to [complete the configuration](/unified/configuring-idp-single-sign) within the Admin Portal.
[See image.](#Step4_UserImg)
- Go to **Configuration**.
- For **Application Details**:
- Log in to the Admin Portal, and [complete step 2 of the new IdP configuration procedure](/unified/configuring-idp-single-sign#Collapse2). During this procedure, you must copy the identifier within the **Service Provider URL** string provided. The following is an example SP URL with the identifier highlighted in green:
...
- On the **All Users** page, click on the user you want to assign the Private Applications application to.
...
For Onelogin, the IdP metadata file you upload to the Admin Portal will populate the **Name**, **Single Sign-On URL**, and **IdP Entity ID** fields. **ZPA (SP) SAML Request **is set to **Signed** and the **IdP Certificate** is uploaded automatically.
