...
Source: https://help.zscaler.com/unified/adding-mcas-nss-feeds
PDF: https://help.zscaler.com/pdf/com/en/1496711.pdf

In order for your MCAS NSS feed to perform optimally, the virtual machine (VM) deployed for the NSS server requires a minimum of 8GB of RAM. To learn more, see [Deploying NSS Virtual Appliances](/unified/deploying-nss-virtual-appliances) and [Integrating with Microsoft Cloud App Security](/unified/integrating-microsoft-cloud-app-security).
If you deployed an NSS virtual appliance before the Internet & SaaS 5.6 release, ensure you have the latest version of Java installed into your VM.
...
- **URL Classes**: Use this filter to limit the logs to specific [URL classes](/unified/about-url-categories). Select those that you want to include. Multiple selections are allowed.
- **URL Super Categories**: Use this filter to limit the logs to specific [URL super categories](/unified/about-url-categories). Select those that you want to include. Multiple selections are allowed.
-
**URL Categories**:** **Use this filter to limit the logs to specific [URL categories](/unified/about-url-categories). Select those that you want to include. Multiple selections are allowed.
...
- An IP address with a netmask (e.g., 203.0.113.0/24)
You can enter multiple entries. Press `Enter` after each entry.
- **Cloud Application Classes**: Use this filter to limit the logs to the selected [cloud application classes](/unified/understanding-cloud-app-categories). Multiple selections are allowed.
-
**Cloud Applications**: Use this filter to limit the logs to selected [cloud applications](/unified/understanding-cloud-app-categories). Multiple selections are allowed.
...
- **Suspicious Content**: Use this filter to limit the logs based on the [Page Risk Index score](/unified/configuring-advanced-threat-protection-policy) of a transaction. Enter either a single value or a range of values, between 0 and 100. Multiple values separated by commas are allowed.
- []**Download File Type Category**: Use this filter to limit the logs based on the [category of the type of file downloaded](/unified/about-file-type-control) during the transaction. Multiple selections are allowed.
- **Download File Type**: Use this filter to limit the logs based on the [type of file downloaded](/unified/about-file-type-control) during the transaction. Multiple selections are allowed.
- **Upload File Type Category**: Use this filter to limit the logs based on the [category of the type of file uploaded](/unified/about-file-type-control) during the transaction. Multiple selections are allowed.
- **Upload File Type**: Use this filter to limit the logs based on the [type of file uploaded](/unified/about-file-type-control) during the transaction. Multiple selections are allowed.
- **Download User-Defined File Type**: Use this filter to limit the logs based on the [user-defined type of file downloaded](/unified/configuring-file-type-control-policy) during the transaction. Multiple selections are allowed. To learn more, see [Configuring Custom File Types](/unified/configuring-custom-file-types).
- **Upload User-Defined File Type**: Use this filter to limit the logs based on the [user-defined type of file uploaded](/unified/configuring-file-type-control-policy) during the transaction. Multiple selections are allowed. To learn more, see [Configuring Custom File Types](/unified/configuring-custom-file-types).
