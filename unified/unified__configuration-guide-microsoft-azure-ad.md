...
Source: https://help.zscaler.com/unified/configuration-guide-microsoft-azure-ad
PDF: https://help.zscaler.com/pdf/com/en/1491226.pdf
...
- For **Identifier (Entity ID)** enter the **Service Provider Entity ID** that is provided for you when you [configured a new IdP configuration](/unified/configuring-idp-single-sign) in the Admin Portal. This ID is specific to your IdP.
- For **Reply URL (Assertion Customer Service URL)** enter the **Service Provider URL** that is provided for you when you configured a new IdP configuration. This URL is specific to your IdP.
-
For **Sign on URL**, perform the following:
- [For Private Applications user SSO configuration](#step6_1_user)
- [For Private Applicatadmin SSO configuration](#step6_2_admin)
- Click **Save**.
...
- Under **SAML Signing Certificate**, for **Federation Metadata XML**, click the **Download** link to obtain the metadata file. You will need to upload this IdP metadata information to the Admin Portal later in order to complete the configuration.
[See image.](#Step9_UserImg)
- Leave **Set up Zscaler Private Access** (for Private Applications user SSO) or **Set up Zscaler Private Access Administrator** (for Private Applications admin SSO) as default, and skip to the next step.
Do not test the configuration at this time.
- In order for Azure AD users to authenticate through Private Applications, you must assign these users to the Private Applications application. In the application, click **Users and Groups** then **Add user**.
[See image.](#Step11_UserImg)
- Search for the user you want to assign to the Private Applications application.
...
- If you are configuring the Azure AD for user SSO and want to use [SCIM](/unified/about-scim), proceed to the [SCIM Configuration Guide for Microsoft Azure AD](/unified/scim-configuration-guide-microsoft-azure-ad).
- If you are configuring the Azure AD for user SSO, proceed to the [Using Roles for Group Mapping](#Roles4GroupMap_userSSO) procedure below, then [complete the IdP configuration](/unified/configuring-idp-single-sign). If you are configuring the Azure AD for admin SSO, you can go directly to the Admin Portal to complete the IdP configuration.
...
To configure group mapping in Azure AD, you must customize the role claim type in the SAML response token in order to push groups to Private Applications. To learn more about configuring role claims, see the [Microsoft product documentation](https://docs.microsoft.com/en-us/azure/active-directory/develop/active-directory-enterprise-app-role-management).
- [1. Adding Roles in the Private Applications Application](#AddingRoles4ZPAApp_userSSO)
...
If you are verifying your Private Applications admin SSO configuration, you can also go to [myapps.microsoft.com](http://myapps.microsoft.com/). From your Dashboard, click on the Private Applications** **app to initiate admin SSO.
...
"keyCredentials": [
You need the roles you added in order to complete step f of the [Assigning Roles to Groups](/unified/configuration-example-microsoft-azure-ad#AssigningRoles2Groups_userSSO) procedure below.
...
- Click **All applications**, then click the **Zscaler Private Access (ZPA) **application you added for the [Adding Roles in the Private Applications Application](/unified/configuration-example-microsoft-azure-ad#AddingRoles4ZPAApp_userSSO) procedure above.
...
- []Click **Select Role**, then choose the role you added for the [Adding Roles in the Private Applications Application](/unified/configuration-example-microsoft-azure-ad#AddingRoles4ZPAApp_userSSO) above (e.g., the **Engineer** role).
[See image.](#AzureRole4GroupMap_AddAssignmentSelectRole)
- Click **Select**.
- In the **Add Assignment** panel, click **Assign**.
[See image.](#AzureRole4GroupMap_AddAssignmentAssignRole)
Repeat this procedure for each role you added for the [Adding Roles in the Private Applications Application](/unified/configuration-example-microsoft-azure-ad#AddingRoles4ZPAApp_userSSO) procedure above.
...
- In the Admin Portal, go to **Administration **> **Identity **>** Private Access **>** IDP Configuration**.
...
- Sign in using your Azure AD credentials. The Private Applications authentication service automatically imports the memberOf attribute.
