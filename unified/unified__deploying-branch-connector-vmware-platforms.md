...
Source: https://help.zscaler.com/unified/deploying-branch-connector-vmware-platforms
PDF: https://help.zscaler.com/pdf/com/en/1517476.pdf
...
This procedure describes the steps for deploying Zscaler Branch Connector on VMware platforms. To learn more about deploying Branch Connector using a Terraform script, see[ Deployment Templates for Zscaler Branch Connector & App Connector](/unified/deployment-templates-branch-connector-app-connector).
[]Prerequisites
...
- Download the Branch Connector OVA image file for VMware ESXi from the [Branch Connector Images ](/unified/downloading-branch-connector-images)page.
- Branch Connector uses an API key to authenticate and register the VM with Zscaler. If you do not already have an API key, generate a new key. Then copy the API key from the [API Key Management ](/unified/about-api-configuration)page.
...
You attach the ISO to your Branch Connector VM in the [Deploying on VMware vSphere Hypervisor (ESXi)](#vsphere) procedure.
...
[]After you have met all the [prerequisites](#Prereqs), perform the following steps to deploy your Branch Connector on VMware vCenter:
...
[See image.](#SeeImage1)
- On the **Select an OVF template **page, select **Local file**. Then, upload the Branch Connector OVA image file that you previously downloaded from the [Branch Connector Images](/unified/downloading-branch-connector-images) page. Then, click **Next**.
[See image.](#seeimage2)
...
- **Provisioning Template URL**: Enter the [Branch Connector Provisioning URL](/unified/configuring-branch-configuration-template).
- **API Key**: Enter the API Key from the [API Key Management](/unified/about-api-configuration) page.
...
To avoid any time sync issues, Zscaler recommends enabling the** Synchronize guest time with host** option after deploying Branch Connector. To learn more, see the [VMware product documentation](https://kb.vmware.com/s/article/1014038).
...
[]After you have met all the [prerequisites](#Prereqs), created [user data](#userdatafile), and applied user data to the VM (if you used the [ISO method](#ISOMethod)), perform the following steps to deploy your Branch Connector on vSphere Hypervisor (ESXi):
...
- On the **Select OVF and VMDK files **page, enter a unique name for the VM and upload the Branch Connector OVA image file you previously downloaded from the [Branch Connector Images](/unified/downloading-branch-connector-images) page. Then, click **Next**.
...
- On the **Additional Settings** page, skip the additional properties for the VM, because values you enter on this page will not be configured. Click **Next**.
...
-
In the **Datastore browser **window that opens, click** Upload **to upload the `user-data.iso` file created in the ISO Method procedure. Then, click** Select**.
...
To avoid any time sync issues, Zscaler recommends enabling the** Synchronize guest time with host** option after deploying Branch Connector. To learn more, refer to the [VMware product documentation](https://kb.vmware.com/s/article/1014038).
...
- [Traffic Forwarding](/unified/configuring-traffic-forwarding-rules)
- [Log and Control Forwarding](/unified/configuring-log-and-control-forwarding-rule)
- [DNS Filtering](/unified/configuring-dns-filtering-rule)