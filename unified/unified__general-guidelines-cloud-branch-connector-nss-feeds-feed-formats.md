# General Guidelines for Cloud & Branch Connector NSS Feeds and Feed Formats
Source: https://help.zscaler.com/unified/general-guidelines-cloud-branch-connector-nss-feeds-feed-formats
PDF: https://help.zscaler.com/pdf/com/en/1519571.pdf
...
In the Admin Portal, enter the desired fields in the **Feed Output Format **text box (Infrastructure > Connectors > Cloud > Nanolog Streaming Service** **> NSS Feeds > Add NSS Feed).
Guidelines
...
- `"%s{time}"`,`"%s{login}"`,`"%s{proto}"\n` results in a log entry that looks like: `"Mon Sep 16 10:50:46 2024","jdoe@example.com", "HTTP"`
- `%s{time}\t%s{login}\t%s{proto}\n` results in a log entry that looks like:
`Mon Sep 16 10:50:46 2024jdoe@example.comHTTP`
...
- You can optionally include static text strings in the output. For example: `%s{time} GMT` produces: `Mon Sep 16 10:50:46 2024 GMT`
