...
Source: https://help.zscaler.com/unified/evaluating-cloud-path
PDF: https://help.zscaler.com/pdf/com/en/1494551.pdf

The Cloud Path provides a path visualization of metrics between hop points of traffic. It can capture a direct traffic path, as in the case of Zscaler Client Connector to Egress to destination, or tunneling through an Internet & SaaS Public Service Edge (from Zscaler Client Connector to Egress to Internet & SaaS Public Service Edge to destination). The Cloud Path has the following views:
...
[]Differential latency is shown over the different legs of the Cloud Path, and the leg with the highest latency displays in orange. The latency shown on top of the Internet & SaaS Public Service Edge represents the network latency between the Internet & SaaS Public Service Edge and the first router in the Zscaler Data Center. This might indicate Internet & SaaS Public Service Edge congestion. Click the magnifying glass icon to expand the Hop View and to see the individual values. To learn more about Service Edges, see [About Internet & SaaS Public Service Edges](/unified/about-public-service-edges), [Understanding Service Edges](/unified/understanding-service-edges), and [About Private Service Edges for Private Applications](/unified/about-private-service-edges-private-applications).
[See image.](#image-cloudpathhops)
[]You can see the flow of traffic through the applicable tunnels as per the configured policies. For GRE tunnels, the Hop View displays underlay hops in the routing path to provide more accurate metrics for latency and packet loss. In cases where the Internet & SaaS Private Service Edge is on a private network (such as a VPN) with a private IP address, the Hop View provides a path from the client to the Internet & SaaS Private Service Edge. To learn more about tunnels in the Cloud Path, see [About Tunnel Information in the Cloud Path](/unified/about-tunnel-information-cloud-path).
[See image.](#reverse-tr)
[]When Citrix is the network interface for TCP or UDP probes, Digital Experience Monitoring displays the path to the VPN Concentrator. Digital Experience Monitoring also supports the topology where traffic from the VPN Concentrator flows to the Internet & SaaS Public Service Edge via a GRE/IPSec tunnel.
...
The following scenario shows the External Proxy forwarding traffic to the Zscaler Service Edge through a GRE tunnel:
![External proxy forwarding traffic to Zscaler Service Edge](/downloads/zdx/analytics/users/evaluating-cloud-path/eproxy-gre-tunnel4.png)
[]If there are any errors, icons are displayed on the Cloud Path. Click the icon to display the error message. To learn more, see [Cloud Path Errors](/unified/cloud-path-errors).
[See image.](#CPErrors)
...
[]When accessing an internet application, you can see the path from Zscaler Client Connector to the application, and if there is an Internet & SaaS Public Service Edge or a Private Applications Public Service Edge in the path, that is also displayed. If a private application is accessed, you can view the path from Zscaler Client Connector to the Private Applications Public Service Edge or Private Applications Private Service Edge. In case you have configured a path for traffic through the Internet & SaaS Public Service Edge, both the Internet & SaaS Public Service Edge and the Private Applications Public Service Edge or the Private Applications Private Service Edge are seen in the Cloud Path. This is displayed in the Hop View and the Command Line View. To learn more about Service Edges, see [About Internet & SaaS Public Service Edges](/unified/about-public-service-edges), [Understanding Service Edges](/unified/understanding-service-edges), and [About Private Service Edges for Private Applications](/unified/about-private-service-edges-private-applications).
The following image shows both the Internet & SaaS Public Service Edge and Private Applications Public Service Edge in the Hop view. The Private Applications Public Service Edge sends packets to the App Connector in the path to the application. To learn more, see [About App Connectors](/unified/about-app-connectors), [About Internet & SaaS Public Service Edges](/unified/about-public-service-edges), and [Understanding Service Edges](/unified/understanding-service-edges).
[See image.](#ZPAinClodpath)
Both views display a 4-digit number for the Internet & SaaS Public Service Edge or Private Applications Public Service Edge. This internal hash value helps Zscaler Support quickly locate and troubleshoot any issues. To learn more, see [About Public Service Edges](/unified/about-public-service-edges) and [About Service Edges](/unified/understanding-service-edges).
...
Adaptive mode is not supported for internal applications through Private Applications. Only ICMP, TCP, and UDP are supported.
...
To learn how Adaptive mode works, see [Using Adaptive Mode](/unified/using-adaptive-mode). To learn more about setting probes for the Cloud Path, see [Configuring a Probe](/unified/configuring-probe).
Command Line View
Click the **Command Line View** tab to view further details about the Cloud Path, including the Hop Direction (probe direction, from the client to your egress IP ↓, from Internet & SaaS Public Service Edge or Internet & SaaS Private Service Edge, or Private Applications Public Service Edge or Private Applications Private Service Edge to your egress IP ↑, and from Service Edges to destination ↓), Region and Geolocation, Packet Loss (%) and Packets Failed, as well as Latency metrics. If there are any errors, they are indicated by an icon next to the IP address. Click the icon to display the error message. To learn more, see [Cloud Path Errors](/unified/cloud-path-errors), [About Internet & SaaS Public Service Edges](/unified/about-public-service-edges), [Understanding Internet & SaaS Private Service Edge](/unified/understanding-internet-saas-private-service-edge), [Understanding Service Edges](/unified/understanding-service-edges), and [About Private Service Edges for Private Applications.](/unified/about-private-service-edges-private-applications)
