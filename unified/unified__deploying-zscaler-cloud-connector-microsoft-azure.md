...
Source: https://help.zscaler.com/unified/deploying-zscaler-cloud-connector-microsoft-azure
PDF: https://help.zscaler.com/pdf/com/en/1518091.pdf
...
-
Terraform: The Terraform method provides support for a richer set of functions, including the option to deploy using Virtual Machine Scale Sets (VMSS). To learn more about the resources created when deploying Cloud Connector using a Terraform script, see [Deployment Templates for Zscaler Cloud Connector](/unified/deployment-templates-zscaler-cloud-connector).
...
- From the[ API Keys](/unified/about-api-configuration) page, retrieve and copy the API key. Cloud Connector uses the API key to authenticate and register the VM with Zscaler.
- Add a[ Location Template](/unified/configuring-location-template-0). Locations identify the various networks from which your organization sends its traffic.
- Add a [Cloud Provisioning Template](/unified/configuring-cloud-provisioning-template) and copy the Cloud Provisioning URL.
-
...
- **api-key**: For the **name **of your secret, enter `api-key`. For the corresponding **value**, enter the value from the [API Keys](/unified/about-api-configuration) page on the Admin Portal.
- **password**: For the **name **of your secret, enter `password`. For the corresponding **value**, enter your Admin Portal password.
- **username**: For the **name **of your secret, enter `username`. For the corresponding **value**, enter your Admin Portal username.
...
[]
Cloud Connector with VMSS deployment requires using the [Terraform Deployment Template with Virtual Machine Scale Sets (VMSS)](/unified/deployment-templates-zscaler-cloud-connector#t-templates-azure-vmss) and a new [Cloud Provisioning Template](/unified/configuring-cloud-provisioning-template) with **Auto Scaling** set to **Enabled**. To enable Auto Scaling, contact Zscaler Support.
...
- [Zscaler Internet Access Gateways](/unified/about-zscaler-internet-access-gateways)
...
If you are deploying Cloud Connector in China, Zscaler recommends creating a custom gateway with Zscaler China data centers and traffic forwarding policies referencing your China location and custom gateway. To learn more, see [China Premium Internet Access](/unified/china-premium-internet-access).
...
[]![The Access policy tab of the Create a Key Vault window. ](/downloads/tech-pubs-drafts/cloud-connector-draft-articles/deploying-cloud-connector-microsoft-azure/CreateKeyVault-AccessPolicyTab.png)
...
[]![The Networking tab in the Create a Key Vault window.](/downloads/tech-pubs-drafts/cloud-connector-draft-articles/deploying-cloud-connector-microsoft-azure/KeyVault-NetworkingTab.png)
