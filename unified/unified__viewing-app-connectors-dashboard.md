...
Source: https://help.zscaler.com/unified/viewing-app-connectors-dashboard
PDF: https://help.zscaler.com/pdf/com/en/1496241.pdf

The App Connectors dashboard (Analytics > Reports > Private Applications > App Connectors) provides information about the App Connectors for your organization.
[See image.](#connector-dashboard-tools)
Dashboard Tools
...
Due to the way data is aggregated for different time period filters, the same point in time in an [Activity Monitor widget](#activitywidgets) may show slightly different values depending on the time chosen. For example, the data with the **30 mins** time period filter at 3:00 PM may not match the data with the **14 Days** time period filter for the same date at 3:00 PM.
...
Clicking on any of the App Connectors in the widgets takes you to the [User Activity Diagnostic logs page](/unified/accessing-user-activity-diagnostics) filtered for this App Connector.
...
[]Widgets may show solid lines or dashed lines. Solid lines represent actual data for the time period. Dashed lines indicate the expected trajectory of the data, but it isn't actual data. You can select a point on the lines in a widget to see the exact date, time, and relevant numbers for the App Connectors as related to the particular widget. You can also click View Logs to see the [User Activity Diagnostic logs page](/unified/accessing-user-activity-diagnostics) filtered for the App Connectors in the widget.
...
- **Logs**: Click to view the [User Activity Diagnostic logs page](/unified/accessing-user-activity-diagnostics) filtered for this App Connector.
...
- **Peak Active Connections to Private Service Edges**: Displays up to the top 10 App Connectors with the highest active connections to Private Service Edges for Private Applications in the selected time range.
- **Peak Active Connections to Public Service Edges**: Displays up to the top 10 App Connectors with the highest active connections to Public Service Edges Private Applications in the selected time range.
...
- **Total Bytes Received from Public Service Edges**: Displays up to the top 10 App Connectors that has the highest cumulative received bytes from Public Service Edges for Private Applications in the selected time range.
- **Total Bytes Transmitted to Public Service Edges**: Displays up to the top 10 App Connectors that has the highest cumulative transmitted bytes to Public Service Edges for Private Applications in the selected time range.
...
- **Active Connections to Private Service Edges**: The number of active connections the App Connector had to Private Service Edges for Private Applications in the selected time range.
- **Active Connections to Public Service Edges**: The number of active connections the App Connector had to Public Service Edges for Private Applications in the selected time range.
...
- **Rate of Bits Received from Public Service Edge**: The number of bits received per second by the App Connector from Public Service Edges for Private Applications in the selected time range. The rate is measured by taking the total bytes (b) for two points in time counted in seconds (t): rate= (b2-b1) */(t2-t1). For example, there are 100 total bytes at 7:00, and 1000 total bytes at 7:05. The rate is 24 bits per second ((1000-100)*8 / 300 = 24 bits/second).
...
- **Rate of Bits Transmitted to Public Service Edges**: The number of bits transmitted per second by the App Connector to Public Service Edges for Private Applications during the selected time range. The rate is measured by taking the total bytes (b) for two points in time counted in seconds (t): rate= (b2-b1) */(t2-t1). For example, there are 100 total bytes at 7:00, and 1000 total bytes at 7:05. The rate is 24 bits per second ((1000-100)*8 / 300 = 24 bits/second).
...
[]![Select a point or view logs for part of a chart on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-selectptlogs.png)
[]![Select a part of a chart on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-selectbluebox.png)
[]![Zoom Out of a chart on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-zoomout.png)
[]![Search and Deselect App Connectors in Widgets on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-searchdeselect.png)
[]![Actions for the App Connector Details table on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-detailstable.png)
[]![Filter App Connectors on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-about-app-connector-dashboard-filters.png)
[]![Number of App Connectors in an App Connector Group in the App Connector Group filter on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/App%20Connector%20Dashboard.png)
...
![General information for an App Connector on the App Connector Dashboard in the Admin Portal.](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-details-generalinfo.png)
...
![Time Selection for Individual App Connector Details on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-individacrecentdata.png)
...
- **Active Connections to Private Service Edges**: The number of active connections the App Connector had to Private Service Edges for Private Applications for the point in time listed.
- **Active Connections to Public Service Edges**: The active connection the App Connector had to Public Service Edges for Private Applications for the point in time listed.
- **Available Disk Space**: The number of bytes available to the App Connector for the point in time listed.
- **Rate of Bits Received from Public Service Edge**: The number of bits received by the App Connector from Public Service Edges for Private Applications for the point in time listed.
...
- **Rate of Bits Transmitted to Public Service Edges**: The number of bits transmitted per second by the App Connector to Private Applications Public Service Edges for the point in time listed.
[]The Activity Monitor section displays the same widgets with the same functionality as seen in the [Activity Monitor section](#activitywidgets). Three widgets are selected automatically. They are:
- Active Application Tunnel Count
- Application Reachability
- Memory Utilization
[]The latency monitor widgets in the Latency tab show the latency trend from an App Connector to Public Service Edges for Private Applications in the selected time range. The time range for the Latency tab is different from the rest of the App Connector Dashboard. It is limited to data over a period between **30 Mins** to **24 Hours**, or you can select **Custom Range** to specify a custom start and end date. If you use **Custom Range**, the start date can be within the last 24 hours. This filter applies to all widgets in the Latency tab, and the default time is 30 minutes.
The widgets available are:
- App Connector To Service Edge TCP Latency
- App Connector To Service Edge Latency
Each widget shows the Public Service Edges for Private Applications that interacted with the selected App Connector during the time range. The widgets have the same functionality as seen in the [Activity Monitor section](#widgetfunctions).
For each widget, you can search by entering part or all of a Public Service Edge for Private Applications name or by using the following search query options with >, <, or = operators:
- location: The location of the Public Service Edge for Private Applications (e.g., location < San Jose).
- name: The name of the Public Service Edge for Private Applications (e.g., name = sj_serviceedge).
- value: A numerical value specific to the widget (e.g., value < 40).
![Latency Tab on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-latency.png)
[]![Details and Latency tabs for App Connectors on the App Connector Dashboard in the Admin Portal](/downloads/zpa/dashboard-diagnostics/about-app-connector-dashboard/zpa-app-connector-dashboard-detailstabs.png)
...
[]![Viewing the App Connectors dashboard tools](/downloads/unified/analytics/private-applications/app-connector-monitoring/viewing-app-connectors-dashboard/unified-app-connectors-dashboard-overview.png)