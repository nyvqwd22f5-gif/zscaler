...
Source: https://help.zscaler.com/unified/app-connector-deployment-guide-nutanix-ahv
PDF: https://help.zscaler.com/pdf/com/en/1496156.pdf
...
- [Step 2: Deploy the App Connector on Nutanix](#deploy)
- [Step 3: Configure the Networking for the Deployed App Connector](#networking)
- Step 4: Verify that the deployed App Connector is [running and healthy](/unified/managing-deployed-app-connectors#Status). Also, check that it is [meeting your sizing requirements](/unified/managing-deployed-app-connectors#VerifySizing).
...
Before you deploy a virtual appliance on Nutanix AHV, you must obtain the latest Zscaler App Connector OVA. Use the following link to download the latest VMware image from Zscaler: [https://dist.private.zscaler.com/vms/VMware/2023.12/zpa-connector-2023.12.ova](https://dist.private.zscaler.com/vms/VMware/2023.12/zpa-connector-2023.12.ova).
After you obtain the latest Zscaler App Connector OVA, upload the OVA to one of the following platforms:
- Prism Central. To learn more, see the Nutanix Prism Central Guide for [OVA Management](https://portal.nutanix.com/page/documents/details?targetId=Prism-Central-Guide-Prism-vpc_2022_1:mul-ova-manage-pc-c.html).
- Prism Element. To learn more, see the Nutanix article on [How to import an OVA image to AHV cluster](https://portal.nutanix.com/page/documents/kbs/details?targetId=kA03200000099TXCAY).
...
- Extract the VMDK file from the OVA. Right-click the OVA file and then click **7-Zip **> **Extract files...** to extract the files.
- Select the destination of the files and click **OK**.
- Upload the VMDK file to Prism Element.
- Log in to Prism Element.
- Click **Settings **and then click **Image Configuration**.
[See image.](#imageConfiguration)
- Click **Upload Image**. The **Create Image **window appears.
[See image.](#createImageWindow)
- In the **Create Image **window, enter the following:
- **Name**: The name of the VM.
- **Image Type**: Select **DISK **from the **Image Type** drop-down menu.
[See image.](#imageType)
- For **Image Source**, select **Upload a file**.
- Click **Choose File** and select the VMDK file extracted in Step 1.b.
- Click **Open**.
- Click **Save**.
- Return to the Image Configuration page **Image Configuration**) and verify that the image is listed as **ACTIVE** in the table.
[See image.](#activeImage)
- Create the VM.
- Click **VM**.
- In the **Table **tab, click **Create VM**.
[See image.](#createVM)
The **Create VM window** appears.
- Under **General Information**, enter the following information:
- **Name**: The name of the VM.
- **Timezone**: The timezone of the VM.
[See image.](#createVMGeneralInformation)
- Under **Compute Details**, enter the following information:
...
- In the **Add Disk** window, enter the following parameters, then click **Add**:
- **Type**: Disk
...
[See image.](#addNewNIC)
- In the **Create NIC window**, select **NR_PRT_DHCP** in the **Subnet Name** drop-down menu, then click **Add**.
[See image.](#createNICwindow)
- Click **Save**.
- In the **Table** tab **VM**, search for the VM by name.
[See image.](#searchForVM)
- Select the VM to view the VM details.
- Click **Power On**.
[See image.](#powerOnVm)
...
[]![Selecting Settings in the Nutanix Portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-img-configuration.png)
[]![Create Image window in the Nutanix Portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm.png)
[]![Selecting DISK in the Image Type drop-down menu.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-disk.png?1650986087)
...
[]
![Clicking Create VM in the Table tab of the Nutanix portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm-window.png)
[]
[]![Entering General Configuration in the Create VM window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm-window-general2.png)
![Entering Compute Details in the Create VM window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm-window-compute.png)
[]
[]![Deleting the CD ROM Disk Type in the Create VM window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm-window-disks.png)
[]![Adding a new disk in the Create VM window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-vm-window-add-new-disk.png)
[]![Entering parameters in the Add Disk window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-nutanix-deployment-add-disk-window2.png)
[]![Adding a new NIC.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-add-new-nic2.png)
[]![Adding a new NIC in the Create NIC window.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-create-nic-window2.png)
![Searching for the VM in the Nutanix portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-search.png)
[]
[]![Powering on the VM in the Nutanix portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-vm-power-on.png)
[]![Launching the App Connector console in the Nutanix portal.](/downloads/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms/app-connector-deployment-guide-nutanix-ahv/zpa-connector-nutanix-deployment-launch-console.png)
...
- Zscaler highly recommends [updating the App Connector system software](/unified/understanding-app-connector-software-updates) before proceeding.
[]
