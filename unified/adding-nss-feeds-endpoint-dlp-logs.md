...
Source: https://help.zscaler.com/zia/adding-nss-feeds-endpoint-dlp-logs
PDF: https://help.zscaler.com/pdf/com/en/1461931.pdf
...
A large number of filters or complex filters, such as string searches, can impact the performance of the NSS. Before you start configuring a feed for Endpoint DLP logs, consider the [guidelines for configuring feeds](/zia/general-guidelines-nss-feeds-and-feed-formats).
...
- **Feed Output Format**:** **These are the fields that display in the output. You can edit the default list and if you select **Custom **as the **Feed Output Type**, change the delimiter as well. See [NSS Feed Output Format: Endpoint DLP Logs](/zia/nss-feed-output-format-endpoint-dlp-logs) for information about the available fields and their syntax.
...
- []**File Type Category**: Use this filter to limit the logs based on the [file type categories](/zia/about-file-type-control) that were detected from the content. Multiple selections are allowed.
- **File Type**: Use this filter to limit the logs based on the [file types](/zia/about-file-type-control) that were detected from the content. Multiple selections are allowed.
...
- **Severity**: Use this filter to limit the logs based on the severity of the incidents detected by the [Endpoint DLP](/zia/about-endpoint-dlp) policy. Multiple selections are allowed.
