...
PDF: https://help.zscaler.com/pdf/com/en/1402151.pdf
...
The legal category consists of applications that provide legal services. This cloud app category includes applications such as Amicus Attorney, DirectLaw, Eclipse Legal Systems, etc. that provide legal solutions. So, you can create rules specifically for these and other similar cloud applications. You can specify which applications your users are allowed to access and define a daily quote by bandwidth or time.
...
Adding a Rule for Legal Apps
To add a rule for Legal apps:
- Go to **Policy** > **URL & Cloud App Control** > **Cloud App Control Policy**.
-
Click **Add** and select **Legal**.
The **Add Legal Rule** window appears.
- In the **Add Legal Rule** window, enter the **Cloud App Control Rule** attributes:
- **Rule Order**: Policy rules are evaluated in ascending numerical order (Rule 1 before Rule 2, and so on), and the rule order reflects this rule’s place in the order. You can change the value, but your assigned admin rank determines the rule order values you can select if you’ve enabled [Admin Rank](/zia/what-admin-rank).
...
[]![ZIA Add Cloud Application Rule with Click to See More in the Cloud Applications Field](/downloads/zia/documentation-knowledgebase/policies/cloud-apps/cloud-app-control-policies/adding-legal-rule-cloud-app-control/ZIA-Add-Cloud-App-Control-Rule-Legal-Click-See-More.png)
