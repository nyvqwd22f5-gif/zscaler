...
Source: https://help.zscaler.com/unified/deploying-zscaler-client-connector-microsoft-intune-ios
PDF: https://help.zscaler.com/pdf/com/en/1492286.pdf
...
- Click **Next**.
-
In the **Configuration settings** section, for **Connection type**, select **Zscaler**.
...
- An administrator removes the device from the Admin Portal.
...
- **Organization’s cloud name**: The name of the cloud where your organization is provisioned. For example, if your cloud name is zscalertwo.net, enter `zscalertwo`.
- **Excluded URLs**: Enter any URLs you want to bypass. For example, enter your identity provider (IdP), MDM (Mobile Device Management) server, or anything else the user should have access to before enrollment. If you use Private Applications, you must enter `authsp.prod.zpath.net`. For a list of additional Intune requirements, refer to the [Microsoft documentation](https://learn.microsoft.com/en-us/mem/intune/fundamentals/intune-endpoints?tabs=north-america).
[See image.](#config-settings-vpn)
-
(Optional) Click **Add** and enter one or more configuration keys and their corresponding configuration values:
- [deviceToken](#deviceToken)
- [policyToken](#policytoken1)
- [username](#username)
- [authByTunnel](#authByTunnel)
- [enableFips](#enableFips)
- [newBindFlow](#newBindFlow)
- [strictEnforcement](#strictenforcement1)
- [DropNonRoutingTraffic](#DropNonRoutingTraffic)
- [SkipInterfaceInstallation](#SkipInterfaceInstallation)
...
[]The appropriate device token from the Admin Portal if you want to use [Zscaler Client Connector as an IdP](/unified/using-zscaler-client-connector-identity-provider).
...
[]When enabled, Private Applications match domains are not appended to search domains. Enter `1` to enable or `0` to disable this option. Default is `0`.
[]To add the policy:
...
-
Enter the configuration keys and their corresponding configuration values. Set the value type as **string **for all, and then click **Next**.
- [deviceToken](#deviceToken2)
- [username](#username2)
- [policyToken](#policyToken2)
- [authByTunnel](#authByTunnel2)
- [enableFips](#enableFips2)
- [newBindFlow](#newBindFlow2)
- [DropNonRoutingTraffic](#DropNonRoutingTraffic2)
...
[]The appropriate device token from the Admin Portal if you want to use [Zscaler Client Connector as an IdP](/unified/using-zscaler-client-connector-identity-provider).
[]The username of the user. For example, if the username is j.doe@zscaler.com, enter` j.doe`.
To use the same username used for enrolling into Intune, you can use the `{{partialupn}}` token. To view a complete list of available Intune tokens, refer to the [Microsoft documentation](https://learn.microsoft.com/en-us/mem/intune/apps/app-configuration-policies-use-ios).
[]Allows you to specify which app profile policy you want to enforce for the app before the user enrolls. All relevant settings associated with the policy apply, including the bypass of the IdP login page. After the user enrolls, this policy is replaced with the app profile policy that matches the user based on group affiliation.
policyToken is only applicable, and required, if you enable the Strict Enforcement option and want users to enroll with the app before accessing the internet.
[]The auto-enrollment settings for users when Zscaler Client Connector is used as an identity provider (IdP) for authentication. To have users always auto-enroll even if they are logged out manually or forcefully removed from the portal, set this configuration key to `1`. For one-time auto-enrollment, set it to `2`. To disable auto-enrollment, set it to `0`.
[]If you enable this option, Zscaler Client Connector uses FIPS-compliant libraries for communication with Zscaler infrastructure. Enter `1` to enable or `0` to disable this option.
...
[]The name of the cloud where your organization is provisioned. For example, if your cloud name is zscalertwo.net, enter `zscalertwo`.
[]If you use the device posture type **Ownership Variable**, add the key `ownership`. You can enter up to 32 alphanumeric characters in the Configuration value field. To learn more, see [Configuring Device Posture Profiles](/unified/configuring-device-posture-profiles).
[]Enable this option if you want to block internet traffic before the user enrolls in Zscaler Client Connector when:
- The user has not logged in after a new install.
- The user logs in and then logs out.
- An administrator removes the device from the Admin Portal.
If you enable Strict Enforcement, you must use the policyToken configuration key. Excluded URLs must also be defined in a device configuration policy or excludeList in an app configuration policy. There are also additional configuration settings required. To learn more, see [Configure Additional Strict Enforcement Settings](#configure-strict-enforcement-settings).
[]Enter any URLs you want to bypass. Enter your IdP, MDM server, or anything else the user should have access to before enrollment. If you use Private Applications, you must enter `authsp.prod.zpath.net`. For a list of additional Intune requirements, refer to the [Microsoft documentation](https://learn.microsoft.com/en-us/mem/intune/fundamentals/intune-endpoints?tabs=north-america).
[]If you use Internet & SaaS and you want to perform SSL inspection, you must configure a certificate profile to push the Certificate Authority (CA) certificate required for SSL inspection. You can use the default Zscaler CA certificate or a custom Root CA certificate.
To learn more, see [Choosing the CA Certificate for SSL Inspection](/unified/choosing-ca-certificate-ssl-inspection) and [Certificate Pinning and SSL Inspection](/unified/certificate-pinning-and-ssl-inspection).
...
- In the **Assignments** section, choose the users, groups, and devices for the profile, and click **Next**.
- In the **Review + create** section, review the summary, and click** Create**.
[]You can add a specific application to Microsoft Intune and associate the application with a per-app VPN configuration profile to ensure that selected applications are protected by Internet & SaaS and can access other applications. For example, you can allow access to private applications only from a specific browser.
...
- The user logs in and then logs out.
- An administrator removes the device from the Admin Portal.
If you enable Strict Enforcement, you must include policyToken in a device configuration policy and use **Excluded URLs** in a device configuration policy or an excludeList in an app configuration policy. There are also additional configuration settings required. To learn more, see [Configure Additional Strict Enforcement Settings](#configure-strict-enforcement-settings).
- **Organization’s cloud name**: The name of the cloud where your organization is provisioned. For example, if your cloud name is zscalertwo.net, enter `zscalertwo`.
- **Excluded URLs**: Enter any URLs you want to bypass. Enter your IdP, MDM server, or anything else the user should have access to before enrollment. If you use Private Applications, you must enter `authsp.prod.zpath.net`. For a list of additional Intune requirements, refer to the [Microsoft documentation](https://learn.microsoft.com/en-us/mem/intune/fundamentals/intune-endpoints?tabs=north-america).
[See image.](#config-settings-vpn2)
-
(Optional) Click **Add** and enter one or more configuration keys and their corresponding configuration values. Set the value type as **string** for all the configuration keys:
- [deviceToken](#deviceTokenPerappvpn)
- [username](#usernamePerappvpn)
- [authByTunnel](#authByTunnelPerappvpn)
- [enableFips](#enableFipsPerappvpn)
- [newBindFlow](#newBindFlowPerappvpn)
- [PAVConnectionSynced](#PAVConnectionSyncedPerappvpn)
- [DropNonRoutingTraffic](#DropNonRoutingTrafficPerappvpn)
- [SkipInterfaceInstallation](#SkipInterfaceInstallationPerappvpn)
...
- Hosts required for Private Applications:
...
[See image.](#strict-enforcement-excludedurls)
-
...
[]![Enter the excluded URLs](/downloads/client-connector/downloading-deployment/deploying-zscaler-client-connector-microsoft-intune-ios/ios-deployment-intune-strict-enforcement-excludedurls.png)
