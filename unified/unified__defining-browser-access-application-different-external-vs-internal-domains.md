...
Source: https://help.zscaler.com/unified/defining-browser-access-application-different-external-vs-internal-domains
PDF: https://help.zscaler.com/pdf/com/en/1492361.pdf
...
- CN=[external FQDN]
- SAN=[internal FQDN]
To learn more, see [About (Web Server) Certificates](/unified/about-web-server-certificates) and [Using Wildcard Certificates for Browser Access Applications](/unified/using-wildcard-certificates-browser-access-applications).
- If your web server is multihomed and is expecting a Host header matching the internal FQDN, the external FQDN is not transformed. So, if your web server is configured for Host headers, you must add the external FQDN as an additional header.
...
- [Learn more about configuring a Browser Access application using Method 1 above](#Method1)
...
- [Learn more about configuring a Browser Access application using Method 2 above](#Method2)
[]To configure Private Applications using Method 1, you must disable [dynamic server discovery](/unified/enabling-dynamic-server-discovery) and then manually define your servers:
- Go to **Policies **>** Access Control **> **Private Applications **> **Servers**.
- Click **Add Server**.
...
If you have multiple web servers supporting the same application (e.g., webapp1, webapp2, etc.), then you must add a server in the Admin Portal for each internal FQDN.
- Click **Save**.
- Go to **Policies **>** Access Control **> **Private Applications **> **Server Groups**.
...
- Click **Save**.
- Go to **Policies **> **Private Applications **> **App Segments**.
...
- Select the proper SSL certificate from the drop-down menu. The certificate must match the external FQDN (i.e., webapp.external.example.com or *.external.example.com).To learn more, see [About (Web Server) Certificates](/unified/about-web-server-certificates) and [Using Wildcard Certificates for Browser Access Applications](/unified/using-wildcard-certificates-browser-access-applications).
...
- On the [**3 Server Groups**](/unified/configuring-defined-application-segments#tab3) tab, for **Select Server Group**, choose the server group your created in step 7 above from the drop-down menu, and click **Next**.
...
- [Publish the CNAMEs for the FQDNs you defined in step 10a above.](/unified/configuring-defined-application-segments#Publish_CNAME)
Your users should now be able to access the [applications you defined](/unified/configuring-defined-application-segments#tab1) within the application segment, based upon the [policies](/unified/configuring-defined-application-segments#tab6) applied. So make sure that you add or update your access policies accordingly for your users.
[]To configure Private Applications using Method 2, you must enable [dynamic server discovery](/unified/enabling-dynamic-server-discovery) and create an internal A record for your external FQDN on your DNS:
...
- In the Admin Portal, go to **Policies **> **Access Control **> **Private Applications **> **Server Groups**.
- Click **Add Server Group**.
- In the **Add Server Group** window:
- **Status**: Select **Enabled**.
...
- Complete the configuration on the [**1 Define Applications**](/unified/configuring-defined-application-segments#tab1) tab, then click **Next**.
- Complete the configuration on the [**2 Segment Group**](/unified/configuring-defined-application-segments#tab2) tab, then click **Next**.
- On the [**3 Server Groups**](/unified/configuring-defined-application-segments#tab3) tab, for **Select Server Group**, choose the server group your created in step 4 above from the drop-down menu, and click **Next**.
- Review your application segment configuration settings on the [**5 Review**](/unified/configuring-defined-application-segments#tab5) tab and review and edit any policies, as needed, on the [**6 Policies**](/unified/configuring-defined-application-segments#tab6) tab.
- [Publish the CNAMEs for the FQDNs you defined in step 7a above.](/unified/configuring-defined-application-segments#Publish_CNAME)
Your users should now be able to access the [applications you defined](/unified/configuring-defined-application-segments#tab1) within the application segment, based upon the [policies](/unified/configuring-defined-application-segments#tab6) applied. So make sure that you add or updated your access policies accordingly for your users.