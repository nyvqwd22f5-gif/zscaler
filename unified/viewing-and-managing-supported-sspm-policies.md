...
- If enabled, the Zscaler SSPM evaluates the policy for the selected SaaS application and the tenant, and displays the policy’s current status (Pass, Fail, Partial) in the [SaaS Security Reports](/zia/about-saas-security-posture-report) page.
- If disabled, the Zscaler SSPM does not evaluate the policy for the selected SaaS application and the tenant, and displays the Disabled status for the policy in the [SaaS Security Reports](/zia/about-saas-security-posture-report) page.
[See image.](#sspm-policy-status)
To learn more about policy statuses, see [SaaS Security Posture Report](/zia/saas-security-posture-report).
[See image.](#sspm-policy-table)
- Click **Save** after disabling or re-enabling the policy status and [activate the change](/zia/saving-and-activating-changes-admin-portal).
[]![The SaaS Security Posture Report](/downloads/zia/policies/saas-security-api/saas-security-api-policies/saas-security-posture-control/configuring-saas-security-posture-control-policy/ZIA-SaaS-Security-Posture-Control-Policy.png)
