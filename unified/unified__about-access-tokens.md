...
On the ZIdentity Issued Tokens page (Administration > API Configuration > OneAPI > Tokens), you can do the following:
...
If the token is revoked but the API client's status is enabled in the Admin Portal, you can generate a new token and access the API resources.
- Reset the filters.
- Search for a specific API client.
- [Revoke the access token](/unified/revoking-access-tokens).
