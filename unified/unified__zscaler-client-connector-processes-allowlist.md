...
Source: https://help.zscaler.com/unified/zscaler-client-connector-processes-allowlist
PDF: https://help.zscaler.com/pdf/com/en/1495636.pdf
...
If your organization uses [Group Policy Object (GPO)](/unified/deploying-zscaler-client-connector-active-directory-windows) to push policies on both Windows 64-bit and Windows 32-bit systems, you must add the file paths of both versions to your GPO policy.
...
Zscaler Client Connector also uses carrier-grade NAT range 100.64.0.0/16 as part of internal health checking and for the Private Applications service.
...
If you are receiving a local FW/AV error after entering the rules, examine the `%windir%\system32\logfiles\firewall\pfirewall.log` to verify that there are no "drop" entries for 100.64 addresses. If you see "drop" entries, the Windows Firewall rules are not applied correctly to allow Zscaler to communicate. The rules must be revised based on the processes listed earlier.
Processes Usage
...
- **ZSAUpm**: The Digital Experience Monitoring service.
...
- **LogonUI**: Used for pre-Windows login in Private Applications.
