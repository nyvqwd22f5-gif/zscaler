...
PDF: https://help.zscaler.com/pdf/com/en/1529067.pdf

VPN (for Legacy Apps) enables support for legacy applications that require server-to-client traffic using the real IP address binding (e.g., VoIP or Active FTP). This service uses [VPN Service Edges](/zpa/about-vpn-service-edges), Network Connectors, [Network segments](/zpa/about-network-segments), and [Zscaler Client Connector](/client-connector/using-zscaler-client-connector). The Network Connector is deployed within your application server's data center, and connects to all known VPN Service Edges. Organizations must configure the route to the client IP pool using the Network Connector’s IP address as the next hop.
...
ZPA supports the following platforms for VPN (for Legacy Apps): Windows and macOS. Network Connectors can only be deployed with RHEL9 in the RPM format for Linux.
About the Network Connectors Page
On the Network Connectors page (VPN (for Legacy Apps) > Network Connectors > Network Connectors), you can do the following:
...
- [Add a new Network Connector.](/zpa/configuring-network-connectors)
...
- **Public Service Edge**: The ZPA Public Service Edge that the Network Connector connects to.
...
- [Edit the configuration of a deployed Network Connector.](/zpa/editing-deployed-network-connector)
...
- Go to the [Network Connector Groups](/zpa/about-network-connector-groups) page to manage your Network Connector groups.
- Go to the [Network Connector Provisioning Keys](/zpa/about-network-connector-provisioning-keys) page to manage your Network Connector provisioning keys.
![Network Connector page in the ZPA Admin Portal](/downloads/zpa/about-network-connectors/PII%20Network%20Connectors%20page%20w%20steps.png)