...
Source: https://help.zscaler.com/unified/deploying-zscaler-cloud-connector-google-cloud-platform
PDF: https://help.zscaler.com/pdf/com/en/1520066.pdf
...
This procedure describes the steps for deploying Zscaler Cloud Connector using Terraform. To learn more about the resources created when deploying Zscaler Cloud Connector, see [Deployment Templates for Zscaler Cloud Connector](/unified/deployment-templates-zscaler-cloud-connector).
[]Prerequisites
...
[]Zscaler recommends that you create a dedicated admin role for Cloud Connector deployment. To configure a new [admin role](/unified/adding-admin-roles-0):
...
[]After you configure the admin role, create a new[ role-based administrator](/unified/about-administrators-0) dedicated to Cloud Connector deployment. See [Adding Users](/zidentity/adding-users).
Select the admin role you created in the previous step and set the status to **Enabled**. See [Managing Entitlements](/zidentity/managing-entitlements).
[]To retrieve the API key:
- Log in to the Admin Portal.
- Go to **Administration **> **API Configuration **> **Legacy API** >** Cloud & Branch Connector API.**
- From the[ API Keys](/unified/about-api-configuration) page, copy the API key and store it in a secure location. Cloud Connector uses the API key to authenticate and register the GCP with Zscaler.
[See image.](#API-Key-Management)
[]You do not need to configure location templates because [locations](/unified/about-locations-1) are created automatically when you deploy Cloud Connector to a cloud provider such as GCP. Optionally, you can configure your own location template:
...
- Click [**Add Location Template**](/unified/configuring-location-template-0).
...
- Click **Save** and [activate the change](/unified/saving-and-activating-changes-admin-portal).
[]Configure a [cloud provisioning template](/unified/configuring-cloud-provisioning-template) and copy the cloud provisioning URL. To configure a cloud provisioning template:
- Log in to the Admin Portal.
...
- Click **Save**.
- From the [Cloud Provisioning Template](/unified/about-cloud-provisioning-templates) page, click the arrow in the **Template Name** column and copy the cloud provisioning URL. Store the cloud provisioning URL in a secure location.
...
-
**api_key**: Enter api_key for the name of your secret key. For the corresponding** **value**,** enter the value you copied from the [API Keys](/unified/about-api-configuration) page on the Admin Portal.
`	{
"username": "cc-deploy-user@company.com",
"password": "cc-deploy-user-password",
"api-key": "ZscalerCompanyAPIKey"
}`
...
[]For information about creating and storing your secrets in Hashicorp Vault, see [Storing Your Secret Credentials in Hashicorp Vault for Google Cloud Platform-Based Cloud Connectors.](/unified/storing-your-secret-credentials-hashicorp-vault-google-cloud-platform-based-cloud-connectors)
