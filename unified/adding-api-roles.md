...
PDF: https://help.zscaler.com/pdf/com/en/1411441.pdf

[Watch a video about Adding API Roles.](https://fast.wistia.net/embed/iframe/leihuwkgjo)
An API role defines a client application's permission and access to the different API categories in the Zscaler cloud service API. An API role is not assigned to an admin. Instead, it is used during the configuration of an external OAuth 2.0 authentication server for API authentication. You can add up to 16 API roles. For a complete list of ranges and limits per API category, see [Ranges & Limitations](/zia/ranges-limitations).
Configuring an API role is one of the tasks you must complete when configuring an external OAuth 2.0 authentication server for API authentication. To learn more, see [Securing ZIA APIs with OAuth 2.0](/zia/securing-zia-apis-oauth-2.0).
You can [edit or delete](/zia/how-do-i-edit-delete-or-duplicate-items-admin-portal) API roles at any time.
Prerequisites
To configure API roles, the admin must be assigned to a role that has full permission to Administration > API Key management in the ZIA Admin Portal. To learn more, see [Adding Admin Roles](/zia/adding-admin-roles).
...
- Traffic Capture
To learn more about the endpoints, see the [Forwarding Control API Reference Guide](/zia/forwarding-control-policy), [Traffic Forwarding API Reference Guide](/zia/traffic-forwarding-0), and [Location Management API Reference Guide](/zia/location-management#/locations-get).
...
- Role Management
To learn more about the endpoints, see the [Advanced Settings API Reference Guide](/zia/user-authentication-settings), [Admin Audit Logs API Reference Guide](/zia/admin-audit-logs), and [Admin and Role Management API Reference Guide](/zia/admin-role-management).
- Backup Controls
- Backup
- Restore
...
[]You can assign **Full**, **View Only**,** None**,** **or** Custom** permissions to the client application to access the URL categories API category and its subcategories within the Zscaler cloud service API. If you select **Full**, **View Only**, or** None **permissions, the selected permission applies to all the categories. To assign custom permissions, select **Custom **and assign appropriate permissions for the following categories:
...
To learn more about the endpoints, see the [URL Categories API Reference Guide](/zia/url-categories).
...
To learn more about the endpoints, see the [Data Loss Prevention (DLP) API Reference Guide](/zia/data-loss-prevention#/dlpDictionaries-get).
...
- Root Certificates
To learn more about the endpoints, see the [Intermediate CA Certificates API Reference Guide](/zia/intermediate-ca-certificates).
