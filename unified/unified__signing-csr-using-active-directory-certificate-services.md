...
Source: https://help.zscaler.com/unified/signing-csr-using-active-directory-certificate-services
PDF: https://help.zscaler.com/pdf/com/en/1493781.pdf

When you configure a [custom intermediate root certificate](/unified/choosing-ca-certificate-ssl-inspection) for [SSL inspection](/unified/about-ssl-inspection), you must generate and download a certificate signing request (CSR) in the Admin Portal, then send the CSR to a certificate authority (CA) for signing. Ensure that the CSR is signed as a subordinate CA or intermediate CA.
Below is a configuration example showing how the CSR can be signed using the Active Directory Certificate Services using Certreq.exe. To learn more about Certreq, refer to the [Microsoft documentation](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/dn296456(v%3dws.11)).
- Generate a certificate signing request. To learn more, see [Choosing the CA Certificate for SSL Inspection](/unified/choosing-ca-certificate-ssl-inspection). For this example, the downloaded file will be named `zscalerdemo.csr`.
