...
Source: https://help.zscaler.com/unified/internet-saas-virtual-service-edge-configuration-guide-dual-arm-mode
PDF: https://help.zscaler.com/pdf/com/en/1495731.pdf

Use a dual arm configuration for your Internet & SaaS Virtual Service Edges if you have an environment that requires your Virtual Service Edges to use two zones: a disconnected network or isolate the internet zone from the internal users and another that faces the internet.
...
This configuration does not apply to the dual arm configuration in standalone mode with external or no LB.
...
- [2. Download the Virtual Service Edge VM](/unified/downloading-virtual-service-edge-vm)
...
To learn more about the other fields, see [Configuring Firewall Filtering Policy](/unified/configuring-firewall-filtering-policy).
...
sudo vzen configure-network
- **em1**: Internal-facing Proxy interface. The interface that receives user traffic from the internal side. This is configured in the Admin Portal.
...
- (Optional) Add the following line to the end of the file:
ifconfig_em2="<IP Address>"
Replace <IP Address> with the IP address in your subnet which is internet-facing.
- (Optional) The default gateway is automatically added via the em0 interface. To add a static route to a different subnet or VLAN, add the following to the end of the file:
static_routes="em2_internal"
route_em2_internal="-net <Destination Subnet> <Gateway IP Address>"
Replace <Destination Subnet> with the IP subnet of the internet-facing network and replace <Gateway IP Address> with your desired gateway IP address (ideally the inside router).
For example, to add a static route to a different subnet or VLAN:
static_routes="em2_internal"
route_em2_internal="-net 192.168.0.0/16 192.168.2.1"
- Enter the following command:
cd /sc/sme/conf
...
To learn more, see [About Virtual Service Edge Clusters](/unified/about-virtual-service-edge-clusters) and [Configuring Virtual Service Edge Clusters](/unified/configuring-virtual-service-edge-clusters).
...
Each of your Virtual Service Edges need two additional IPs for this dual arm configuration. The cluster IP is the proxy gateway that users point to. It’s also the VIP for the Virtual Service Edges. To learn more, see [Locating the Virtual IP Addresses for Internet & SaaS Public Service Edges](/unified/locating-virtual-ip-addresses-internet-saas-public-service-edges).
By default, your Virtual Service Edge has the following interfaces:
- **em0**: Internet-facing Management interface. This management communication is used for downloading software updates, SSH, SNMP, NTP, etc. This is configured on the CLI using the following command:
sudo vzen configure-network
- **em1**: Internal-facing Proxy interface. The interface that receives internal user/remote user traffic from the internal side. This is configured in the Admin Portal.
- **em2**: Internal-facing The LB service interface. The interface in the user network that receives user traffic from the internal-facing cluster IP. This is configured in the Admin Portal.
In the dual arm configuration, you must configure two extra interfaces for each of your Virtual Service Edges:
...
For example, to add a static route to a different subnet or VLAN:
static_routes="em3_internal"
route_em3_internal="-net 192.168.0.0/16 192.168.2.1"
- Enter the following command:
cd /sc/sme/conf
- Create a new file called vzen_custom.conf.
- Add the following lines:
...
To learn more, see [About Internet & SaaS Virtual Service Edge Clusters](/unified/about-internet-saas-virtual-service-edge-clusters) and [Configuring Internet & SaaS Virtual Service Edge Clusters](/unified/configuring-internet-saas-virtual-service-edge-clusters).
...
sudo vzen configure-network
- **em1**: Internal-facing Proxy interface. The interface that receives user traffic from the internal side. This is configured in the Admin Portal.
- **em2**: Internal-facing LB service interface. The interface in the user network that receives user traffic from the internal-facing cluster IP. This is configured in the Admin Portal.
...
For example, to add a static route to a different subnet or VLAN:
static_routes="em4_internal"
route_em4_internal="-net 192.168.0.0/16 192.168.2.1"
- Enter the following command:
cd /sc/smlb/conf
