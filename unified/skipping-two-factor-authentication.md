...
Source: https://help.zscaler.com/zidentity/skipping-two-factor-authentication
PDF: https://help.zscaler.com/pdf/com/en/1517701.pdf

ZIdentity allows administrators to permit users to skip two-factor authentication (2FA) for a short duration and use only their password to log in to the subscribed Zscaler service admin portal. For example, if a user does not have the device on which they receive the one-time passcode (OTP) for authentication, they can be allowed to skip 2FA temporarily.
