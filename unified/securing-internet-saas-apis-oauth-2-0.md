...
Source: https://help.zscaler.com/unified/securing-internet-saas-apis-oauth-2-0
PDF: https://help.zscaler.com/pdf/com/en/1494871.pdf

The Zscaler service supports OAuth 2.0 authentication to securely access the Internet & SaaS API. OAuth 2.0 authentication allows third-party applications to obtain controlled access to protected resources using access tokens. The Zscaler service uses the Client Credentials OAuth flow, in which the clients exchange their credentials for an access token and gain access to the Internet & SaaS API, outside the context of users. The Zscaler service supports OAuth 2.0 implementations with PingFederate, Okta, and Microsoft Entra ID (formerly Azure Active Directory).
...
- **Limits Exposure of Credentials:** Unlike the authentication model that uses API keys and admin credentials and may involve user management outside the organization's identity provider, OAuth 2.0 does not require admin credentials for authentication.
- **Granular Access Control:** The Client Credentials OAuth flow employs [API Roles](/unified/adding-api-roles) to define permissions required to access specific categories of Internet & SaaS API. Unlike admin roles, API roles are not assigned to Internet & SaaS admin users. Instead, API roles are associated with the client applications that are accessing the API. OAuth 2.0 provides added security to API access by isolating API permissions from admin users with access to the Admin Portal.
...
**Client:** The client application that requests access to protected resources residing in the resource server (i.e., Internet & SaaS API). The client authenticates with the authorization server to obtain an access token and then uses the access token to access the Internet & SaaS API.
**Access Tokens:** An access token is a string issued to a client application by the authorization server. The client application uses the access token to identify itself and gain authorized access to protected resources residing in the resource server (i.e., Internet & SaaS API).
...
**Scope:** Scope defines the specific permissions required by a client application to access the data in the resource server (i.e., Internet & SaaS API). In Zscaler’s OAuth 2.0 authentication model, scope is defined using [API Roles](/unified/adding-api-roles) and must be associated with the client application in the OAuth 2.0 service console.
[]OAuth 2.0 Flow
The Zscaler service uses the Client Credentials OAuth flow. In this model, client applications make API calls to the Internet & SaaS API using an access token obtained from the authorization server in exchange for their credentials. Therefore, the clients access the Internet & SaaS API resources on their behalf without requiring any user interaction.
...
The client sends an API request to the resource server (i.e., Internet & SaaS API) with the signed access token (JSON Web Token) in the request authorization header.
-
**The resource server grants access to protected resources**
The following series of events take place before the resource server (i.e., Internet & SaaS API) can accept the API request:
...
You need to set up the following configurations sequentially before initiating Internet & SaaS API authentication using OAuth 2.0:
- [Configure API Roles in the Admin Portal](#configure-api-roles)
- [Register applications on the external OAuth provider](#register-applications-on-auth-server)
- [Add OAuth 2.0 Authorization Servers to the Admin Portal](#add-auth-server)
API operations that are authenticated using OAuth 2.0 are associated with an auto-generated Admin ID and recorded in [Audit Logs](/unified/about-audit-logs). An Admin ID is generated for each API role in the following format: `oauth-``<rolename>``$@``<orgid>``.``<cloud_domain>`. To learn more, see [Adding API Roles](/unified/adding-api-roles).
[]With the OAuth 2.0 authentication model, the Zscaler service provides role-based, controlled access to its Internet & SaaS API resources. API roles define the scope of access for client applications to specific categories of Internet & SaaS API endpoints.
...
You can add and manage API roles on the Role Management page. To learn more, see [Adding API Roles](/unified/adding-api-roles).
...
[]The Zscaler service requires you to add your authorization servers to the Admin Portal to verify the authenticity of the access token in API requests and optionally validate against various supported claims.
You can add and manage your authorization servers from the OAuth 2.0 Authorization Servers page. To learn more, see [About OAuth 2.0 Authorization Servers](/unified/about-oauth-2-0-authorization-servers).