...
Source: https://help.zscaler.com/unified/configuring-locations
PDF: https://help.zscaler.com/pdf/com/en/1488306.pdf

This article describes how to add a single location. You can add up to 32K locations. For a complete list of ranges and limits per feature, see [Ranges & Limitations](/unified/ranges-limitations). You can also use a CSV file to import multiple locations and sub-locations. To learn more, see [Configuring Multiple Locations and Sub-Locations](/unified/configuring-multiple-locations-and-sub-locations).
To add a location:
...
- **Name**: Enter a name for the location. To learn more about naming a location, see [Naming Locations & Sub-Locations](/unified/about-locations#naming-locations-sub-locations).
...
When you specify the location in a policy, the service applies the policy according to the location's time zone. For example, if a [Cloud App Control policy](/unified/about-cloud-app-control) blocks posting to Facebook between 8:00 AM and 5:00 PM, and the rule is applied to locations in Spain and California, users at each location are blocked during their respective daytime hours.
-
...
- **Manual Location Groups**: Search for and select the [manual location group](/unified/configuring-manual-location-groups) names. The location is added to the manual groups you selected. Otherwise, leave the selection as **None**.
-
**Dynamic Location Groups**: This field shows the [dynamic location groups](/unified/configuring-dynamic-location-groups) that the location belongs to.
...
-
**Extranet**
Selecting **Extranet** as the location type changes the configuration steps. To learn more, see [Configuring an Extranet](/unified/configuring-extranet).
- **Guest Wi-Fi traffic**
- **IoT traffic**
- **Server traffic**
...
- **Managed By**: If this location is being managed by an [SD-WAN partner](/unified/about-partner-integrations), search for and select their name from the drop-down menu. If this location is not being managed by a partner, select **Self**.
...
The static IP addresses that appear in the drop-down menu are the addresses that are provisioned for your organization. To learn more, see [Self-Provisioning of Static IP Addresses](/unified/self-provisioning-static-ip-addresses). If you want Zscaler to provision your static IP addresses, submit them to Zscaler Support so that they can be properly added to the menu.
- **Proxy Ports**: Search for and choose your organization's [subscribed ports](/unified/configuring-dedicated-proxy-ports) for the location.
- []**VPN Credentials**: If you are configuring an [IPSec VPN tunnel](/unified/configuring-ipsec-vpn-tunnel) to forward traffic to the Zscaler service, search for and choose IP addresses or FQDNs for the location.
...
- **Virtual Service Edges**: Search for and select your organization's [Virtual Service Edges](/unified/about-virtual-service-edges) for the location.
- **Virtual Service Edge Clusters**: Search for and select your organization's [Virtual Service Edge clusters](/unified/about-virtual-service-edge-clusters) for the location.
...
- Forward [proxy chaining](/unified/configuring-proxy-chaining) with the **Enable XFF Forwarding** option turned on for the location
-
[]**Enable AUP**: If you disabled **Enforce Authentication**, you can enable this feature to display an Acceptable Use Policy (AUP) for unauthenticated traffic and require users to accept it. To learn more, see [Configuring End User Notifications](/unified/configuring-end-user-notifications#AUP-notification). If you enable this feature:
...
- If **Force SSL Inspection **is enabled, the [SSL Inspection](/unified/about-ssl-inspection) enforces an AUP for HTTPS traffic.
...
-
[]**Enable IP Surrogate**: If you enabled **Enforce Authentication**, select this option if you want to map users to device IP addresses. Any sub-locations associated with this location do not inherit this setting. To learn more, see [About Surrogate IP](/unified/about-surrogate-ip).
...
- **DNS64 Prefix**: (Optional) If you enabled IPv6, select a DNS64 prefix for the IPv6 addresses. To learn more, see [About DNS64 Prefix](/unified/about-dns64-prefix).
...
-
[]In the **Bandwidth Control** section, you can enforce [bandwidth controls](/unified/about-bandwidth-control) for the location. If enabled, specify the maximum bandwidth limits for **Download (Mbps)** and **Upload (Mbps)**. All [sub-locations](/unified/understanding-sub-locations) share the bandwidth limits assigned to this location. However, you can [override this behavior](/unified/configuring-sub-locations#subloc-bwcontrols) to assign a fixed bandwidth to a sub-location.
...
[]![Providing a name, location group, time zone, and other settings for a location within the Admin Portal for ZIA](/downloads/zia/documentation-knowledgebase/traffic-forwarding/locations/configuring-locations/ZIA-Add-Location-Location-Section.png)
