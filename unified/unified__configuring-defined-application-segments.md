...
Source: https://help.zscaler.com/unified/configuring-defined-application-segments
PDF: https://help.zscaler.com/pdf/com/en/1492086.pdf

When defining a new application within an application segment, you can:
- [Define applications individually](/unified/about-application-access#AboutApplicationDefinitions) or [enable application discovery](/unified/about-application-discovery). This includes defining applications for [Browser Access](/unified/about-browser-access) and for use as part of [Source IP Anchoring](/unified/understanding-source-ip-anchoring-direct).
- Specify the [server groups](/unified/about-server-groups) hosting the applications.
- Specify the [App Connector groups](/unified/about-app-connector-groups) that have access to those server groups.
For a complete list of ranges and limits per feature, see [Ranges & Limitations](/unified/ranges-limitations).
...
- []If you [defined a Browser Access-enabled application](/unified/configuring-defined-application-segments) within this application segment, complete the following steps after saving the configuration:
- Go to the [**Browser Access**](/unified/about-browser-access) page (Policies > Access Control > Clientless > Browser Access).
...
Your users should now be able to access the [applications you defined](/unified/about-defined-application-segments) within the application segment, based upon the policies applied.
...
This setting is tied to Source IP Anchoring. When enabled, you can connect to it. You can't delete the application while in use. There are ranges and limitations for Source IP Anchoring. To learn more, see [About Source IP Anchoring](/unified/understanding-source-ip-anchoring-direct) and [Ranges & Limitations](/unified/ranges-limitations).
- **Description**: (Optional) Enter a description.
- **Inspect Traffic**: Enable to leverage single posture for securing internet/SaaS and private applications and apply Data Loss Prevention policies to the application segment you are creating. If **Source IP Anchor** is **Enabled**, you must disable it prior to enabling **Inspect Traffic** as both features cannot be enabled at the same time.
[See image.](#img-define-gnrlinfo)
- []**Active Directory Inspection**: Enable to allow this application segment's traffic to be inspected by Active Directory Inspection.  The Active Directory Inspection data can be found on the AD Protection Diagnostics page. Active Directory Inspection is disabled by default.
- **Auto AppProtection**: Enable to allow this application segment's traffic to be inspected by AppProtection. If AppProtection is enabled, then you need to [generate an Enrollment (CA) Certificate](/unified/generating-zscaler-issued-enrollment-ca-certificates). AppProtection is disabled by default.
If **Active Directory Inspection** is enabled, Auto AppProtection and API Protection cannot be enabled. If **AppProtection** is enabled, Active Directory Inspection cannot be enabled.
...
- For applications that users access using host names (e.g., DFS), ensure that you [configure DNS search suffixes](/unified/adding-dns-search-domains) so that the search domain is automatically added to the host name.
When defining applications for Browser Access, you can only configure FQDNs.
To learn more, see [Defining an Application](/unified/about-application-access) and [Configuring Application Discovery](/unified/about-application-discovery).
[See image.](#img-define-apps)
- []Select the checkbox to enable **Browser Access** for an application, and then [complete the following steps.](#BAsteps)
- []Select the checkbox to enable **Inspect AppProtection Application** for an application, select the checkbox, then [complete the following steps.](#protectsteps)
...
- (Optional) Enter a domain in the **Domain for Browser Isolation** field that can be used in place of the application domain for browser isolation. To learn more, see [What Is Isolation?](/zia/about-cloud-browser-isolation)
...
-
From the drop-down menu, select the protocol (**Dynamic**, **HTTP**, or **HTTPS**). The **Dynamic** protocol option allows the certificate to be [generated by the AppProtection Certificate](/unified/generating-zscaler-issued-enrollment-ca-certificates). This option is handled by the back end that the application domain is mapped to. If you select this option, you can view the Protocol Discovery details on the [Protocol Discovery Dashboard page](/unified/viewing-protocol-discovery-dashboard), along with the diagnostics for the related ports on the [Protocol Discovery Diagnostics page](/unified/accessing-protocol-discovery-diagnostics). If you want to select **HTTP** or **HTTPS**, if your web server supports **HTTPS**, select this protocol. Otherwise, select **HTTP**. You can click **Clear Selection** to remove any selections.
...
- Select a web server certificate from the drop-down menu. You can click **Clear Selection** to remove any selections. To learn more, see [About (Web Server) Certificates](/unified/about-web-server-certificates).
...
- From the drop-down menu, select the protocol you want to use when a request is made to access the application (**Dynamic**, **HTTPS**, or **HTTP**). The Dynamic protocol option allows the certificate to be [generated by the AppProtection Certificate](/unified/generating-zscaler-issued-enrollment-ca-certificates). This option is handled by the back end that the application domain is mapped to. If you select this option, you can view the Protocol Discovery details on the [Protocol Discovery Dashboard page](/unified/viewing-protocol-discovery-dashboard), along with the diagnostics for the related ports on the [Protocol Discovery Diagnostics page](/unified/accessing-protocol-discovery-diagnostics). If you want to select **HTTP **or **HTTPS**, if your web server supports **HTTPS**, select this protocol. Otherwise, select **HTTP**. Click **Clear Selection**, to remove any selections.
To select both **Browser Access** and the **AppProtection**, **HTTP** or **HTTPS** needs to be selected.
-
If you selected **HTTP** in step ii above, **Use Untrusted Certificates** is selected by default. Select **Use Untrusted Certificates** to allow inspection with private certificates. This option maintains the TLS connection between the inspection and the application. The verification of the private CA certificate would otherwise fail if this option wasn’t selected. If you do not select **Use Untrusted Certificates** and you are using private CA certificates that are not verified, requests to access applications with inspection fail. If you enable **AppProtection**, then the **Use Untrusted Certificates** option is selected by default. This is because it needs to be selected to verify the CA certificate related to the AppProtection-enabled applications. If you deselect the **Use Untrusted Certificates** checkbox, the App Connector verifies the certificate of the server. Since it’s a self-signed certificate and not recognized by CA, it cannot be established and fails.
Selecting **Use Untrusted Certificates** results in successful application connectivity, regardless of verification of the certificate.
...
[See image.](#PRAbox)
Client-based remote assistance is only available for end users using Zscaler Client Connector.
- Click **Add More** to add more domains or IP addresses for each application you want to define.
When two or more application segments cover the same destination address, Zscaler Client Connector attempts to match traffic to the more granular application segment. If there is no match in this application segment, Zscaler Client Connector bypasses Private Applications and sends traffic directly. To learn more, see [About Application Access](/unified/about-application-access).
- []**Default Port Ranges**: Enter the port ranges to set as the default for the application segment.
- **TCP Keepalive**: Enables the App Connector to periodically send TCP keepalives to an application server when a user has accessed that server on a TCP port. This setting is disabled by default. To learn more, see [Networking Deployed App Connectors](/unified/networking-deployed-app-connectors#Tcpcommunication).
...
To learn more, see [About Double Encryption](/unified/about-double-encryption).
- **Bypass**: From the drop-down menu, choose when users can bypass Private Apps to access an application. You can choose from the following options:
- **Use Client Forwarding Policy**: This option is selected by default. If selected, the decision to forward a user’s application request is defined by the [client forwarding policy](/unified/about-client-forwarding-policy). If none of the policy rules apply, then access to the application is implicitly set to **Forward**. To learn more, see [Configuring Client Forwarding Policies](/unified/configuring-client-forwarding-policies). This is also the case if you do not define any client forwarding policy rules at all within the Admin Portal.
- **Always**: If selected, users can always bypass Private Apps when accessing an application.
- **On Corporate Network**: If selected, users can bypass Private Apps when accessing an application from a trusted network. The user is checked to be on a trusted network defined in the Zscaler Client Connector forwarding profile.
...
- **ICMP Access**: Enable for various clients (Zscaler Client Connector, Cloud and Branch Connectors, or Source IP Anchor) to allow ICMP communication to the applications in this Application Segment. By default, this is set to **Disabled** and ICMP requests are allowed or blocked based on [access policies](/unified/about-access-policy). If you configure an application for ICMP Access, there are some things to consider when using ICMP:
- If ICMP is enabled for a specific application in an application segment and the same application is disabled for ICMP in a different application segment, then Zscaler Client Connector considers ICMP enabled for that application and forwards the ICMP request.
...
- [][]**Health Reporting**: Choose whether the App Connector reports the health status of all applications within an application segment continuously (**Continuous**), while a user is accessing it (**On Access**), or not at all (**None**). The default setting is **On Access**. You can see the status of the application on the Health dashboard. To learn more, see [Understanding Health Reporting](/unified/understanding-health-reporting).
You can't choose **Continuous** health reporting for applications configured with more than 10 ports or if any of the applications in the application segment are defined with only a wildcard (e.g., *). Additionally, if an application has client hostname validation enabled to facilitate client-to-client remote assistance, then the application is not shown on the Health Dashboard. To learn more, see [Validating a Client Hostname](/unified/validating-client-hostname).
- **App Connector Selection Method**: Choose **Closer to Application** if you prefer the App Connector is closest to the application to be selected. By default, the App Connector closest to the user (**Closer to User**) is chosen. After you make a selection, an App Connector that has the shortest round-trip time towards the requested application is selected.
...
This setting is not applicable for Browser Access applications, and Private Apps function as if the option is disabled. If Zscaler Client Connector is on a macOS or an iOS device and the application needs to be accessed by CNAME, it is best to disable this setting.
...
Multimatch is not supported for applications that have Browser Access, Privileged Remote Access, AppProtection, or Source IP Anchoring enabled. If Multimatch is enabled, Health Reporting on an app segment can only be set to** On Access** or **None**. To learn more about limitations, see [Using Application Segment Multimatch](/unified/using-application-segment-multimatch).
[See image.](#img-define-cmmnconfig)
[]![General Information section for Define Applications in the Add Application Segment window within the Admin Portal](/downloads/tech-pubs-drafts/zpa-draft-articles/configuring-defined-application-segments-draft-doc-47541/Updated%20app%20segment%20inspect%20zia.png)
...
[]![Applications section for Define Applications in the Add Application Segment window within the Admin Portal](/downloads/zpa/documentation-knowledgebase/applications/application-segments/configuring-application-segments/Application%20access%20type.png)
[]![Client Connector Access section for Define Applications in the Add Application Segment window within the Admin Portal](/downloads/zpa/documentation-knowledgebase/applications/application-segments/onboard/zpa-configuring-app-segments-client-connector2.png)
...
[]You must identify the segment group to which this application segment belongs. Placing the application segment in a segment group allows you to configure user access policies based on that group.
...
On the **Server Groups** tab, choose one of the following options to configure the server group accordingly, or select **Skip** if you want to enable client-to-client remote assistance.
...
**Extranet **only appears if your organization has the Extranet Application Support feature enabled. To learn more, see [About Extranet](/unified/about-extranet).
...
[See image.](#seeimage7)
[]![Select an existing server group section](/downloads/zpa/documentation-knowledgebase/applications/application-segments/onboard/zpa-c2c-configuring-application-segments3_0.png)
...
- **Dynamic Server Discovery**: If enabled, the appropriate servers for applications as users request the applications are automatically discovered. If disabled, you must manually add defined servers to the server group.
- **Connector Type**: Choose one of the following options:
-
Select **App Connector** to use a Private Applications App Connector to connect to applications that are available on your network.
...
-
Select **Extranet **to use Private Applications to connect to an application at a partner site or offshore development center that is not directly available on your organization’s network, and then choose options for connecting to that site:
Make sure that you set extranet options at the tenant level as Microtenants aren't supported.
- **Extranet Resource**: Choose a partner outside your organization that was configured in Internet & SaaS to connect to the application. You can choose only one partner per server group.
-
**Extranet Location Groups**: Choose a location group if your organization created extranet location groups in Internet & SaaS. The location group represents the set of all the IPSec tunnels that can reach the application at partner locations.
...
[See image.](#seeimage8)
- Click **Next**.
[]![Creating a new server group section](/downloads/zpa/documentation-knowledgebase/applications/application-segments/onboard/zpa-c2c-configuring-application-segments4_0.png)
[]You must add servers to the server group you just created. On the **Server **tab, choose one of the following options, and configure the server accordingly.
...
[]On the **Policies** tab, you see a list of access policies that are applied to the applications defined within the application segment. If you need to add new or modify existing policies, click **Edit Policy**. To learn more, see [About Access Policy](/unified/about-access-policy).
