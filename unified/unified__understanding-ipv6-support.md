...
Source: https://help.zscaler.com/unified/understanding-ipv6-support
PDF: https://help.zscaler.com/pdf/com/en/1494976.pdf
...
IPv6 support is extended by Zscaler based on the [traffic forwarding method](/unified/choosing-traffic-forwarding-methods) and also whether the client device is inside a [location](/unified/about-locations-internet-saas).
- **For clients inside a location:** Forward IPv6 traffic inside an IPv4 tunnel to Internet & SaaS Service Edges using a [GRE tunnel](/unified/understanding-generic-routing-encapsulation-gre) or [IPSec tunnel](/unified/understanding-ipsec-vpns). Both web and non-web traffic can be forwarded using these tunneling methods.
- **For clients outside a location:** Forward web requests using [PAC files](/unified/understanding-pac-files) or Zscaler Client Connector using Z-Tunnel 1.0 to Service Edges via a self-hosted or ISP-provided NAT64 gateway. To forward both web and non-web traffic from IPv6 clients, use Zscaler Client Connector over Z-Tunnel 2.0 and a self-hosted or ISP-provided NAT64 gateway.
...
Furthermore, when using Zscaler Client Connector, you must forward your users' IPv6 traffic only to data centers in the IPv6-enabled subcloud managed by Zscaler. To learn how to do this, see [Configuring IPv6 Settings](/unified/configuring-ipv6-settings#client-connector).
...
- Zscaler's My IP Address service ([ip.zscaler.com](http://ip.zscaler.com)) might not recognize IPv6 traffic passing through the Zscaler cloud. Exercise caution when using the My IP Address service for troubleshooting or [verifying connectivity to the Zscaler cloud](/unified/verifying-users-traffic-being-forwarded-zscaler-service) when IPv6 is enabled on the client.
...
- [Generic Routing Encapsulation (GRE) Tunnel](/unified/understanding-generic-routing-encapsulation-gre)
- [IPSec Tunnel](/unified/understanding-ipsec-vpns)
...
[]When web traffic from IPv6 clients arrives at a Service Edge in [explicit proxy mode](/unified/understanding-proxy-mode) via Zscaler Client Connector over Z-Tunnel 1.0 or using PAC files, the Zscaler service establishes an IPv4 or IPv6 connection to the destination based on how the server can be reached, as described in the following bullet points:
...
[]When traffic from IPv6 clients arrives at a Service Edge in [transparent proxy mode](/unified/understanding-proxy-mode) using an IPv4 tunnel (GRE or IPSec) or via Zscaler Client Connector (Z-Tunnel 2.0 only), the Zscaler service establishes an IPv4 or IPv6 connection to the destination based on how the server can be reached, as described in the following bullet points:
- If the destination IPv6 address has a prefix match with [NAT64 prefixes](/unified/about-nat64-prefixes) supported for the organization, then the IPv4 address is extracted from the destination IPv6 address and an IPv4 connection is established.
...
The Zscaler service uses the well-known prefix and its default NAT64 and DNS64 prefixes for the DNS64/NAT64 mechanism and organizations do *not* require any additional configuration. However, organizations can configure their network-specific NAT64/DNS64 prefixes in the Admin Portal. To learn more, see [About NAT64 Prefixes](/unified/about-nat64-prefixes) and [About the DNS64 Prefix](/unified/about-dns64-prefix).
...
After enabling IPv6 support, you can optionally configure your network-specific NAT64 and DNS64 prefixes under Infrastructure > Internet & SaaS > Traffic Forwarding > IPv6 Configurations > Settings. To learn more, see [Configuring IPv6 Settings](/unified/configuring-ipv6-settings).
...
[]You can configure policies based on [Locations](/unified/about-locations) or [Location Groups](/unified/about-location-groups) criteria to be enforced on all IPv6 traffic that originates from those locations. When a sublocation is added to a location with the IPv6 option enabled, the Zscaler service automatically creates a new Other6 sublocation that identifies all the IPv6 addresses in that location. Using Other6 as the location criteria, you can define policies for all of the IPv6 traffic that originates from that location. To learn more, see [Understanding Sublocations](/unified/understanding-sub-locations).
The Locations and Location Groups criteria are supported in various web and firewall policies.
[]You can configure policies based on URL Categories to be enforced on traffic bound to specific IPv6 sites or destinations. The Zscaler service allows you to add individual domains or IP addresses to URL categories, which can then be used in policies to control the traffic bound to those IPv6 destinations. To learn more, see [Configuring Custom URL Categories](/unified/configuring-custom-url-categories).
The URL Categories criterion is supported in various web and firewall policies.
[]You can configure policies based on Source or Destination IP Address Groups to be enforced on traffic originating from or destined to any IPv6 device. The Zscaler service provides predefined source and destination IPv6 address groups, All IPv6, which encompasses all IPv6 source or destination addresses. Using All IPv6 as the source or destination group criteria, you can define policies for all traffic originating from or destined to an IPv6 device. To learn more, see About [Source](/unified/about-source-ip-groups) or [Destination IP Address Groups](/unified/about-destination-ip-groups).
...
- Web Insights Logs: [Filters](/unified/web-insights-logs-filters) and [Columns](/unified/web-insights-logs-columns)
- Firewall Insights Logs: [Filters](/unified/firewall-insights-logs-filters) and [Columns](/unified/firewall-insights-logs-columns)
- DNS Insights Logs: [Filters](/unified/dns-insights-logs-filters) and [Columns](/unified/dns-insights-logs-columns)
In addition, the [Nanolog Streaming Service](/unified/understanding-nanolog-streaming-service-nss) allows you to stream your logs in real time from the Zscaler Nanolog to your security information and event management (SIEM) system. To learn more, see [Adding NSS Feeds](/unified/adding-nss-feeds).