...
Source: https://help.zscaler.com/unified/configuring-device-posture-profiles
PDF: https://help.zscaler.com/pdf/com/en/1492796.pdf

[Watch a video on device posture profiles.](https://fast.wistia.net/embed/iframe/8op4rkia3r)
Posture profiles for both Internet & Saas and Private Applications devices are created in the Device Posture section. To learn more about posture profiles, see [About Device Posture Profiles](/unified/about-device-posture-profiles). The posture profiles are used for [configuring access policies](/unified/configuring-access-policies) in the Admin Portal and for [adding posture profile trust levels](/unified/adding-internet-saas-posture-profiles) for Internet & SaaS. To learn more about posture profiles for Internet & SaaS, see [About Internet & SaaS Posture Profiles](/unified/about-internet-saas-posture-profiles).
To configure a device posture profile for Zscaler Client Connector:
- In the Admin Portal, go to **Policies > Common Configuration > Resources > Device Posture**.
...
- **Android**
- **iOS**
- **Apply to Windows Machine Tunnel**: When selected, the posture type is evaluated and applied to the Private Applications pre-Windows login machine tunnel. If not selected, the posture type is only evaluated for Private Applications and Internet & SaaS tunnels. Applies to the following device postures for Windows: Client Certificate, Certificate Trust, File Path, Registry Key, Firewall, Full Disk Encryption, Domain Joined, AzureAD Domain Joined, Server Validated Client Certificate, OS Version and Zscaler Client Connector Version.
- **Apply to macOS Machine Tunnel**: When selected, the posture type is evaluated and applied to the Private Applications pre-macOS login machine tunnel. If not selected, the posture type is only evaluated for Private Applications and Internet & SaaS tunnels. Applies to the following device postures for macOS: CrowdStrike ZTA Score, Full Disk Encryption, File Path, Firewall, Domain Joined, and OS Version.
- **Apply when added as Partner Tenant**: Applies to Zscaler Client Connector version 4.6 and later for Windows. When selected, the posture type is evaluated and applied to the Private Application tunnel used to connect to a partner tenant. If not selected, the posture type is only evaluated for Private Applications and Internet & SaaS tunnels.
-
...
- Click **Save**.
After configuring the posture profiles, you can use these profiles to [configure access policies](/unified/configuring-access-policies) in the Admin Portal and [add posture profile trust levels](/unified/adding-internet-saas-posture-profiles) for Internet & SaaS.
...
- If you select **Value**, you must also complete the **Name **and **Data **fields. For example, if you enter the registry key `\HKEY_CURRENT_USER\Software\Zscaler\App`, you would enter `ZNW_State` in the **Name** field and `CONNECTING` in the **Data** field to describe that the service is connecting.
...
[]Applies to Windows, macOS, Linux, and Android. For details on how to configure Linux for the Client Certificate, see [Configuring the Client Certificate Posture Check for Linux](/unified/configuring-client-certificate-posture-check-linux).
...
Select **Client Certificate** from the drop-down menu and upload a certificate issued by an internal root Central Authority trusted by your organization's users. The certificate must be in the user store at the endpoint, issued by the CA that matches the uploaded certificate in the posture profile. The user must have a private and public key pair issued by the specified CA to pass this posture validation check.
Zscaler accepts Base64-encoded .pem and .cer files, and you can upload any one of the following:
- A root CA certificate
- An intermediate certificate
...
- The CRL server is hosted as a Private Applications application which could lead to ambiguous caching behavior due to Private Applications access expiration.
...
| Zscaler Client Connector version 3.5 and later | Zscaler Client Connector version 3.4 and earlier |
| ---------------------------------------------- | ------------------------------------------------ |
| 4d66d506976bde36ae01ab3335d501bec9fb9837f855a4a29ecefdd9ad04384ae3a099aff61d717f9033309926659b4346c496d44407d39e0487868cd9665dc3abce52eaf263dad3412c7cedb9e79b9d | 4d66d506976bde36ae01ab3335d501bec9fb9837 |
...
The following signer certificate matching thumbprints are supported for the CrowdStrike **CSFalconService.exe** executable:
| Zscaler Client Connector version 3.5 and later | Zscaler Client Connector version 3.4 and earlier |
| ---------------------------------------------- | ------------------------------------------------ |
| cd485eaadb584dcf911bb9dae6ecae1253f0530893324968ecd73a9570c347cf0c8ee4bc390d394838b7c74e37392713e436e19a2be053100115da88ca5296e9d87044679d0b405104b8db2c8d56eb03 | cd485eaadb584dcf911bb9dae6ecae1253f05308 |
...
Enter a score between 1 and 100. This number represents a risk score that helps determine the security level of the device. The higher the number you designate, the more secure you consider the device. Zscaler Client Connector compares this number against an overall score (a combined score based on the device OS settings and sensor settings) assigned by CrowdStrike’s user agent. If that overall score is greater than or equal to the score you’ve entered, the posture check passes. If the overall score is less than the score you entered, the posture check fails. To learn more about the ZTA score, refer to the help documentation from CrowdStrike.
![CrowdStrike ZTA Score Device Posture Type](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zscaler-client-connector-device-posture-crowdstrike-zta.png)
...
- [Customizing Zscaler Client Connector with Install Options for Android](/unified/customizing-zscaler-client-connector-install-options-android)
- [Customizing Zscaler Client Connector with Install Options for iOS](/unified/customizing-zscaler-client-connector-install-options-ios)
...
![Detect Antivirus for Device Posture Configuration on the Zscaler Client Connector portal](/downloads/z-app/policy-administration-settings/device-posture/configuring-device-posture-profiles-zpa/clientconnector-configpostureprofile-detectantivirus.png)
...
For Windows only, select an edition from the **OS Editions** drop-down menu, click **Add OS **to add it, and enter a minimum version number. If the machine is running this version or later, then the posture check passes. You can add additional OS editions if more are available after your initial OS selection.
![Detect OS for Device Posture Configuration ](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zclient-connector-device-posture-OS-version-windows.png)
For macOS only, select a version from the** OS Version **drop-down menu, click **Add OS** to add it, and enter the OS version.
![OS Version posture type macOS](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zclient-connector-device-posture-OS-version-macOS_0.png)
For Linux only, select a distribution from the **Linux Distributions** drop-down menu, click **Add OS**, and enter the OS version.
![OS Version posture type Linux](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zclient-connector-device-posture-OS-version-linux.png)
...
![OS version posture type Android](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zclient-connector-device-posture-OS-version-android.png)
...
- **JAMF Risk Level**: Zscaler Client Connector checks for JAMF-provided risk levels. Choose from the following options:
- Secure
- Low risk
- Medium
- High
...
[]Applies only to Zscaler Client Connector version 4.4 for Windows.
...
![CrowdStrike ZTA Score Device Posture Type](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zscaler-client-connector-device-posture-crowdstrike-os.png)
...
![CrowdStrike ZTA Score Device Posture Type](/downloads/unified/policies/client-connector/common-configuration-resources/configuring-device-posture-profiles/zscaler-client-connector-device-posture-crowdstrike-sensor.png)
