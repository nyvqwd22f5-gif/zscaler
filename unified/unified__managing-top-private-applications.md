...
Source: https://help.zscaler.com/unified/managing-top-private-applications
PDF: https://help.zscaler.com/pdf/com/en/1492831.pdf

The list of Top Private Apps is created with readily available data from Zscaler Private Applications to provide an overview of the top private applications and a seamless probe configuration. You can search application data based on the port numbers that you enter and then sort by users or bandwidth. You can configure the private applications and their respective probes in the configuration wizard, which prepopulates with application information from Private Applications.
To view a list of Top Private Apps:
- Go to **Policies **> **Digital Experience Monitoring** > **Configuration **> **Applications **> **Top Private Apps**.
...
- **Configured for ZDX**: Provides status information if the application's probe configuration was enabled or configured on Digital Experience Monitoring.
- **Configure**: The option to configure the probing configuration.
If an application has the field **Configured for ZDX** set to **Yes**, then you cannot configure its probe configuration in the Top Private Apps page. You can access the probe configuration for the application in **Configuration** > **Probes**. To learn more, see [Editing a Probe](/unified/editing-probe).
[See image.](#SearchResults)
To configure a private application's probes on the Top Private Apps page:
...
If you selected only the web probe in step 3, then you do not have the option to configure a Cloud Path Probe.
...
To learn more about configuring a probe, see [Configuring a Probe](/unified/configuring-probe).
[]
