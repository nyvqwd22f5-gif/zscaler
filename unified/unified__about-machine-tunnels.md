...
Source: https://help.zscaler.com/unified/about-machine-tunnels
PDF: https://help.zscaler.com/pdf/com/en/1497151.pdf
...
- Admins can enable **ZPA Machine Authentication** in [App Profiles](/unified/configuring-zscaler-client-connector-app-profiles) to require users to authenticate against their IdP before the machine tunnel starts.
...
To use a machine tunnel, you must [configure Machines groups and Machine Provisioning keys](/unified/configuring-machine-provisioning-keys) in the Admin Portal, add keys to Zscaler Client Connector app profile [rules for Windows or macOS](/unified/configuring-zscaler-client-connector-app-profiles), and [enable the Private Applications machine tunnel for all](/unified/configuring-private-applications-machine-tunnel-all)
...
- **Active**:** **The default status. The machine tunnel is connected to Private Applications before user login.
- **Inactive**: The machine tunnel is not connected to Private Applications for various reasons, including the following:
...
- The machine tunnel was not enabled for the device in [Zscaler Service Entitlement](/unified/about-zscaler-service-entitlement).
- **Removed: **The machine tunnel configuration was deleted or deactivated from the Admin Portal.
