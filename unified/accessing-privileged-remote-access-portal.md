...
Source: https://help.zscaler.com/zpa/accessing-privileged-remote-access-portal
PDF: https://help.zscaler.com/pdf/com/en/1485126.pdf

A Privileged Remote Access (PRA) Portal displays Microtenants, shared privileged sessions, and privileged consoles that your end users are authorized to access. After an end user logs in, the PRA Portal home page appears. The privileged consoles an end user can see are based on access policies for the application segments that are assigned to the privileged consoles. The displayed privileged consoles can be launched directly from the portal by clicking on them. Any privileged consoles with the policy rule set to **Deny** are not visible to the end user.
The PRA Portal is dependent on the [default timeout policy](/zpa/about-timeout-policy#DefaultReauthRule) rule's authentication timeout.
PRA Portal Features
![PRA Portal home page](/downloads/tech-pubs-drafts/zpa-draft-articles/accessing-privileged-remote-access-portal-doc-53920/PRA%20Portal%20w%20BA%20icon%20updated.png)
You can [update the logo and favicon](/zpa/configuring-company-profile) on the Company Profile page.
The icons at the top of the PRA Portal do the following:
- **Browser Access**: Click the **My Web Applications** icon to go to the **My Web Applications** page. This icon is enabled if [User Portal for Portal Links](/zpa/configuring-privileged-portals) has been configured in the PRA Portal.
- **Home**: Click the **Home** icon to return to the PRA Portal landing page. This icon is only displayed if you have the [PRA File Transfer System](/zpa/about-privileged-remote-access-pra-file-transfer-system) feature enabled.
- **PRA File Transfer System**: Click the **PRA File Transfer System** icon to go to the **My Files** and **Uninspected Files** pages. This icon is only interactive if you have the [PRA File Transfer System](/zpa/about-privileged-remote-access-pra-file-transfer-system) feature enabled.
- **End-User Initiated Approvals**: Click the **End-User Initiated Approvals** icon to go to the [End-User Initiated Approvals](/zpa/about-my-approvals) page.
- **Refresh**: Click the **Refresh** icon to refresh the PRA Portal page.
- **Settings**: Click the **Settings** icon to choose a keyboard language, display resolution, and high quality option for the privileged console.
...
After you select a privileged console, you can use the virtual keyboard by selecting the keyboard icon at the top of the screen. When you are using an SSH-privileged console and you are using the virtual keyboard, any entered text is not initially visible. To fix this, you need to do one of the following actions:
...
When you have selected a privileged console in the PRA Portal page, you need to fill out the information in the User Account window. The window will be for VNC, RDP, or SSH, depending on the protocol linked to the privileged console you’ve selected:
...
When you select a privileged console with a Session Recording-enabled [privileged capabilities policy](/zpa/configuring-privileged-capabilities-policies), a Recording notification appears when you are in the privileged console. The recording starts when you are in the privileged session. The recording stops after you disconnect that privileged session. The privileged session recordings can be found on the [Privileged Sessions page](/zpa/accessing-privileged-sessions).
[See image.](#Record)
...
You can search within the **Share Sessions** list using the search bar to locate a specific privileged session. The characters & and + cannot be used in the PRA Portal search field. Use the arrows to navigate the displayed shared privileged sessions. To learn more, see [Configuring Privileged Capabilities Policies](/zpa/configuring-privileged-capabilities-policies).
[See image.](#sessionim)
...
If a privileged console is active and does not include a checkmark, this means that the user can access the privileged console at any time. To learn more, see [Configuring Privileged Approvals](/zpa/configuring-privileged-approvals-draft).
...
[]![Notification message banner in the Privileged Remote Access portal](/downloads/tech-pubs-drafts/zpa-draft-articles/accessing-privileged-remote-access-portal-doc-53920/notification%20banner.png)
[]![Recording shown in a privileged console](/downloads/tech-pubs-drafts/zpa-draft-articles/accessing-privileged-remote-access-portal-doc-53920/Portal%20changes.png)
[]![The microtenants displayed within a PRA portal](/downloads/zpa/privileged-remote-access-management/privileged-portals/accessing-privileged-remote-access-portal/Updated%20Microtenant_0.png)
[]![The privileged sessions displayed in a PRA portal](/downloads/zpa/privileged-remote-access-management/privileged-portals/accessing-privileged-remote-access-portal/Updated%20Sessions_0.png)
[]![The privileged consoles displayed in a PRA portal](/downloads/zpa/privileged-remote-access-management/privileged-portals/accessing-privileged-remote-access-portal/Updated%20Consoles_0.png)
