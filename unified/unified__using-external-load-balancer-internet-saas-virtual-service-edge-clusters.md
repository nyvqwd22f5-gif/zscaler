...
Source: https://help.zscaler.com/unified/using-external-load-balancer-internet-saas-virtual-service-edge-clusters
PDF: https://help.zscaler.com/pdf/com/en/1495651.pdf

Zscaler recommends deploying Internet & SaaS Virtual Service Edges in cluster mode with a minimum of two Virtual Service Edges to ensure redundancy. The virtual machine (VM) images include a load balancer (LB) along with the Virtual Service Edge. Zscaler recommends using the built-in LB. The benefits of using the Zscaler built-in LB are:
...
- Configure the external LB to use one of the monitoring methods listed in the Monitoring section of this article.
Updates
Updates for Virtual Service Edges in standalone mode using an external LB should be scheduled to avoid downtime. To learn more, see [Configuring Internet & SaaS Virtual Service Edge Clusters](/unified/configuring-internet-saas-virtual-service-edge-clusters#Scheduling-Virtual-Service-Edge-Cluster-Updates).