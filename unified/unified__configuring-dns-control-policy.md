...
Source: https://help.zscaler.com/unified/configuring-dns-control-policy
PDF: https://help.zscaler.com/pdf/com/en/1494091.pdf

Configuring a DNS Control rule provides you with greater control over your DNS traffic. You can capture and store traffic blocked through this policy as PCAP files. To learn more, see [About Traffic Capture](/unified/about-traffic-capture).
To learn about the recommended configurations for the DNS Control policy, see [Best Practices for DNS Control Rules](/unified/best-practices-dns-control-rules).
Policy Execution
...
- [Users](/unified/about-users), [groups](/unified/about-groups), [departments](/unified/about-departments), and [locations](/unified/about-locations) to which the DNS Control policy applies
- [Location groups](/unified/about-location-groups)
- [Time intervals](/unified/defining-time-intervals)
- Optionally, [source](/unified/configuring-source-ip-groups) and [destination IPv4](/unified/configuring-destination-ip-groups) address groups
- Optionally, [IPv6 configuration](/unified/understanding-ipv6-support)
-
...
To learn more, see [About DNS Control](/unified/about-dns-control#dns-traffic-control-methods).
...
- Go to **Policies > Access Control > Firewall > DNS Control**.
...
- **Admin Rank**:** **Choose your admin rank. This option appears if you've enabled [Admin Ranking](/unified/about-admin-rank) on the [Advanced Settings](/unified/configuring-advanced-settings) page.
...
- From the **Time** menu, choose the [time interval](/unified/defining-time-intervals) during which the rule applies. Select **Always** to apply this rule to all time intervals, or select up to two time intervals. You can search for a time interval or click the **Add** icon to add a new time interval.
- From the **Location Groups **menu, choose the [location groups](/unified/about-location-groups) to which this rule applies. You can select **Any** to select all location groups, or select up to 32 location groups. You can search for location groups.
If you've enabled the policy for unauthenticated users under [Advanced Settings](/unified/configuring-advanced-settings), and want to apply this rule to any unauthenticated traffic, see [Configuring Policies for Unauthenticated Traffic](/unified/configuring-policies-unauthenticated-traffic). If you want to apply this rule only to remote users' traffic, select **Road Warrior** from the **Locations** field. Rules configured for locations other than Road Warrior also apply to remote user traffic from those locations.
-
On the **Source IPs** tab:
-
Select the [source IPv4 groups](/unified/about-source-ip-groups) that you want to control with this rule. You can also add a new Source IPv4 Group by clicking the **Add **icon. To control source IPv6 addresses with this rule, select the **All IPv6** group, which is the default source IPv6 group for all IPv6 addresses.
...
- **DNS Tunnels & Network Apps**: Select any tunnels** **that you wish to control. Tunnels are categorized as **Commonly Allowed DNS Tunnels**, **Commonly Blocked DNS Tunnels**, or **Unknown DNS Tunnels**. To learn more, see [Detecting and Controlling DNS Tunnels](/unified/detecting-and-controlling-dns-tunnels). In addition to controlling tunneling traffic, you can also include specific **Web** pages, **Social Networking** sites, **Search Engines**, or **Network Services** that you wish to control at the DNS level.
- **DNS Application Groups**: Select any DNS Application Groups that you want to control. To learn more, see [About DNS Application Groups](/unified/about-dns-application-groups) and [Adding DNS Application Groups](/unified/configuring-dns-application-groups).
...
- **Requested Categories**: Select the request categories that you want to control. Select **Any **to ignore the field for matching the rule. Click the **Sync** icon if you want the **Resolved Categories** field value to be the same as **Requested Categories**. You can also [create custom categories](/unified/configuring-custom-url-categories) to control the DNS request.
-
**Resolved Categories: **Select the response categories that you want to control. Select **Any **to ignore the field for matching the rule. Click the **Sync** icon if you want the **Requested Categories** field value to be the same as **Resolved Categories**. You can also [create custom categories](/unified/configuring-custom-url-categories) to control the DNS response.
You can also select [custom TLD categories](/unified/about-tld-categories) from the **Requested Categories** and **Resolved Categories** fields.
-
...
When you select this action while Traffic Capture is enabled, an additional **Capture** field appears. Enable this option to capture and store traffic in PCAP files. To learn more, see [Configuring Traffic Capture](/unified/configuring-traffic-capture).
-
...
In the **DNS Gateway** field, select the DNS Gateway that must be used to redirect the DNS request. The DNS Gateway must be configured to support protocols used in incoming requests to redirect the DNS requests successfully. To learn more, see [About DNS Gateways](/unified/about-dns-gateways).
-
...
-
**Resolve by ZPA**: Resolve domain names of Private Applications to an ephemeral IP address from a preconfigured IP pool. This action is used for redirecting traffic to Private Applications and is applicable only to application segments that have Source IP Anchoring enabled.
...
When this option is enabled, an additional **Notification Message** drop-down menu appears. You can select from default and [custom notification messages](/unified/configuring-euns-dns-control) to display for users.
This feature configuration requires Advanced DNS which is provided by the Advanced Firewall. The EUN is supported on Windows devices running Zscaler Client Connector version 4.8 or later over Z-Tunnel 2.0. To learn more, see [Configuring EUNs for DNS Control](/unified/configuring-euns-dns-control).
[See image.](#zscaler-client-connector-eun)
To learn more, see [DNS End User Notifications.](/unified/dns-end-user-notifications#advanced-eun)
...
After adding rules to the DNS Control Policy, you might also need to do the following before [enabling the firewall](/unified/enabling-firewall-locations) for your locations.
- Modify rules for the [NAT Control Policy](/unified/about-nat-control) and [Firewall Filtering Policy](/unified/configuring-firewall-filtering-policy) as applicable.
- Configure [custom ports](/unified/configuring-custom-ports) as applicable.
...
Zscaler recommends using the **Web EUN** notification for the **Block** action to display an EUN web page for blocked domains, as described in the preceding section. The static DNS EUN web page, accessible at `34.215.46.88`, is intended for users whose HTTP web requests are sent directly to the internet (without passing through Internet & SaaS) from unknown locations, or when the client browser/device does not trust the Zscaler CA certificate or the organization's custom certificate and it is not desirable to display a TLS certificate warning to the end users. Also, the static DNS EUN web page (at `34.215.46.88`) comes with certain limitations.
To learn more, see [DNS Static Web End User Notification](/unified/dns-static-web-end-user-notification) and [DNS End User Notifications](/zia/dns-end-user-notifications).
- []It is recommended to send DNS requests from users directly to or through Internet & SaaS (instead of sending them via a DNS server or forwarder) if it is recursive DNS traffic. Since these requests include user context, Internet & SaaS can enforce policies based on users, groups, and departments.
- For DNS traffic that comes from a DNS server, Internet & SaaS sees only the source and not the user context. Send DNS traffic from a DNS server directly to Internet & SaaS (DNS server acts as a forwarder) or transit this traffic via Internet & SaaS if it is recursive.
- The DNS traffic transiting Internet & SaaS can be intercepted using a Destination NAT rule. Direct recursive DNS requests sent through Internet & SaaS (i.e., transiting Internet & SaaS) to the Zscaler Trusted Resolver using the predefined NAT rule, Zscaler Trusted DNS Resolver rule. DNS traffic directed to Internet & SaaS is already using the Zscaler Trusted Resolver.
The Zscaler Trusted DNS Resolver rule is enabled by default.
[]Iterative DNS requests from DNS servers should only transit Internet & SaaS (i.e., pass through Internet & SaaS and not be sent directly to it). Zscaler can secure iterative DNS traffic that transits Internet & SaaS, but enabling the Zscaler Trusted DNS Resolver rule disrupts this type of traffic.
