...
Source: https://help.zscaler.com/unified/configuring-forwarding-profiles-zscaler-client-connector
PDF: https://help.zscaler.com/pdf/com/en/1494926.pdf

The [forwarding profile](/unified/about-forwarding-profiles) tells Zscaler Client Connector how to treat traffic from your users' systems in different network environments for the Internet & SaaS and Private Applications services.
...
You cannot configure Trusted Network Criteria for a network (i.e., **DNS Server**, **DNS Search Domains**, or **Hostname and IP**) and use the **Pre-defined Trusted Networks** option at the same time.
...
[]Select the **Tunnel Driver Type** to use when handling traffic for Windows devices when in [**Tunnel** mode](/unified/configuring-forwarding-profiles-zscaler-client-connector#tunnel):
...
[]Define how Zscaler Client Connector treats traffic from your users' systems for the Internet & SaaS service for On-Trusted Network, VPN-Trusted Network, Off Trusted Network, or Split VPN-Trusted Network types. To learn more, see [About Forwarding Profiles](/unified/about-forwarding-profiles).
...
-
(Optional) For **Tunnel Version Selection**, you can select which version of Z-Tunnel to use. To see this field, you must select **Packet Filter-Based** for the driver method.
To use Z-Tunnel 2.0, you must complete all configuration steps. To learn more, see [Z-Tunnel 2.0](/unified/about-z-tunnel-1-0-z-tunnel-2-0).
...
- For **Block Unreachable Domains Traffic**,** **this option drops or bypasses domains that are not reachable from the Internet & SaaS Public Service Edge.
...
- **Threshold Rank**: Ranks the change in latency of the primary and secondary Internet & SaaS Public Service Edges, Private Service Edges, or Virtual Service Edges. Select a rank from 1 to 5 from the drop-down menu. If the value exceeds the threshold limit, the switch is made from the primary to the secondary Service Edge, or vice versa. Rank 1 is a minimum change, which is the most aggressive for switching from the primary to the secondary Service Edge.
-
...
- **PAC URL**: If you select this option, enter a valid PAC URL in the **Custom PAC URL** field. The maximum number of characters is 512. If [Enforce Secure PAC URLs](/unified/enforcing-secure-pac-urls) is enabled, you must enter a URL with the https:// prefix (not http://).
...
- **Use Proxy Server for Your LAN**: Select this option if you want to use a specific proxy server and port. For **IP Address or Domain**, enter a FQDN, IP address, or a plain hostname with the http:// or https:// prefix. For **Port**, enter any port ranging from 1 to** **65534. Select **Bypass Proxy Server for Local Addresses** if you want to bypass local resources.
...
- **Block all traffic**: Zscaler Client Connector blocks all traffic.
-
...
Applies only to Zscaler Client Connector version 4.7 for Windows and is available only if **Fall back to Z-Tunnel 1.0 and bypass non-web traffic** or **Block all traffic** is selected.
...
-
**Drop IPv6 Include Traffic When not supported**:** **Enable this option to drop IPv6 traffic that matches the destination inclusion if IPv6 is not provisioned for the company or the Internet & SaaS Public Service Edge.
...
Zscaler recommends enabling both **Redirect Web Traffic to Zscaler Client Connector Listening Proxy** and **Use Z-Tunnel 2.0 for Proxied Web Traffic**. For more information, see [Best Practices for Adding Bypasses for Z-Tunnel 2.0](/unified/best-practices-adding-bypasses-z-tunnel-2-0).
...
If you select Z-Tunnel 1.0, you have the option to select **Block Unreachable Domains Traffic**. This drops or bypasses domains that are not reachable from the Internet & SaaS Public Service Edge.
In the **Configure System Proxy Settings** drop-down menu, define the proxy settings for your users’ systems.
For **Proxy Action Type**:
...
- **Execute GPO Update**: Select this option if you want to execute the GPO update command on Windows devices when Zscaler Client Connector detects a network change.
- To ensure that Zscaler Client Connector never updates any system proxy settings for user devices, select **Never**.
[]Define how Zscaler Client Connector treats traffic from your users' systems for the Private Applications service for On-Trusted Network, VPN-Trusted Network, Off-Trusted Network, or Split VPN-Trusted Network types. To learn more, see [About Forwarding Profiles](/unified/about-forwarding-profiles).
...
- **On-Trusted Network**:
-
**Tunnel**: Zscaler Client Connector tunnels traffic for the Private Applications service (i.e., Zscaler Client Connector uses the Private Applications service to provide users with access to internal applications even when users are on trusted networks).
...
- **Threshold Rank**: Ranks the change in latency of the primary and secondary Private Applications Public Service Edges. Select a rank from 1 to 5 from the drop-down menu. If the value exceeds the threshold limit, the switch is made from the primary to the secondary Service Edge, or vice versa. Rank 1 is a minimum change, which is the most aggressive for switching from the primary to the secondary Service Edge.
- **Enable Machine Tunnel**: Select to enable **Dynamic ZPA Service Edge Assignment** for machine tunnels.
- **None**: Private Applications is disabled and Zscaler Client Connector does not provide access to internal applications. Users access the applications directly.
-
**VPN-Trusted Network**:
Do not select **Tunnel **for the **VPN-Trusted Network **type**.**
-
**Tunnel**: For Private Applications, the app does not forward user traffic if a VPN is also running on the device.
You can enable **Dynamic ZPA Service Edge Assignment **to configure settings for probe interval, probe sample size, and threshold rank:
- **Probe Interval (In Seconds)**: The time interval in which an IP address connects with a server. Select an option from the drop-down menu. Options are in 30-second intervals and range from 30 to 600 seconds. The default is 30 seconds.
- **Probe Sample Size**: The number of retries required to establish a connection before a failover. Select an option from the drop-down menu. The default is 5 times.
- **Threshold Rank**: Ranks the change in latency of the primary and secondary Private Applications Public Service Edges. Select a rank from 1 to 5 from the drop-down menu. If the value exceeds the threshold limit, the switch is made from the primary to the secondary Service Edge, or vice versa. Rank 1 is a minimum change, which is the most aggressive for switching from the primary to the secondary Service Edge.
- **Enable Machine Tunnel**: Select to enable **Dynamic ZPA Service Edge Assignment** for machine tunnels.
- **None**: Private Applications is disabled and Zscaler Client Connector does not provide access to internal applications.
- **Off-Trusted Network**:
-
**Tunnel**: Zscaler Client Connector tunnels traffic for the Private Applications service (i.e., Zscaler Client Connector uses the Private Applications service to provide users with access to internal applications when users are off trusted networks).
You can enable **Dynamic ZPA Service Edge Assignment **to configure settings for probe interval, probe sample size, and threshold rank:
- **Probe Interval (In Seconds)**: The time interval in which an IP address connects with a server. Select an option from the drop-down menu. Options are in 30-second intervals and range from 30 to 600 seconds. The default is 30 seconds.
- **Probe Sample Size**: The number of retries required to establish a connection before a failover. Select an option from the drop-down menu. The default is 5 times.
- **Threshold Rank**: Ranks the change in latency of the primary and secondary Private Applications Public Service Edges. Select a rank from 1 to 5 from the drop-down menu. If the value exceeds the threshold limit, the switch is made from the primary to the secondary Service Edge, or vice versa. Rank 1 is a minimum change, which is the most aggressive for switching from the primary to the secondary Service Edge.
- **Enable Machine Tunnel**: Select to enable **Dynamic ZPA Service Edge Assignment** for machine tunnels.
- **None**: Private Applications is disabled and Zscaler Client Connector does not provide users with access to internal applications. Users can only access internal applications if they have another mechanism for reaching internal networks.
- **Split VPN-Trusted Network**:
-
**Tunnel**: For Private Applications, the app can forward any traffic not going through Split Tunnel VPN to Private Applications. This condition can require VPN configuration to exclude Private Applications infrastructure.
You can enable **Dynamic ZPA Service Edge Assignment **to configure settings for probe interval, probe sample size, and threshold rank:
- **Probe Interval (In Seconds)**: The time interval in which an IP address connects with a server. Select an option from the drop-down menu. Options are in 30-second intervals and range from 30 to 600 seconds. The default is 30 seconds.
- **Probe Sample Size**: The number of retries required to establish a connection before a failover. Select an option from the drop-down menu. The default is 5 times.
- **Threshold Rank**: Ranks the change in latency of the primary and secondary Private Applications Public Service Edges. Select a rank from 1 to 5 from the drop-down menu. If the value exceeds the threshold limit, the switch is made from the primary to the secondary Service Edge, or vice versa. Rank 1 is a minimum change, which is the most aggressive for switching from the primary to the secondary Service Edge.
