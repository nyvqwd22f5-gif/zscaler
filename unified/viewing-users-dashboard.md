...
Source: https://help.zscaler.com/unified/viewing-users-dashboard
PDF: https://help.zscaler.com/pdf/com/en/1498036.pdf

The Users dashboard (Analytics > Reports > Private Applications > Users) provides information about user activity in your organization.
[See image.](#img_usersdbrd)
Dashboard Tools
...
- **Current Connected Users**: View the total number for this category at the top of the page, and then view its details in the widget below. This number is based on the users who have logged into the Zscaler Client Connector and who are authenticated and connected to Private Applications.
...
If you are using the [Log Streaming Service](/unified/about-log-streaming-service) (LSS), the Users dashboard includes information for a Private Applications LSS Client user. This user represents the LSS service, not an actual user. Also, each log receiver is displayed as an application, to reflect the data coming in from the service. To learn more, including how to stop the LSS service from streaming Private Applications LSS Client logs, see [Configuring a Log Receiver](/unified/configuring-log-receiver#Step2).
...
For users accessing applications using Browser Access through unauthenticated HTTP preflight OPTIONS request, the username will appear as **Private Applications BA Unauthenticated**. To learn more, see [Configuring Application Segments](/unified/configuring-defined-application-segments#BrowserAccess_NewAppSeg).
...
) to export a file (.csv) containing information on the users that accessed applications or successful machine tunnel connections with Private Applications for the selected time frame (i.e., Timestamp (in UTC) and username).
[]This widget displays real-time information of users and machine tunnels currently connected to Private Application.
...
- Click on a user or machine tunnel to view more details in **Diagnostics**.
- Click the **Download** icon (![Download icon within the ZPA Admin Portal](/downloads/zpa/dashboard-diagnostics/about-applications-dashboard/zpa-dashboardwidgets-downloadicon.png)
) to export a file (.csv) containing information on the users or machine tunnels that are currently accessing applications with Private Applications for up to the last 30 minutes.
...
[]![Users Dashboard within the ZPA Admin Portal](/downloads/unified/analytics/private-applications/application-user-and-health-monitoring/viewing-users-dashboard/unified-privateapp-usersdashboard.png)