...
Source: https://help.zscaler.com/unified/configuring-index-tool-amazon-web-services
PDF: https://help.zscaler.com/pdf/com/en/1493286.pdf

Before you can create index templates for DLP dictionaries (i.e.,[ Exact Data Match (EDM)](/unified/about-exact-data-match) and [Indexed Document Match (IDM)](/unified/about-indexed-document-match) templates), you must configure the virtual machine (VM) image for the Index Tool with Amazon Web Services (AWS), Azure, or VMware.
To learn more about the VMware Index Tool, see [Configuring the Index Tool for Azure VMs](/unified/configuring-index-tool-azure) and [Configuring the Index Tool with VMware](/unified/configuring-index-tool-vmware).
...
- **CPUs**: 4 CPUs. Zscaler requires 4 CPUs because the CPUs ensure that hash generation performance is not impacted.
- **RAM**: 64 GB
- A [Zscaler Index Tool](/unified/about-index-tool) added in the Admin Portal. You need this configuration to complete the VM setup.
...
- Make sure you have [added an Index Tool Configuration](/unified/adding-index-tool-configuration). You need this configuration to complete the VM setup.
...
- Go to **Zscaler Index Tool Requirements**. The **Zscaler Index Tool Requirements** page is displayed listing all of the outbound connections that need to be configured for the Zscaler Index Tool to communicate with the Zscaler cloud.
...
- **Inbound rules:** Add the incoming traffic IPs for the Index Tool VM. Make sure that you add the IPs that are listed on the **Zscaler Index Tool Requirements** page. See steps 5a through 5d.
- **Outbound Rules:** Add the outgoing traffic IPs for the Index Tool VM.
...
- In the **Amazon Machine Image (AMI)** drop-down menu, select the AMI image that Zscaler Support shared with you (e.g., Zscaler ADP/EDM 500GB (6.2r.46.331757)).
...
- Go back to the Admin Portal, and go to **Policies** >** Data Protection **>** Common Resources **>** Index Tool**.
- Locate the [Index Tool Configuration](/unified/about-index-tool) you added previously, and under the **SSL Certificate** column click **Download**.
...
- Enter the domain name that is used for the Index Tool's fully qualified domain name (FQDN). For example, if the Index Tool is reachable from `indextool.mycompany.com`, then the domain name entered here would be `mycompany.com.`The self-signed certificate would be generated for `*.mycompany.com.`
[See image.](#VM_CLI5)
-
