...
Source: https://help.zscaler.com/zia/understanding-predefined-firewall-filtering-rules
PDF: https://help.zscaler.com/pdf/com/en/1401671.pdf

Zscaler provides a few predefined firewall filtering rules in the Firewall Filtering Policy page. These rules are used to allow or block specific types of traffic from your network to the internet. To learn more, see [About Firewall Control](/zia/about-firewall-control).
Some predefined firewall filtering rules are visible only after you enable certain related settings for the first time. For example, Office 365 One Click Rule is predefined. It appears in the Firewall Filtering Policy page, only after the Enable Microsoft-Recommended One Click Office 365 Configuration option in the Advanced Policy Settings page is enabled for the first time.
...
- **URL Categories**: This attribute is preset to** Zscaler Proxy IPs**. It includes IP addresses owned by Zscaler's data centers, and services such as IP addresses of the ZIA Public Service Edge on a cloud and global VIP service respectively.
Zscaler recommends not to edit the attributes of this rule.
To learn more about the attributes of firewall filtering rule, see [Configuring the Firewall Filtering Policy](/zia/configuring-firewall-filtering-policy)
[]When [IPv6 support is enabled](/zia/configuring-ipv6-settings) at the organization level, a default firewall filtering rule (Block All IPv6) is automatically created to block IPv6 traffic from your organization to any destination through the Zscaler service.
...
You can modify these attributes to suit your organization’s requirements. To learn more about the attributes of firewall filtering rules, see [Configuring the Firewall Filtering Policy](/zia/configuring-firewall-filtering-policy).
When IPv6 support is disabled, the Block All IPv6 rule is deleted from the ZIA Admin Portal. The rule is recreated in an enabled state with the Block/Drop action when IPv6 support is enabled again. Therefore, the Block All IPv6 rule exists *only* when IPv6 support is enabled.
...
- **Action**: (Modifiable) Under this section, Logging option is set to **Full Logging** by default. Additionally, if [Traffic Capture is enabled](/zia/configuring-traffic-capture), the **Capture** option appears. By enabling this option, you can capture blocked traffic and store it in PCAP files for later analysis. To learn more, see [About Traffic Capture Settings](/zia/about-traffic-capture).
To learn more about the attributes of firewall filtering rules, see [Configuring the Firewall Filtering Policy](/zia/configuring-firewall-filtering-policy).