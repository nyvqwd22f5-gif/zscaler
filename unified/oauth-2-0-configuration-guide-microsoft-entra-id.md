...
Source: https://help.zscaler.com/unified/oauth-2-0-configuration-guide-microsoft-entra-id
PDF: https://help.zscaler.com/pdf/com/en/1495116.pdf
...
- You have configured the necessary [API Roles](/unified/adding-api-roles) in the Admin Portal and assigned appropriate API permissions.
...
- [1. Register the Internet & SaaS API client application.](#register-client-app)
- [2. Configure the Internet & SaaS API service to expose a web API.](#register-app-and-expose-web-api)
...
- In the Microsoft Entra admin center, go to **Identity **> **Applications **> **App registrations**.
-
Select the Internet & SaaS API client app configured in [Step 1](#register-client-app).
...
-
Navigate to the **My APIs** tab and then click the Internet & SaaS API app that you configured in [Step 2](#register-app-and-expose-web-api).
...
- In the Microsoft Entra admin center, go to **Identity **> **Applications **> **App registrations**.
- Select the Internet & SaaS API app configured in [Step 2](#register-app-and-expose-web-api).
-
...
- **OAuth 2.0 JWKS Location:** Enter the JSON Web Key Set (JWKS) endpoint value obtained through the `jwks_uri` parameter in [Configuring Microsoft Entra ID](#collect-info). The JWKS endpoint returns the public key set of the authorization server that is used by Zscaler to cryptographically verify the authenticity of the JWT in API requests.
