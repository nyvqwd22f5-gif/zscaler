...
Source: https://help.zscaler.com/unified/deploying-zscaler-client-connector-jamf-pro-macos
PDF: https://help.zscaler.com/pdf/com/en/1492306.pdf
...
- **Property List**: Enter the following PLIST file and change the values per your organization's settings.
...
<string>1</string>
<key>policyToken</key>
<string> 363036373A343A35646161663635372D666165332D346531642D383839652D613961303565626131313532</string>
...
- **deviceToken**: The appropriate device token from the Admin Portal, if you want to use the [Zscaler Client Connector Portal as an IdP](/unified/using-zscaler-client-connector-identity-provider).
...
- **externalRedirect**: Allows you to redirect authentication to your organization's SAML Identity Provider (IdP) through the Safari browser. When redirected to the browser for the first time, the users must select **Remember Me** on their IdP login screen. For any subsequent authentications, the browser remembers the user and automatically logs them in.
- **userName**: The username for the user. For example, if the username is j.doe@zscaler.com, you would enter` j.doe`.
- **externalDeviceId**: The identifier that associates an external MDM device ID with devices in the Admin Portal. You can use this to associate devices in an MDM solution with devices in the Admin Portal. The externalDeviceId parameter is not supported on Zscaler Client Connector version 4.1 and earlier for macOS.
...
- Better performance
- Z-Tunnel 2.0 compatibility with third-party VPN clients
...
- In the **Scope** section, set **Targets**, **Limitations**, and **Exclusions** to bind the configuration profile to particular devices and users.
- []In the Jamf Pro Portal, go to **Computers** > **Configuration Profiles**.
- [Download the mobileconfig file](/downloads/zscaler-client-connector/downloading-deployment/deploying-zscaler-client-connector-jamf-pro-macos/TRPSample_SysExt.mobileconfig).
...
- Under Platform Settings, select **macOS**, and add a new macOS policy or edit an existing one.
...
- In the **Scope** section, set **Targets**, **Limitations**, and **Exclusions** to bind the configuration profile to particular devices and users.
[]This configuration setting is only supported with Route-based Traffic Interception and is not compatible with Transparent Proxy-based Traffic Interception.
You can use a property list (PLIST) file to configure a custom VPN profile for [VPN (for Legacy Apps)](/unified/about-network-connectors) in the Jamf Pro Portal:
- In the Jamf Pro Portal, go to **Computers** > **Configuration Profiles**.
- In **General**, select **New** to create a new profile and enter a name for the profile.
- From the left-side navigation, go to **VPN** and click **Configure**.
...
[]![VPN settings](/downloads/tech-pubs-drafts/client-connector-draft-articles/deploying-zscaler-client-connector-jamf-pro-macos-doc-57940/deploy-config-profile-system-extension-vpn-config-pkt.png)
...
- To ensure a seamless user experience, configure the system extension in Jamf Pro and activate it. To learn more, see [Configuration Profile for Auto Approval of Zscaler System Extension](/unified/blocking-lan-access#configuration-profile-auto-approval-Zscaler-system-extension).
- Click **Save**.
- In the **Scope** section, set **Targets**, **Limitations**, and **Exclusions** to bind the configuration profile to particular devices and users.
...
- Select **Upload**.
- Upload the mobile config file.
- Use the [ZscalerSampleMobileConfig](/downloads/zscaler-client-connector/downloading-deployment/deploying-zscaler-client-connector-jamf-pro-macos/TRPSample_SysExt_Appbypass.mobileconfig) file as a starting template, and edit the values in the `<VendorConfig>` section of the file based on your needs.
- To ensure a seamless user experience, configure the system extension in Jamf Pro and activate it. To learn more, see [Configuration Profile for Auto Approval of Zscaler System Extension](/unified/blocking-lan-access#configuration-profile-auto-approval-Zscaler-system-extension).
- In the **Scope** section, set **Targets**, **Limitations**, and **Exclusions** to bind the configuration profile to particular devices and users.
- []In the Admin Portal, go to **Infrastructure** > **Connectors** > **Client**.
- Under Platform Settings, select **macOS**, and edit the respective profile from the list.
