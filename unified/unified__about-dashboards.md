...
PDF: https://help.zscaler.com/pdf/com/en/1497406.pdf

Predefined dashboards are available in the Admin Portal. These dashboards contain widgets according to your organization's subscriptions. For example, if your organization has a subscription to the web security service only, you can see dashboards and widgets that report on web traffic, but not those that report on mobile traffic. Each dashboard contains [widgets](/unified/about-widgets) that present data in interactive charts, so you can instantly jump from a chart to individual transactions. A dashboard or widget displays the message "View Restricted"** **when you do not have the appropriate permissions to view its content.
Depending on your [role permission](/unified/adding-admin-roles) and [scope](/unified/about-admin-scope), you can customize, view and edit the predefined dashboards. When you customize a dashboard, the changes are visible only to you. To view any newly-released widgets, the customized dashboards must be reset.
...
- The **Top URL Categories** widget displays the number of transactions for the top [URL categories](/unified/about-url-categories).
...
-
The **Sandbox Patient 0 Events** widget lists patient 0 events that occurred in your organization. This widget is available only if you have Advanced Sandbox. To learn more, see [About the Sandbox Patient 0 Event Widget](/unified/configuring-patient-0-alert#about-sandbox-patient-0-events-widget).
If you have Advanced Sandbox and don't see the **Sandbox Patient 0 Events** widget, [reset](/unified/about-dashboards#what-to-do) the Security dashboard.
...
- The **Web Browsing Trend** widget provides the browsing activity trend of your users for each [URL class](/unified/about-url-categories) in the selected time period.
...
- The **Top Blocked URL Categories** widget displays the number of attempts to access URL categories that were blocked due to the configured [URL policies](/unified/about-url-filtering).
...
- The **Top Users** widget lists the users with the most browsing activity, from the user with the most number of transactions to the least.
- The **Top Blocked Rules Hit** widget lists the [DNS policy](/unified/about-dns-control) rules that were enforced when traffic was blocked.
...
[]This dashboard provides data about the [firewall](/unified/about-firewall-filtering) traffic of your organization.
...
- The **Top Users** widget lists the users with the most browsing activity, from the user with the most number of transactions to the least.
- The **Top Blocked Rules Hit** widget lists the [firewall policy rules](/unified/understanding-firewall-capabilities) that were enforced when traffic was blocked.
...
[See image.](#office365)
[]This dashboard provides real-time visibility into your organization's [IPS](/unified/about-ips-control) traffic.
...
On any Dashboard page (Analytics > Internet & SaaS > Dashboard), you can do the following:
...
- View, edit, delete, resize, and move the widgets that the dashboard contains. To learn more, see [About Widgets](/unified/about-widgets).
- Edit or delete a note to the dashboard. To learn more, see [Adding Notes to Dashboards and Interactive Reports](/unified/adding-notes-dashboards-and-interactive-reports).
- Roll over a chart to obtain more information about an item, click it, and select one of the following options:
-
**View Logs** to see its associated logs. This option is not available for charts that display secure browsing class, data or status, because they only use samples as their data units. To learn more, see [About Insights Logs](/unified/about-insights-logs).
If you select this option** **for the following widgets on the Office 365 dashboard, then some predefined filters are automatically set on the [Insights Logs](/unified/about-insights-logs) page:
...
-
**Analyze Chart** to view the chart in the **Analytics **tab where you can interactively drill down to the logs. To learn more, see [Analyzing Traffic Using Insights](/unified/analyzing-traffic-using-insights).
If you select this option** **for the following widgets on the Office 365 dashboard, then some predefined data types and filters are automatically set on the [Web Insights](/unified/about-insights) page:
...
- Hide or show all notes. To learn more, see [Adding Notes to Dashboards and Interactive Reports](/unified/adding-notes-dashboards-and-interactive-reports).
...
- Add a note to the dashboard. To learn more, see [Adding Notes to Dashboards and Interactive Reports](/unified/adding-notes-dashboards-and-interactive-reports).
- Create a new widget. A dashboard can contain up to 12 widgets. To learn more, see [About Widgets](/unified/about-widgets).
...
[]For this widget, on the **Insights Logs** page, the [filters](/unified/web-insights-logs-filters) are set as follows:
...
[]For this widget, on the **Web Insights** page, the [data type and filters](/unified/web-data-types-and-filters) are set as follows:
...
[See image.](#other)
![Dashboard Sample](/downloads/unified/analytics/internet-saas/about-dashboards/Unified-About-Dashboards.png)
...
To learn more, see [Hosting and Security Characteristics](/unified/about-shadow-it-report#hosting-security).
MCAS
