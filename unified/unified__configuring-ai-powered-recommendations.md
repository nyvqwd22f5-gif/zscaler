...
Source: https://help.zscaler.com/unified/configuring-ai-powered-recommendations
PDF: https://help.zscaler.com/pdf/com/en/1520811.pdf

AI-powered recommendations for application segments provide you with helpful configuration options and additional application segment opportunities for your consideration. When you find an AI-powered recommendation that you want as a defined application segment, you can [merge it with an existing defined application](/unified/merging-ai-powered-recommendations), or you can add the AI-powered recommendation on its own. You can configure the requirements for AI-powered recommendations on the [AI-Powered Recommendations Settings](/unified/configuring-ai-powered-recommendations-settings) page. This article provides steps to configure an AI-powered recommendation to become a defined application segment and move it to the **Defined Application Segments** page.
...
This setting is tied to Source IP Anchoring for Internet & SaaS. When enabled, you can connect to it in Internet & SaaS. You can't delete the application while in use by a Internet & SaaS entity. There are ranges and limitations for Source IP Anchoring. To learn more, see [Understanding Source IP Anchoring](/unified/understanding-source-ip-anchoring) and [Ranges & Limitations](/unified/ranges-limitations).
...
- For applications that users access using host names (e.g., DFS), ensure that you [configure DNS search suffixes](/unified/adding-dns-search-domains) so that Private Applications can automatically add the search domain to the host name.
When defining applications for Browser Access, you can only configure FQDNs.
To learn more, see [Defining an Application](/unified/about-application-access#AboutApplicationDefinitions) and [Configuring Application Discovery](/unified/about-application-discovery#config_appdiscovery).
...
[]Private Applications Browser Access requires that the application server supports TLS 1.2 encryption.
...
- (Optional) Enter a domain in the **Domain for Browser Isolation** field that Private Applications can use in place of the application domain for browser isolation. To learn more, see [What Is Isolation?](/zia/about-cloud-browser-isolation)
- From the drop-down menu, select the certificate you want Private Applications to use when a request is made to access the application. If your web server supports **HTTPS**, select this protocol from the drop-down menu. Otherwise, select **HTTP**. You can click **Clear Selection** to remove any selections.
-
...
Private Applications inserts a Via header in HTTP requests made using Browser Access.
...
- (Optional) Select **Allow Unauthenticated OPTIONS Request** to allow Private Applications to forward an unauthenticated HTTP preflight OPTIONS request from the browser to the application.
Private Applications only accepts valid OPTIONS requests that include the headers Access-Control-Request-Headers, Access-Control-Request-Method, and Origin in the HTTP request. If these are not present, Private Applications doesn't forward the OPTIONS request to the application.
Private Applications accepts CORS requests if the requests are issued by one valid Browser Access domain to another Browser Access domain. The application server requires `with credentials mode` be added to JavaScript. This is to allow the browser to pass cookies to the JavaScript front end. The application server must also allow requests where the Origin header is set to null or to a valid Browser Access application.
...
- **TCP Keepalive**: Enables the App Connector to periodically send TCP keepalives to an application server when a user has accessed that server on a TCP port. TCP keepalive is disabled by default. To learn more, see [Networking Deployed App Connectors](/unified/networking-deployed-app-connectors#Tcpcommunication).
-
...
-
**Bypass**: From the drop-down menu, choose when users can bypass Private Applications to access an application. You can choose from the following options:
- **Use Client Forwarding Policy**: This option is selected by default. If selected, the decision to forward a user’s application request to Private Applications is defined by the [client forwarding policy](/unified/about-client-forwarding-policy). If none of the policy rules apply, then access to the application is implicitly set to **Forward to ZPA**. To learn more, see [Configuring Client Forwarding Policies](/unified/configuring-client-forwarding-policies). This is also the case if you do not define any client forwarding policy rules at all within the Admin Portal.
...
- The [IP](/unified/accessing-user-activity-diagnostics#connector) data appears and the Internal Status Code on the [User Activity Diagnostics page](/unified/accessing-user-activity-diagnostics#time) shows as successful (e.g., BRK_MT_TERMINATED) after the App Connector is selected, even though it is unreachable. This can occur if the App Connector is set up to provide on access or no [health reporting](/unified/understanding-health-reporting), as health reporting does not work well with ICMP.
...
[]
-
[]**Health Reporting**: Choose whether the App Connector reports the health status of all applications within an application segment continuously (**Continuous**), while a user is accessing it (**On Access**), or not at all (**None**). The default setting is **On Access**. You can see the status of the application on the [Health dashboard](/unified/viewing-health-dashboard). To learn more, see [About Health Reporting](/unified/understanding-health-reporting).
...
[]You must identify the segment group to which this application segment is associated with. Placing the application segment in a segment group allows you to configure user [access policies](/unified/about-policies) based on that group.
...
On the **Server Groups** tab, choose one of the following options to configure the server group accordingly, or select **Skip** if you want to enable client-to-client remote assistance. To learn more, see [Validating a Client Hostname](/unified/validating-client-hostname).
...
If you select or add a server group with [Dynamic Server Discovery](/unified/enabling-dynamic-server-discovery) enabled, the **Servers** step is no longer required because the server group automatically discovers the server.
Click **Next**.
