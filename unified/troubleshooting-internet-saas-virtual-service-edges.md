...
Source: https://help.zscaler.com/unified/troubleshooting-internet-saas-virtual-service-edges
PDF: https://help.zscaler.com/pdf/com/en/1495621.pdf

You can use the following commands within the virtual machine (VM) console to configure and troubleshoot Internet & SaaS Virtual Service Edges. By default, root login is not permitted, so admins must use the `sudo` utility to run a command with higher privileges.
...
sudo cd /home/zsroot        sudo vzen stop       sudo vzen cleanup
...
An admin can request remote assistance and allow Zscaler Support to log in to their Virtual Service Edge instance without opening a firewall connection for inbound traffic. This feature is disabled by default and must be enabled explicitly for the duration that remote support assistance is required. You can enable this feature from the Admin Portal. To learn more, see [Adding Internet & SaaS Virtual Service Edge Instances](/unified/adding-internet-saas-virtual-service-edge-instances#zs-support-tunnel).
