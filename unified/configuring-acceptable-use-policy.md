...
PDF: https://help.zscaler.com/pdf/com/en/1401136.pdf
...
The service tracks the AUP acceptance time and expiration for each user. The Zscaler service sets an [AUP cookie](/zia/about-zscaler-cookies) when a user accepts the AUP. For example, if you select **Weekly**, the service displays the AUP page one week from when the AUP cookie was set. If the AUP cookie is not present, the service displays the AUP page the next time the user logs in.
- **AUP Message (HTML)**: Enter the *Acceptable Use* statement. You can use HTML and [CSS styles](/zia/customizing-euns-css-styles) to customize the AUP notification message and its appearance. You can also add image files that are hosted on the internet.
...
This information also appears on the [block](/zia/configuring-block-notifications), [caution](/zia/configuring-caution-notification), and [quarantine](/zia/configuring-quarantine-notification) notifications.
...
[]![The logs generated for Acceptable Use Policy (AUP) action taken by users](/downloads/zia/authentication-administration/end-user-notifications/configuring-acceptable-use-policy/aup-logs.png)