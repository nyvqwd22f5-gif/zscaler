...
Source: https://help.zscaler.com/unified/network-connector-deployment-guide-linux
PDF: https://help.zscaler.com/pdf/com/en/1532388.pdf
...
After Network Connectors are deployed, you can [create network segments](/unified/configuring-network-segments) to pass application traffic. For example, to test your configuration, consider setting up a simple application segment that allows SSH access to your Network Connectors.
[]
...
To learn more, see the [Network Connector Deployment Guide for Linux](/unified/network-connector-deployment-guide-linux).
...
After a Network Connector is enrolled, an outbound TLS tunnel over port 443 is established to the Private Applications cloud infrastructure. Outbound VPN tunnels establish a gateway to VPN Service Edge using a port range from 51820 to 53000. This communication channel provides various functionality and utilizes minimal bandwidth, which includes traffic: for Network Connector software upgrades (upgrades are completed based on a weekly schedule).
...
[]Before you begin any procedures within the [Network Connector Deployment Guide for Linux](/unified/network-connector-deployment-prerequisites), make sure that you have met all the following prerequisites:
-
...
-
A Network Connector [provisioning key](/unified/about-network-connector-provisioning-keys) obtained from the Admin Portal
-
...
- Gateway IP address is available in the Admin Portal after you instantiate a gateway instance in VPN Service Edges. To learn more, see [About VPN Service Edges](/unified/about-vpn-service-edges).
...
Because the service enforces TLS certificate pinning for both client and server certificates, all forms of inline or man-in-the-middle TLS interception or inspection must be disabled. Network Connectors do not function if the TLS certificates presented by the Public Service Edges or Private Service Edges for Private Applications do not cryptographically verify against Zscaler-trusted public keys.
By design, certificate verification is not configurable in order to maintain the integrity of the service. So ensure that *.prod.zpath.net is in your SSL bypass list for traffic originating from the Network Connector. This is necessary for allowing the Network Connector to resolve and reach Public Service Edges or Private Service Edges for Private Applications. If you need to allowlist additional Zscaler IP addresses, see [config.zscaler.com/private.zscaler.com/zpa](https://config.zscaler.com/private.zscaler.com/zpa) (for the private.zscaler.com cloud) or [config.zscaler.com/zpatwo.net/zpa](https://config.zscaler.com/zpatwo.net/zpa) (for the zpatwo.net cloud).
Firewall Requirements and Interoperability Guidelines
...
- RPM package ([np-connector.rpm](https://yum.private.zscaler.com/yum/el9/np-connector-25.48.4-1.el9.x86_64.rpm))
...
$ scp np-connector-25.48.4-1.el9.x86_64.rpm admin@<Network Connector Hostname or IP Address>
...
After the Network Connector provisioning key is applied, be sure to verify that the deployed Network Connector is [running and healthy](/unified/viewing-network-connectors-dashboard).
- Zscaler highly recommends updating the Network Connector system software before proceeding. To learn more, see [Configuring Network Connectors](/unified/configuring-network-connectors).
- []Complete the following steps if you need to enable SSH:
- Log in to the Network Connector console using your [admin credentials](/unified/managing-deployed-app-connectors#Default).
...
[admin@np-connector ~]$ sudo vi /opt/zscaler/var/provision_key[admin@np-connector ~]$ sudo cp provision_key
...
In the Admin Portal, the Network Connector appears as “up” and “connected”.
