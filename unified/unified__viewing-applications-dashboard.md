...
Source: https://help.zscaler.com/unified/viewing-applications-dashboard
PDF: https://help.zscaler.com/pdf/com/en/1498031.pdf

The Applications dashboard (Analytics > Reports > Private Applications > Applications) provides information about applications in your organization.
[See image.](#img_applicationsdbrd)
Dashboard Tools
...
If you are using the [Log Streaming Service (LSS)](/unified/about-log-streaming-service), the Users dashboard includes information for a Private Applications LSS Client user. This user represents the LSS service, not an actual user. Also, each log receiver is displayed as an application to reflect the data coming in from the service. To learn more, including how to stop the LSS service from streaming Private Applications LSS Client logs, see [Configuring a Log Receiver](/unified/configuring-log-receiver#Step2).
...
[]This widget displays the top errors experienced by users per connection status code, application, App Connector, and Private Service Edges over the selected time frame.
...
- Click on an application to view more details in **Diagnostics**.
[]This widget displays the applications that Private Applications have [discovered](/unified/about-application-discovery) for your organization in the past 14 days, with the most recently discovered application listed first.
...
- To define a user access policy or change settings for an application (e.g., enable health monitoring or configure bypass settings), you can explicitly define an application by clicking on **Add Application Segment**, selecting the applications, and then clicking **Define Selected Applications**. To learn more, see [Defining a Dynamically Discovered Application](/unified/defining-dynamically-discovered-application).
To learn more about application discovery, see [About Application Discovery](/unified/about-application-discovery).
...
[]![Applications dashboard in the ZPA Admin Portal](/downloads/unified/analytics/private-applications/application-user-and-health-monitoring/viewing-applications-dashboard/unified-privateapp-applicationsdashboard.png)