...
Source: https://help.zscaler.com/unified/configuring-ssl-inspection-policy
PDF: https://help.zscaler.com/pdf/com/en/1493756.pdf

You can configure Secure Sockets Layer (SSL) Inspection policies to perform scanning of the SSL traffic based on the source and destination of the traffic. Using these policies, you can simplify the deployment and ongoing operations of SSL Inspection and address the compliance and operational environmental requirements. To learn more, see [About SSL Inspection Policy](/unified/about-ssl-inspection-policy).
Policy Execution
...
Configuring SSL Inspection Rules
...
- **Source IP Groups**:** **Select any number of [source IP groups](/unified/about-source-ip-groups). You can also search for source IP groups or click the **Add** icon to add a new [source IP group](/unified/configuring-source-ip-groups). Selecting no value ignores the criterion in the policy evaluation.
- **URL Categories**: Select any number of [URL super categories or categories](/unified/about-url-categories). Selecting no value ignores the criterion in the policy evaluation.
- **Cloud Applications**: Select any number of [cloud applications or cloud application classes](/unified/about-cloud-app-control). Selecting no value ignores the criterion in the policy evaluation.
-
**Destination Groups**: Select any number of [destination groups](/unified/about-destination-ip-groups). Selecting no value ignores the criterion in the policy evaluation. The supported group types are IP, FQDN, and Wildcard FQDN. The countries and custom categories configured in the destination groups are ignored.
...
- **Forwarding Gateways**: Select any number of [gateways](/unified/about-gateways-proxies) configured for third-party proxies. Selecting no value ignores the criterion in the policy evaluation.
-
**ZPA Application Segment**:** **Select up to 255 Private Applications' [application segments](/unified/configuring-defined-application-segments). Selecting no value ignores the criterion in the policy evaluation.
The list displays only those application segments that have the **Source IP Anchor** option enabled.
- **Locations**: Select up to 32 [locations](/unified/about-locations). Selecting no value ignores the criterion in the policy evaluation.
- **Location Groups**: Select up to 32 [location groups](/unified/about-location-groups). Selecting no value ignores the criterion in the policy evaluation.
- **Users**: Select up to 32 general and/or special users. Select **General Users** for all authenticated users and **Special User**s for all [unauthenticated users](/unified/configuring-policies-unauthenticated-traffic). Selecting no value ignores the criterion in the policy evaluation.
- **Groups**: Select up to 32 [groups](/unified/about-groups). Selecting no value ignores the criterion in the policy evaluation.
-
**Departments**: Select up to 32 departments. If you've enabled the [unauthenticated users policy](/unified/configuring-policies-unauthenticated-traffic), you can select **Special Departments** for unauthenticated traffic. Selecting no value ignores the criterion in the policy evaluation.
...
-
**Device Groups**: Select the [device group](/unified/about-device-groups) for which you want to apply the rule. For Zscaler Client Connector traffic, select the appropriate group based on the device platform. Select **Cloud Browser Isolation**, **IoT**, or **No Client Connector** to apply the rule to Isolation traffic, IoT traffic, or to traffic that is not tunneled through Zscaler Client Connector, respectively. Selecting no value ignores the criterion in the policy evaluation. Zscaler provides the [Unauthorized Traffic Bypass for IoT Classifications](/unified/about-ssl-inspection-policy#unauthorized-traffic) predefined rule for the SSL Inspection policy.
...
The trust levels assigned to the devices are based on your [posture configurations](/unified/adding-internet-saas-posture-profiles).
...
- **Minimum TLS Version**: Select the minimum TLS version required for your connections. The service blocks connections that do not meet the specified minimum TLS version requirement.
...
[]![New SSL Inspection Rule Configuration](/downloads/zia/policies/ssl-inspection/configuring-ssl-inspection-policy/SSL%20Inspection%201_0.png)
