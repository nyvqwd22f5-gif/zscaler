...
Source: https://help.zscaler.com/unified/troubleshooting-app-connectors
PDF: https://help.zscaler.com/pdf/com/en/1496216.pdf
...
- [Frequent Disconnections to the Cloud](#frequentclouddisconnect)
...
You can likely see this error if an App Connector is deleted from the Admin Portal accidentally. Private Applications immediately recognizes that it should not communicate with the App Connector, and it breaks the active connection. There is no way to recover the App Connector.
...
If you attempt to delete the App Connector and the App Connector still reports an ID number, then the App Connector was not fully deleted.
- Restart the process for [configuring](/unified/configuring-app-connectors) an App Connector and [deploying](/unified/about-deploying-app-connectors) on a platform.
[]If you have a DNS failure and the App Connector “root” process is successful, you might see an error similar to the following:
The domain (e.g., api.private.com) in the echo statement will depend on what Private Applications cloud you are on.
...
The DNS failure may be an issue with the permissions for the user account “zscaler”. Typically this means the “root” process is able to read `/etc/resolv.conf`, but the “zscaler” user account can not.
...
[]If you have an error similar to the following, a device between the App Connector and cloud service is attempting a SSL interception:
...
Private Applications do not support SSL interceptions. Private Applications use TLS connections and pinned certificates, so a device performing SSL interception causes the connection to fail between the App Connector and the Zscaler cloud.
To make sure traffic is not SSL intercepted, you need to allowlist all the Private Applications domains or IP addresses in the SSL interception device that are listed on [config.zscaler.com/private.zscaler.com/zpa](https://config.zscaler.com/private.zscaler.com/zpa) (for the private.zscaler.com cloud) or [config.zscaler.com/zpatwo.net/zpa](https://config.zscaler.com/zpatwo.net/zpa) (for the zpatwo.net cloud). This ensures App Connectors and the Zscaler URLs (e.g., *.zpath.net, *.zscaler.com) are bypassed from a SSL inspection.
[]You should routinely monitor and address memory leaks or high memory usage in your App Connectors. For more information, see [Monitoring App Connector Performance](/unified/monitoring-app-connector-performance).
If there is a need to contact Zscaler Support, be prepared to run the following script. It collects a memory report every minute. This may need to run for at least one hour or up to multiple days.
Script:
#!/bin/bash
...
[]You should routinely monitor and address high CPU for your App Connectors. For more information, see [Monitoring App Connector Performance](/unified/monitoring-app-connector-performance).
If there is a need to contact Zscaler Support, be prepared to run the following script. It collects a CPU report. This may need to run for at least one hour or up to multiple days.
Script:
#!/bin/bash
# trap ctrl-c and call ctrl_c()
trap ctrl_c INT
...
[]If you have issues with the App Connector connecting to the cloud, the problem may come from one of a couple areas.
There are a few things to check when narrowing down what is preventing the App Connector to connect:
- [Check the IP ranges.](#cloudconnect_iprange)
...
[]Make sure all the IP ranges allowed by the firewall match those in the firewall policies: [config.zscaler.com/private.zscaler.com/zpa](https://config.zscaler.com/private.zscaler.com/zpa) (for the private.zscaler.com cloud) or [config.zscaler.com/zpatwo.net/zpa](https://config.zscaler.com/zpatwo.net/zpa) (for the zpatwo.net cloud).
[]Test connectivity to the upstream Public Service Edge by querying the DNS address `co2br.prod.zpath.net` from the server that is running the App Connector software.
...
[]Test if the App Connector can do TLS connection to a Public Service Edge using the `openssl` command.
...
[admin@localhost ~]$
If the `openssl` command shows an error, then there is a problem with your network. You can try running the openssl command in a loop to see if there is a regular pattern to the success/failure rate. For example, the firewall rule may not allow the App Connector to open a connection to a Public Service Edge. Sometimes TCP goes through, but TLS fails. This is a sign that the firewall is blocking the application layer, but it’s allowing the transport layer.
[]Get the tcpdump and see the delay between the packet sent and the TCP RST received. If the delay is too less (e.g., 1 or 2 milliseconds), then most likely the firewall in your organization's premise is sending the RST.
[]This error sometimes occurs intermittently, which indicates a networking issue either in your organization’s environment or in the internet path from the App Connector to Public Service Edge. In those cases, do the following:
...
- The [diagnostic information](/unified/accessing-app-connector-status-diagnostics) about App Connectors in the Admin Portal
- The App Connectors’ log information in either the [Live Logs](/unified/about-live-logs) on the Admin Portal or through the [Log Streaming Service](/unified/about-log-streaming-service)
If the Central Authority can not determine an application or resolve the connection to the App Connector for the user, it displays the App Connector ID as zero. This can occur for the following session status codes:
- APP_NOT_REACHABLE.
- INVALID_DOMAIN
- NO_CONNECTOR_AVAILABLE
To learn more, see [Understanding Session Status Codes](/unified/understanding-session-status-codes).
[]This is expected behavior. Each application requires a TCP port and has its health monitored. Many of these ports can reside in a TIME_WAIT state. To learn more, see [Monitoring App Connector Performance](/unified/monitoring-app-connector-performance).
...
[]If you see frequent disconnections between an App Connector to the cloud, the problem may come from one of a couple areas.
...
- Go to the **Diagnostics **page (Logs > Insights > Private Applications > Diagnostics). For more information, see [About App Connector Status Diagnostics](/unified/accessing-app-connector-status-diagnostics).
- Choose **App Connector Status** under **Log Type**.
...
[]Sometimes the load (i.e., the interface stats) is not equally split between the App Connectors in the App Connector group. Private Applications maintains App Connector affinity for an active user for 30 minutes following the user’s last application request. This means that once an App Connector is selected for a user, all application requests for that user go through this App Connector.
You can check the [Health Dashboard](/unified/about-health-dashboard) to help you address your questions about App Connector load balance and to see the application split between App Connectors in a given period of time.
...
For [remote troubleshooting](/zpa/about-support-information#Status), App Connectors may reject additional session commands when they are currently in a paused state. The following command is returned in the App Connector console when they are currently in a paused state and reject a session command:
...
- [Image can not download since there is no disk space left.](#upgradefailure_nodiskspace)
- [Image can not download due to inconsistent connection between the App Connector and co2br (App Connector to Public Service Edge endpoint).](#upgradefailure_inconsistentconnection)
- [The Provisioning Key was deleted in the Admin Portal.](#upgradefailure_provisioningkey)
...
- Restart the App Connector. To learn more, see [Start, Stop, or Restart an App Connector](/unified/managing-deployed-app-connectors#Starting).
...
- Return to the [App Connectors page](/unified/about-app-connectors#AboutConnectorsPage) (Infrastructure > Private Access Component > App Connectors) to see if the Update Status changes from Failure to Success. If the status still says Failure, proceed to the next step.
...
[admin@localhost ~]$
-
Check if the App Connector can start a TLS connection using the openssl command. You should receive a certificate subject string returned from the Public Service Edges. For example:
[admin@localhost ~]$ openssl s_client -servername mockcompany.com.server1.net -connect 13.60.119.37:443 2>&1 | grep subject
subject=/C=US/ST=California/L=San Jose/O=Zscaler/OU=Emerging Technologies/CN=broker1a.sjc8.prod.zpath.net
[admin@localhost ~]$
...
If the software downloads, return to the [App Connector page](/unified/about-app-connectors) (Infrastructure > Private Access Component > App Connectors) to see if the Update Status changes from **Failure** to **Scheduled**.
...
- On the Connector Provisioning Key page (Infrastructure > Private Access Component > App Connector Keys), verify the number of provisioning keys in the **# of Enrolled Connectors** column is less than the number in the **Maximum # of Connectors** column for the provisioning key.
If needed, you can raise the maximum enrollment count. To learn more, see [Editing App Connector Provisioning Keys](/unified/editing-app-connector-provisioning-keys).
- Stop the App Connector. To learn more, see [Start, Stop, or Restart an App Connector](/zpa/managing-deployed-connectors#Starting).
-
...
- Copy the provisioning key from the Admin Portal and use an editor to paste it into the file. To learn more, see [About the App Connector Provisioning Keys Page](/unified/about-app-connector-provisioning-keys).
...
- Delete any extra directories, except `/opt/zscaler`, to free up space. Once disk space is available, the image downloads to `opt/zscaler/var/image.bin`.
- []Verify the App Connector has a stable connection to Public Service Edge.
-
Run the following command:
journalctl -n1000 | grep zscaler-update
- []Go to the [App Connector page](/unified/about-app-connectors#AboutConnectorsPage) (Infrastructure > Private Access > Component > App Connectors) and identify the App Connector’s group.
- Go to the [Provisioning key page](/unified/about-app-connector-provisioning-keys) (Infrastructure > Private Access > Component > App Connector Keys) and find the App Connector group. If the group is not listed in the App Connector group column, the key is no longer in the Admin Portal.
...
[]You may experience issues where you can not enroll a new App Connector in the Admin Portal, but you have less than 100 active App Connectors.
Within the Admin Portal, you can have up to 100 App Connectors. This total includes both App Connectors enrolled that are active and App Connectors that are disabled, but not deleted. If you need to add more App Connectors, [delete](/unified/about-app-connectors#AboutConnectorsPage) the disabled App Connectors and try enrolling your new App Connectors. To learn more, see [Configuring App Connectors](/unified/configuring-app-connectors).
...
The App Connector enrollment process involves the creation of certificates. If the App Connector’s timestamp does not match the timestamp on the Private Application backend servers, there can be a delay before the certificate becomes valid and enrolls the App Connector.
To resolve this issue, verify that the time on the App Connector is properly set and maintained. To learn more, see [Configure Network Time Protocol (NTP) Servers for an App Connector](/unified/app-connector-deployment-guide-amazon-web-services#NTP).
[]If the disk space utilization of the App Connector exceeds 90%, the [configuration and binary snapshots](/unified/managing-disaster-recovery-configuration-and-binary-snapshots) used for disaster recovery can no longer be captured until the low disk space condition is resolved.
