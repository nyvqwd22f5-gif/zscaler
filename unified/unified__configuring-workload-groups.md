...
Source: https://help.zscaler.com/unified/configuring-workload-groups
PDF: https://help.zscaler.com/pdf/com/en/1496426.pdf

You can configure workload groups to apply security policies to the workloads deployed on the Amazon Web Services (AWS) platform. You can view all the workloads and their respective tags in Zscaler Cloud & Branch Connector. A maximum of 8 key-value pairs can be used to build the expression for the workload group. To learn more, see [About Workload Groups](/unified/about-workload-groups).
Only admins with full policy access can add or edit the workload groups.
To configure a workload group:
- Go to **Policies **>** Common Configuration** > **Resources** > **Workload Groups**.
-
Click **Add Workload Group**.
The **Add Workload Group** window appears.
...
For the **Attribute** tag type, you cannot add tag keys manually. The **Add Tag** field populates a set of [predefined tag keys](#predefined-tag-keys). To learn more, see [Understanding Workload Groups](/unified/understanding-workload-groups).
