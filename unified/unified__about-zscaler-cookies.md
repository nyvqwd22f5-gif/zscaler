...
Source: https://help.zscaler.com/unified/about-zscaler-cookies
PDF: https://help.zscaler.com/pdf/com/en/1488331.pdf
...
- **Domain cookie**: After a user logs in to the Zscaler service, the service sets an additional cookie for each domain to which a user browses. This enables the service to identify which domains a user has visited, so it won’t require the user to log in again. This cookie is set by the Internet & SaaS Public Service Edge.
- **Acceptable Usage Policy (AUP) cookie**: The Zscaler service sets this cookie when a user accepts the AUP. This cookie is set by the Internet & SaaS Public Service Edge.
The service needs to authenticate users only once to set the gateway cookie. However, you can require users to [authenticate more often](/unified/about-authentication-default-settings) based on your organization's needs.
