# Adding a Ruleset
Source: https://help.zscaler.com/unified/adding-ruleset
PDF: https://help.zscaler.com/pdf/com/en/1533785.pdf
...
To learn more, see [About Location-Based Policies](/unified/about-location-based-policies).
...
- [Lists of IP addresses to bypass or tunnel traffic](/unified/adding-traffic-steering-ip-list) to the Zscaler Internet Access (ZIA) service.
- [Lists of domains](/unified/adding-domain-list) for which to bypass or tunnel DNS queries to the ZIA service.
- If you want to include endpoint firewall rules in the ruleset:
- [Applications](/unified/adding-application-ruleset) to include in inbound or outbound endpoint firewall rules.
- [Lists of IP addresses](/unified/adding-firewall-ip-list) to include in inbound or outbound endpoint firewall rules.
...
- Click **Save**.
After you add a ruleset, you can add it to an app profile on the [Location Policies](/unified/configuring-zscaler-client-connector-app-profiles#location-policies) tab and apply it to one or more network types.
...
- **Domain Inclusion**: Select a domain list from the [Domain Lists](/unified/adding-domain-list) tab for which Zscaler Client Connector should send DNS requests to the Internet & SaaS Public Service Edge.
- **Domain Exclusion**: Select a domain list from the [Domain Lists](/unified/adding-domain-list) tab for which Zscaler Client Connector should send DNS requests to the DNS server defined on the device (and not to the Internet & SaaS Public Service Edge).
- **Prioritize DNS Exclusions over Z-Tunnel 2.0**: When enabled, Zscaler Client Connector prioritizes DNS domains in Domain Exclusions over Domain Inclusions. To learn more, see the [How Zscaler Client Connector processes the domain inclusions and exclusions](/unified/configuring-zscaler-client-connector-app-profiles#DNS_IP_in_Inclusion) section.
...
- **Process-Based Application Bypass**: Allows bypassing of [process-based applications](/unified/adding-process-based-applications-bypass-traffic) that you can use for both Zscaler Tunnel (Z-Tunnel) 1.0 and Z-Tunnel 2.0. Click **Select All** or select applications from the drop-down menu.
- **Predefined IP-Based Application Bypass**: Allows bypassing of [predefined IP-based applications](/unified/about-application-bypass). Click **Select All** or select applications from the drop-down menu.
- **Custom IP-Based Application Bypass**: Allows bypassing of [custom IP-based applications](/unified/adding-ip-based-applications-bypass-traffic) that you can use for Z-Tunnel 2.0. Click **Select All** or select applications from the drop-down menu.
- **IP Exclusion List**: Select a list of IP addresses from the [Traffic Steering IP Lists](/unified/adding-traffic-steering-ip-list) tab to exclude, or keep the default value of `Default IP Exclusion List` to use the default exclusion list on the tab.
- **IP Inclusion List**: Select a list of IP addresses from the [Traffic Steering IP Lists](/unified/adding-traffic-steering-ip-list) tab to include, or keep the default value of `Default IP Inclusion List` to use the default inclusion list on the tab.
-
...
- **<IP List name>**: Apply to the selected Firewall IP list from the [Firewall IP Lists](/unified/adding-firewall-ip-list) tab.
- **Application**: Select the application from the [Applications](/unified/adding-application-ruleset) tab that the inbound firewall rule applies to, or select **Any **to apply it to all applications.
...
- **Allow ZPA Client to Client Traffic**: Select this option to allow inbound access for Private Applications client-to-client traffic (e.g., client-based remote assistance) even if all other inbound traffic is blocked.
...
- **Local subnet**: Apply to the subnet range into which the device falls. The local subnet is auto-detected and Zscaler Client Connector creates both IPv4 (multicast address) and IPv6 (link-local IP and multicast address) filters based on the subnet.
- **<IP List name>**: Apply to the selected Firewall IP list from the [Firewall IP Lists](/unified/adding-firewall-ip-list) tab.
- **Application**: Select the application from the [Applications](/unified/adding-application-ruleset) tab that the outbound firewall rule applies to or select **Any **to apply it to all applications.
- **Action**: Select the action for the rule:
