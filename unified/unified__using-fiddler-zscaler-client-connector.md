...
Source: https://help.zscaler.com/unified/using-fiddler-zscaler-client-connector
PDF: https://help.zscaler.com/pdf/com/en/1495711.pdf
...
To learn more, see [Best Practices for Using PAC Files with Zscaler Client Connector](/unified/best-practices-using-pac-files-zscaler-client-connector).
...
[]To learn more about creating a custom PAC file, see [Writing a PAC File](/unified/writing-pac-file).
...
You can host the PAC file in the Admin Portal. To learn more, see [Using Custom PAC Files to Forward Traffic to Internet & SaaS](/unified/using-custom-pac-files-forward-traffic-internet-saas).
...
[][Create a new forwarding profile](/unified/configuring-forwarding-profiles-zscaler-client-connector) or update an existing one, and then add the custom PAC file.
...
[][Create a Zscaler Client Connector profile](/unified/configuring-zscaler-client-connector-app-profiles) or update an existing one, and then add the configured forwarding profile.
...
You must add [the port that Zscaler Client Connector listens on](/unified/configuring-port-zscaler-client-connector-listen) to the proxy string. For example, if Zscaler Client Connector listens on port 9000, then the proxy string is the following:
