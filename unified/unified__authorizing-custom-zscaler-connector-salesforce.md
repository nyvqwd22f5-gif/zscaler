...
Source: https://help.zscaler.com/unified/authorizing-custom-zscaler-connector-salesforce
PDF: https://help.zscaler.com/pdf/com/en/1530714.pdf
...
When you create a custom connector for Salesforce for SSPM Scan, you must provide the client ID, client secret, integration user's username, and instance URL in the Admin Portal so that the Zscaler service can access the application. For DLP and Malware scanning SaaS API onboarding, you additionally must include the Malware and DLP quarantine library names.
...
[]The custom onboarding process for Salesforce requires uploading your private key file to the Admin Portal. Zscaler recommends generating a certificate authority-signed (CA-signed) certificate which can be a more authoritative way to prove that your organization’s data communications are genuine. If you are unable to obtain the CA-signed certificate, a self-signed certificate can also be used. To learn more, refer to the [Salesforce documentation](https://help.salesforce.com/s/articleView?id=xcloud.security_keys_uploading_signed_cert.htm&type=5).
...
- In the next window, copy your **Consumer Key**, and the **Consumer Secret**. In the Admin Portal these are used for your **Client ID** and **Client Secret**.
...
- In the Admin Portal, click **Save** and activate the change.
