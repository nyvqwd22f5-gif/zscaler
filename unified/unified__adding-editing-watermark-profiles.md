# Adding & Editing Watermark Profiles
Source: https://help.zscaler.com/unified/adding-editing-watermark-profiles
PDF: https://help.zscaler.com/pdf/com/en/1508286.pdf

You can use watermark profiles as part of your SaaS Security Data at Rest Scanning Data Loss Prevention (DLP) policy to protect files that trigger policy rules. When sensitive content triggers a SaaS Security Data at Rest Scanning DLP rule, the Zscaler service uses the watermark profile associated with the policy rule to superimpose text diagonally across pages and images to clearly identify the content you want to protect. Additionally, you can include headers and footers as part of your watermark profiles.
[]Adding a Watermark Profile
- Go to the** Policies **>** Data Protection **>** Policy **>** Rights Management **>** Watermarking **page.
-
Click **Add Profile**.
The **Add Watermark Profile** window opens.
[See image.](#add-watermark-profile-dialog)
-
Specify settings for the watermark profile:
- **Profile Name**: Enter a name of up to 128 characters for the watermark profile.
- **Header Text**: Enter up to 50 characters for the watermark header.
- **Footer Text**: Enter up to 50 characters for the watermark footer.
- **Watermark Text**: Enter up to 15 characters for the watermark itself.
The font style (i.e., color, size, style, etc.) is applied automatically and can't be configured.
- Click **Save** and [activate the change](/unified/saving-and-activating-changes-admin-portal).
[]Editing a Watermark Profile
- Go to the** Policies **>** Data Protection **>** Policy **>** Rights Management **>** Watermarking **page.
-
Locate the watermark profile in the list, then click the **Edit** icon.
The **Edit Watermark Profile** window opens.
[See image.](#edit-watermark-profile-dialog)
-
Specify settings for the watermark profile:
- **Profile Name**: Enter a name of up to 128 characters for the watermark profile.
- **Header Text**: Enter up to 50 characters for the watermark header.
- **Footer Text**: Enter up to 50 characters for the watermark footer.
- **Watermark Text**: Enter up to 15 characters for the watermark itself.
The font style (i.e., color, size, style, etc.) is applied automatically and can't be configured.
- Click **Save** and [activate the change](/unified/saving-and-activating-changes-admin-portal).
[]![Screenshot of the Add Watermark Profile dialog box in the ZIA Admin Portal](/downloads/zia/policies/saas-security-api/rights-management/ZIA-DLP-Rights-Management-Add-Watermark-Profile.png)
[]![Screenshot of the Edit Watermark Profile dialog box in the ZIA Admin Portal](/downloads/zia/policies/saas-security-api/rights-management/ZIA-DLP-Rights-Management-Edit-Watermark-Profile.png)