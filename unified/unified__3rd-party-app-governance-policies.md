...
Source: https://help.zscaler.com/unified/3rd-party-app-governance-policies
PDF: https://help.zscaler.com/pdf/com/en/1516121.pdf
...
There are two ways to create a new policy. You can create a policy from the Inventory page or the Policies page. You must first define the rule or the view for which you want to apply the policy. To learn more, see [C](/unified/custom-views)[ustom Views](/unified/custom-views).
...
[See image.](#My-Views)
You should consider adding the App Status filter when creating custom views to be used in policies, so if an app is reinstalled, the policy will be triggered again.
-
Click **Set Policy**.
...
- **Policy enabled**: Select whether this policy should be enabled or disabled.
...
-
**Slack**: Notifications are sent to the selected Slack channel. You must configure Slack for receiving alerts. To learn more, see Adding Outbound Integrations.
...
- **Classification**: Select the desired classification value for the apps. To learn more, see [Classifying Apps](/unified/classifying-apps).
- **Automated workflow (for new apps matching policy)**:** **Select the desired automated workflow.** **Automated workflow is supported on Google and partially on Microsoft and is dependent on your license. To learn more, see [Configuring End User Review](/unified/configuring-end-user-review) and [Revoking and Banning Apps](/unified/revoking-and-banning-apps).
...
The** Set Policy** window appears.
- In the **Set Policy** window, enter the relevant data as required. To learn more, see the Set Policy window configuration in[ Creating a Policy from the Inventory Page](#Policy-Inventory-Page).
- After you define the policy, click **Save Policy** to save it.
...
- **Edit Policy Window**: Click on the policy name. The **Edit Policy **window appears, and you can make changes to the policy as necessary.
...
[]![Screenshot of Set Policy Window](/downloads/tech-pubs-drafts/zia-draft-articles/apptotal-policies-draft/Set_Policy.png)
[]![Screenshot of Create Policy Option](/downloads/zia/policies/data-loss-prevention/apptotal/using-apptotal/apptotal-policies/Create_Policy.png)