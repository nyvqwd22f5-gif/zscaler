...
Source: https://help.zscaler.com/unified/about-threat-categories
PDF: https://help.zscaler.com/pdf/com/en/1494216.pdf

Threat categories group common threats (e.g., viruses, botnets, exploits) together. Zscaler's Intrusion Prevention System (IPS) uses signature-based detection to identify and control these threats. Zscaler provides a number of predefined threat categories, each associated with a collection of [custom IPS signature rules](/unified/about-custom-ips-signature-rules), which are like fingerprints of known network attack patterns. In addition to the predefined threat categories, you can define custom threat categories with custom signature rules that are specific to your organization’s requirements. Using these threat categories, you can define [IPS Control](/unified/about-ips-control) rules to monitor and protect your network traffic against malicious activities.
...
- Add a [custom threat category](/unified/adding-threat-categories).
...
- Edit a threat category. This action also allows you to delete a threat category.
- [Modify the table and its columns](/unified/using-tables).
- Search for a threat category.
- Go to the [Custom Signature Rules](/unified/about-custom-ips-signature-rules) page to add and manage custom signature rules.
