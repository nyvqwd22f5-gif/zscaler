...
Source: https://help.zscaler.com/unified/adding-saml-identity-providers
PDF: https://help.zscaler.com/pdf/com/en/1505456.pdf
...
-
[]On the **Basic **tab, configure the following:
...
-
On the **Advanced **tab:
- **Enable SAML Request Signing**:** **Enable the option to configure SAML request signing for authentication. The following fields appear:
- **Signing Algorithm**: Select the signing algorithm.
- **SP SAML Certificate**: Download the SP SAML certificate. You need to upload this certificate to the IdP.
- **Enable Encrypted SAML Assertion**:** **Enable the option to configure encrypted SAML response for authentication. When this option is enabled, the **SAML Encryption Certificate **field appears with a button to download the SAML encryption certificate. You need to upload this certificate to the IdP.
...
-
If you are configuring Zscaler Identity Proxy for Cloud Apps, you must map the `Is Device Managed` session attribute with the appropriate IdP attribute, and then configure the managed devices settings in the Admin Portal. To learn more, see [Configuring the Zscaler Identity Proxy for Cloud Apps](/unified/configuring-zscaler-identity-proxy-cloud-apps).
[See image.](#manged-device-mapping)
-
On the **Provisioning **tab:
- **Enable Just-in-Time (JIT) Provisioning**: Enable this option to allow automated user provisioning to the ZIdentity service when the user tries to access ZIdentity for the first time. The IdP sends a SAML Assertion (XML file) containing user information, which the ZIdentity service uses to create a [user database](/unified/about-users-0) with ZIdentity in real time. If you disable this option, unprovisioned users are denied access to the ZIdentity service until the [users are configured](/unified/adding-users-0) in the Admin Portal .
...
[]![Advanced Tab in Add Identity Provider Window](/downloads/zslogin/authentication/external-idp-configuration/adding-openid-providers/zidentity-idp-advanced-tab_0.png)
