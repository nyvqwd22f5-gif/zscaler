...
Source: https://help.zscaler.com/unified/configuring-patient-0-alert
PDF: https://help.zscaler.com/pdf/com/en/1494361.pdf

You must have Advanced Sandbox to configure patient 0 alerts.
If you've configured the [first-time action](/unified/configuring-sandbox-policy#first-time-action) of a [Sandbox rule](/zia/configuring-sandbox-policy) to allow and scan unknown files, the Zscaler service:
- Allows users to download files that match the rule criteria.
- Sends the files to the Sandbox for behavioral analysis.
A patient 0 event occurs when a user downloads an unknown file that is scanned and found to be malicious. On the [Alerts page](/unified/about-alerts), you can add the patient 0 alert and receive emails about these events within approximately two hours.
[]Configuring the Patient 0 Alert
...
On the [Security dashboard](/unified/about-dashboards#security), the **Sandbox Patient 0 Events** widget displays the patient 0 events that occurred in your organization.
...
- [Sandbox Detail Report](/unified/viewing-sandbox-reports-and-data#about-sandbox-detail-report)
- [CrowdStrike Endpoint Hits report](/unified/viewing-crowdstrike-endpoint-hits-report) (requires a [CrowdStrike integration](/unified/integrating-crowdstrike))
...
- **Allowed Transactions**: The total number of allowed transactions that occurred with the malicious file. Click to view the transaction logs on the [Web Insights Logs](/unified/about-insights-logs) page.
...
- Go to **Administration **>** Alerts **>** Internet & SaaS **>** Alerts**.
-
