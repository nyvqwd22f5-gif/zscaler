...
- [Log in to the App Connector console](#LoginPassword) using your admin credentials.
-
...
To learn more, see [App Connector Specifications and Sizing Requirements](/zsdk/app-connector-deployment-prerequisites).
...
- [Log in to the App Connector console](#LoginPassword) using the default admin username and password.
-
...
- Log in to the system hosting the App Connector package via Secure Shell (SSH) or log in to the local App Connector console using your admin credentials.
-
...
-
Using the App Connectors page and Health dashboard: You can check the status by accessing the [App Connectors page](/zsdk/about-app-connectors) and using the [Health dashboard](/zpa/about-dashboard/health) within the ZSDK Admin Portal.
To check the App Connector's status using the ZSDK Admin Portal:
- Go to **Configuration & Control** > **Private Infrastructure** >** App Connector Management** > **App Connectors**. Check that the App Connector you deployed appears in the table of configured App Connectors.
-
Go to **Dashboard** > **Health**. Check that the App Connector you deployed appears in the **App Connectors** widget. The App Connector must have enrolled successfully at least once for it to appear within the Health dashboard.
...
![App Connectors page within the ZPA Admin Portal](/downloads/zsdk/applications/app-connectors/maintaining-deployed-app-connectors/ZSDK-App-Connectors-Selection.png)
-
In **Dashboards** > **Health**, within the **App Connectors** widget:
![Health Dashboard with App Connectors widget displayed within the ZPA Admin Portal](/downloads/zsdk/applications/app-connectors/maintaining-deployed-app-connectors/ZSDK-App-Connectors-Health.png)
-
...
After completing the update process, restart the App Connector using the following command:
`[admin@zpa-connector ~]$ sudo systemctl restart zpa-connector      `
- []Download one of the following `.rpm` files:
...
-
Use the scp command to copy the `.rpm` file to the App Connector. For example:
`$ scp <RPM Version>`
- Log in to the App Connector console using your admin credentials.
-
Update the App Connector software package using the following command:
...
- Log in to the ZPA Admin Portal and go to **Configuration & Control** > **Private Infrastructure** >** App Connector Management** > **App Connectors**.
- Locate the App Connector that you want to replace the provisioning key for and click the **Delete **icon.
- In the confirmation window that appears, click **Delete**.
- (Optional) If you do not already have your new App Connector provisioning key, be sure to complete the procedure for [adding a new App Connector with a new provisioning key](/zpa/about-connectorprovisioningwizard).
- Log in to the App Connector console using your admin credentials.
- Temporarily enable sshd:
- For Amazon Web Services (AWS), update your security group to allow inbound connections from port 22. To learn more, see the [App Connector Deployment Guide for Amazon Web Services (AWS)](/zpa/connector-deployment-amazon-web-services-aws#security).
- For Microsoft Azure, update your network security group (firewall) to allow inbound connections from port 22. To learn more, see the [App Connector Deployment Guide for Microsoft Azure](/zpa/connector-deployment-microsoft-azure#Deployment).
-
...
-
Enter the following command to start the zpa-connector service:
`[admin@zpa-connector ~]$ sudo systemctl start zpa-connector`
-
...
[]If you need to replace a deployed App Connector using a new provisioning key on an existing VM image, complete the [Replace an App Connector provisioning key](/zsdk/maintaining-deployed-app-connectors#ReplaceProvisioningKey) procedure. If you need to replace a deployed App Connector using an existing provisioning key on a new VM image:
- Log in to the ZSDK Admin Portal and go to **Configuration & Control > Private Infrastructure > App Connector Management > App Connector Provisioning Keys**.
- On the **App Connector Provisioning Keys** page, locate the existing key that you want to use and click the **Edit **icon.
- In the **Edit App Connector Provisioning Key** window, increase the **Maximum Reuse of Provisioning Key** value to accommodate the number of App Connectors that you are going to replace. To learn more, see [Managing App Connector Provisioning Keys.](/zsdk/managing-app-connector-provisioning-keys)
- Deploy the new replacement App Connector VM images using the existing provisioning key from the previous step. By doing so, these new App Connectors automatically join the same App Connector group as the App Connectors that you are replacing. To learn more, see the [Deployment Guide for your App Connector](/zpa/app-connector-management/app-connector-deployment-guides-supported-platforms).
- When the new App Connectors are deployed, they appear within the [Health dashboard](/zpa/about-dashboard/health). Verify that the new App Connectors in the App Connector group are enabled and showing an **Up Time**. To learn more, see [Viewing App Connector Details](/zpa/about-dashboard/health#cdetails).
- Go to **Configuration & Control** > **Private Infrastructure** >** App Connector Management** > **App Connectors**.
- On the **App Connectors **page, locate the App Connectors that you want to replace and click the **Edit** icon.
- In the **Edit App Connector** window, set the **Status **to **Disabled**. To learn more, see [Managing App Connectors](/zsdk/managing-app-connectors).
- Verify that user access to your applications is still working as desired.
- To ensure that all user traffic has fully transferred to the new replacement App Connectors, wait at least 24 hours before deleting the disabled App Connectors.
