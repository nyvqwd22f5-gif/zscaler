# Adding Users
Source: https://help.zscaler.com/zidentity/adding-users
PDF: https://help.zscaler.com/pdf/com/en/1499101.pdf

This article describes how to add individual user accounts for ZIdentity. When adding users, a user can belong to up to 1,024 groups. You can use a [CSV file to add multiple users](/zidentity/importing-user-details-csv-file) at once.
You can also add and manage users via System for Cross-domain Identity Management (SCIM) and single sign-on (SSO) just-in-time (JIT) provisioning. To learn more, see [About External Identity Providers](/zidentity/about-external-identity-providers).
To add a user:
...
-
**Department**: [Assign a department](/zidentity/adding-departments) to the user.
Department is a system-defined attribute.
-
**Assign Groups**: Select the [group](/zidentity/adding-user-groups) to which the user must be added.
[See image.](#zsl-generaltab)
If you would like to provide super admin privileges to users for ZIdentity and all the services linked to ZIdentity, you need add those users to the `Zscaler Global Administrators` group. To learn more, see [About User Groups](/zidentity/about-user-groups).
-
On the **Security Settings** tab:
- **Change Password Settings: **Enable to modify the password configurations. This remains disabled until the user is added to ZIdentity. To learn more, see [Configuring Security Settings for Users](/zidentity/configuring-security-settings-users).
...
-
On the **Additional Attributes **tab, click **Create New Attributes** to [add new user attributes](/zidentity/adding-user-attributes) or assign the values for user attributes that are already created in the ZIdentity Admin Portal by admins.
