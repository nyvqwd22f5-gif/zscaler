...
PDF: https://help.zscaler.com/pdf/com/en/1364451.pdf

[Watch a video about alerts and alert rules in ZDX](https://fast.wistia.net/embed/iframe/5ozc9luba9)
The Rules page in the ZDX Admin Portal provides an overview of details of all the configured rules, including each rule's status and alert delivery method. Rules allow you to configure metrics for alert throttling when a predetermined threshold is met.
...
The predetermined threshold is defined when you create a rule for alert throttling. To learn more, see [Triggering an Alert](/zdx/triggering-alert).
If the rules are not muted, you can configure alerts to be sent via email or webhooks. If the rules are muted, you can view the alerts in the ZDX Admin Portal and no information is sent via email or webhooks.
...
- Configure a new rule by clicking **Add New Alert Rule**. To learn more, see [Configuring a Rule](/zdx/configuring-alert-rule).
- View criteria details of configured rules. Click the arrow to the left of the rule name to view details.
-
Edit, mute, copy, or delete a rule. To learn more, see [Editing a Rule](/zdx/editing-alert-rule).
The Edit icon opens the Edit a Rule window for the selected rule. The Copy icon opens the Add New Alert Rule window with editable fields and copied details of the selected rule.
- Access the navigation menu to go to the following pages:
- [Alerts](/zdx/about-alerts)
- [Templates](/zdx/about-templates)
![Overview of the Rules Page](/downloads/zdx/alerts/about-rules/ZDX-About-Rule-Overview-steps-1.png)