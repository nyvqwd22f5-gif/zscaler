...
PDF: https://help.zscaler.com/pdf/com/en/1461996.pdf
...
Depending on which ZDX integration your service user needs access to, you are required to have one of the Incident Management roles assigned to your service user. Optionally, you can assign both roles to a service user if they require access to Root Cause Analysis, Alerts, and Deep Tracing.
-
-
...
To learn more about how to assign the role, see [ServiceNow Webhook Configuration Guide](/zdx/servicenow-webhook-configuration-guide#CreateServiceUser_SNOW).
...
- [A ZDX Role with the correct permissions to create an API Key](#ZDXRoleAPIKey)
- [A ZDX Admin Portal URL](#ZDXAdminPortalURL)
- [A ZDX Public API URL](#ZDXPublicAPIURL)
[]To create an API Key:
- [Add a ZDX Role](/zdx/adding-zdx-roles) with the following permissions to create an API Key:
- **Dashboard Access**: **View Only**
- **Deep Tracing**: **Full**
- **Device and User Information**: **Visible**
- After creating the ZDX Role, [create an API Key](/zdx/managing-zdx-api-keys#CreateAPIKey) on the API Key Management page.
- Save the API Key ID and Secret for the ServiceNow Settings Module.
[]Your ZDX Admin Portal URL is where you run Deep Tracing Sessions and Root Cause Analysis.
The format must be: `[subdomain].[second-level-domain].[top-level-domain]` (e.g., `admin.zdxcloud.net`). To learn more, see [What Is My Cloud Name for ZDX?](/zdx/what-my-cloud-name-zdx)
[]Your ZDX Public API URL is where the public API has access.
The format must be: `[subdomain].[second-level-domain].[top-level-domain]` (e.g., `api.zdxcloud.net`). To learn more, see [What Is My Cloud Name for ZDX?](/zdx/what-my-cloud-name-zdx)
...
- **ZDX Admin Portal URL**: Enter your ZDX Admin Portal URL.
- **ZDX Public API URL**: Enter your ZDX Public API URL.
...
- **ZDX Link**: A link to the Deep Tracing session in the ZDX Admin Portal.
- **Name**: The name of the Deep Tracing session on the ZDX Admin Portal.
...
- Click **Submit**.
-
Under the **Application** section, you can click **Analyze Score** on one of the existing applications if a low score is detected (less than or equal to 34).
[See image.](#ServiceNow-InProgress-AnalyzeScore)
A progress bar for the Root Cause Analysis appears.
[See image.](#inprogress)
-
When the Root Cause Analysis is completed, view a list of factors that determined the ZDX Score.
...
You can now view the Alert Impact and Impacted Users pages. You can also configure the Mappings module. To learn more, see [Understanding the ZDX Application Fields on ServiceNow](/zdx/understanding-zdx-application-fields-servicenow).
Alert Impact
...
- **Link**: A link to the impacted device page in the ZDX Admin Portal.
