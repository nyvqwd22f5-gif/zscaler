...
Source: https://help.zscaler.com/zdx/understanding-zscaler-hosted-monitoring
PDF: https://help.zscaler.com/pdf/com/en/1505966.pdf

Hosted Monitoring operates from within the Zscaler cloud infrastructure as a multi-tenant service. The service allows you to logically group Web and Cloud Path probes into independent collections to set up your own tests for monitoring performance. As an extension to ZDX probes for predefined and custom applications, Zscaler Hosted Probes monitor the performance of a specific service or application directly from a Zscaler data center to an endpoint destination.
To access Hosted Monitoring, make sure you've configured Zscaler Hosted Probes and met the feature prerequisites. To learn more, see [Configuring Zscaler Hosted Probes](/zdx/configuring-zscaler-hosted-probes).
To access Hosted Monitoring:
- Go to **Analytics **> **Hosted Monitoring**.
...
- Select the drop-down menu to incrementally add Web probe metrics and charts to Page Fetch Time. To learn more about Web probe metrics, see [About Probes](/zdx/about-probes) and [Evaluating User Details](/zdx/evaluating-user-details).
...
- Click any data point to view metrics for the hosted locations.
- Select the drop-down menu to incrementally add Cloud Path probe metrics and charts to End-to-End Latency. To learn more about Cloud Path probe metrics, see [About Probes](/zdx/about-probes) and [Evaluating the Cloud Path](/zdx/evaluating-cloud-path).
- Click the chart format icon to switch views between a line chart and a scatter chart. Each data point within a scatter chart represents an individual probe run.
...
To learn more about the visualization of data traffic, see [Evaluating the Cloud Path](/zdx/evaluating-cloud-path).
[]![Select filter locations and probe](/downloads/zdx/analytics/understanding-zscaler-hosted-monitoring/zdx-zprobe-monitoring-locations-filter_0.png)
...
[]![Cloud Path probe granular details page](/downloads/zdx/analytics/understanding-zscaler-hosted-monitoring/zdx-zprobe-monitoring-cp-granular-details-page_0.png)
