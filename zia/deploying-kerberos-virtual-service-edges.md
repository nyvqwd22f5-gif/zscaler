# Deploying Kerberos for Virtual Service Edges
Source: https://help.zscaler.com/zia/deploying-kerberos-virtual-service-edges
PDF: https://help.zscaler.com/pdf/com/en/1400371.pdf

Zscaler supports authentication using Kerberos, an industry-standard security protocol. To learn more about Kerberos benefits, features, and limitations, see [About Kerberos Authentication](/zia/about-kerberos-authentication). Before deploying Kerberos for ZIA Virtual Service Edge, see [Kerberos Requirements](/zia/about-kerberos-authentication#kerberos-requirements), [Kerberos Deployment Guidelines](/zia/kerberos-deployment-guidelines), and [Deploying Kerberos](/zia/how-do-i-deploy-kerberos).
To deploy Kerberos authentication for a Virtual Service Edge:
- Go to **Administration **>** Authentication Settings**.
- On the **Authentication Profile** page, under **Kerberos Authentication**, select **Enable Kerberos**. You must enable Kerberos in the **Authentication Profile** page in order for the **Enable Kerberos** option to appear within the **Edit Location** window.
- Click **Save**.
...
[]The hostname format depends on the [deployment mode](/zia/adding-virtual-service-edge-instances#deployment-mode) of the Virtual Service Edge:
- For cluster deployment mode, use the [cluster IP address](/zia/adding-virtual-service-edge-clusters#cluster-ip-address):
vzen-<Cluster IP Address>-<Company ID>.gateway.<Cloud Name>.net
For example:
vzen-10-66-63-22-370255.gateway.zscalertwo.net
- For standalone deployment mode, use the [proxy IP address](/zia/adding-virtual-service-edge-instances#proxy-ip-address) of the Virtual Service Edge instance:
