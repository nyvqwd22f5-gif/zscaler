...
PDF: https://help.zscaler.com/pdf/com/en/1401966.pdf
...
- **Log Domain**:
- If you want to configure Cloud NSS for ZIA Firewall logs, choose **Firewall** and follow the successive steps.
...
- **Feed Output Format**: These are the fields that are displayed in the output. See [NSS Feed Output Format: Firewall Logs](/zia/nss-feed-output-format-firewall-logs) for information about the available fields and their syntax.
...
- []**DNAT Policy Action**:** **Use this filter to limit the logs to traffic on which the service performed [destination NAT](/zia/about-nat-control) and redirected traffic to specific IP addresses and optionally, ports.
...
- An IP address with a netmask (e.g., 203.0.113.0/24)
You can enter multiple entries. Press `Enter` after each entry, then click **Add Items**. For item lists, you can view up to 500 items on a page; filter the list by searching for a word, phrase, or number contained in an item; and remove all items from the list (**Remove All**) or only items from a specific page (**Remove Page**). If you select **Remove All** or **Remove Page**, a confirmation window appears.
- **Server Destination Ports**: Use this filter to limit the logs to specific server destination ports. For aggregated sessions, this is the server destination port of the last session in the aggregate. You can specify individual ports and a range of ports. For item lists, you can view up to 500 items on a page; filter the list by searching for a word, phrase, or number contained in an item; and remove all items from the list (**Remove All**) or only items from a specific page (**Remove Page**). If you select **Remove All** or **Remove Page**, a confirmation window appears.
- **Tunnel IP Addresses**: The tunnel IP address of the server. For aggregated sessions, this is the server's tunnel IP address corresponding to the last session in the aggregate. For item lists, you can view up to 500 items on a page; filter the list by searching for a word, phrase, or number contained in an item; and remove all items from the list (**Remove All**) or only items from a specific page (**Remove Page**). If you select **Remove All** or **Remove Page**, a confirmation window appears.
- **Server IP Classes**: The [URL class](/zia/about-url-categories) that corresponds to the server IP address.
- **Server IP Super Categories**: The [URL super category](/zia/about-url-categories) that corresponds to the server IP address.
- **Server IP Categories**: The [URL category](/zia/about-url-categories) that corresponds to the server IP address.
...
- []**Network Applications**: Use this filter to limit the logs to specific [network applications](/zia/about-network-applications) associated with the session or aggregated sessions.
...
- **Network Services**: Use this filter to limit the logs to specific [network services](/zia/about-network-services) associated with the session or aggregated sessions.
-
[]**Threat Names**: Use this filter to limit the logs to specific threat names. You can specify the threats. Use the guidelines:
