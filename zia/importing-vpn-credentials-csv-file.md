...
PDF: https://help.zscaler.com/pdf/com/en/1401091.pdf

Configuring a VPN credential is one of the tasks you must complete when configuring an IPSec VPN tunnel. To learn more, see [Configuring an IPSec VPN Tunnel](/zia/configuring-ipsec-vpn-tunnel). You can import up to 3,000 entries per CSV file. For a complete list of ranges and limits per feature, see [Ranges & Limitations](/zia/ranges-limitations#VPNCredentials). You can also [manually add VPN credentials](/zia/adding-individual-vpn-credentials) to the ZIA Admin Portal.
...
- Within the CSV file, enter your VPN credential information. To learn more, see [Formatting CSV Files](/zia/importing-vpn-credentials-csv-file#format_csv).
...
- IP
- XAUTH
You can delete (-) an existing XAUTH VPN credential but cannot add (+) a new VPN credential using the CSV file. However, you can edit an existing XAUTH VPN credential directly from the ZIA Admin Portal.
...
- **Managed_By**: If this VPN credential is being managed by an [SD-WAN partner](/zia/about-partner-integration-management), enter the name of the partner. If the credential is not being managed by a partner, enter a comma instead. The valid partner names are:
- Cisco Viptela
- Citrix SD-WAN
- CloudGenix
- HPE Aruba
