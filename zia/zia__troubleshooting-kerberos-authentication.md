...
Source: https://help.zscaler.com/zia/troubleshooting-kerberos-authentication
PDF: https://help.zscaler.com/pdf/com/en/1399591.pdf
...
- Your Zscaler cloud, which includes the Kerberos server (KDC), the Zscaler Central Authority (CA), and the ZIA Public Service Edge. (See [What is my cloud name for ZIA?](/zia/what-my-cloud-name-zia))
...
To learn more about using Kerberos for your organization, see [About Kerberos Authentication](/zia/about-kerberos-authentication).
...
- Configure the browser to use Zscaler cloud on port 8800, the default Kerberos authentication port on ZIA Public Service Edges. Internet Explorer and Mozilla Firefox browsers support Kerberos authentication by default.
...
- If the cross-realm settings are configured incorrectly, then the ZIA Public Service Edge displays a page with an error code. For more information about the error codes, see [ZIA Public Service Edge Error Codes for Kerberos](/zia/zen-error-codes-kerberos).
...
[See image.](#seeimage9)
Your Zscaler domain name is the same as your Zscaler cloud name. In this example, it is ZSCALERTWO.NET. To learn how you can find your cloud name, see [What is my cloud name for ZIA?](/zia/what-my-cloud-name-zia)
If you do not see the Zscaler domain in the domain trusts list, you must add it as a trusted domain. See [1. Create the New Trust](/zia/kerberos-configuration-example-trust-relationship-windows-server-2012-and-gpo-push#create-new-trust) in *Kerberos Trust Relationship Configuration Guide for Windows Server 2012 and GPO Push*.
...
If the Zscaler KDC domain has been configured incorrectly, reconfigure the KDC. For instructions, see [Kerberos Trust Relationship Configuration Guide for Windows Server 2012 and GPO Push](/zia/kerberos-configuration-example-trust-relationship-windows-server-2012-and-gpo-push).
...
[See image.](#seeimg1)
If the Zscaler domain is not displayed, you must add the Zscaler domain as a trusted domain. See [1. Create the New Trust](/zia/kerberos-configuration-example-trust-relationship-windows-server-2012-and-gpo-push#create-new-trust) in *Kerberos Trust Relationship Configuration Guide for Windows Server 2012 and GPO Push*.
...
[See image.](#seeimg4)
If the correct values are not displayed, verify your configuration. For instructions, see [Kerberos Trust Relationship Configuration Guide for Windows Server 2012 and GPO Push](/zia/kerberos-configuration-example-trust-relationship-windows-server-2012-and-gpo-push).
Troubleshooting on User Devices
It is possible for Kerberos authentication to work on the domain controller, but to not work on a user's device. In this case, there may be an error with the GPO settings. You must check if GPO settings have been propagated from the domain controller to users. See [4. Configure GPO to Push the Configuration to Users](/zia/kerberos-configuration-example-trust-relationship-windows-server-2012-and-gpo-push#configure-gpo-push-users) in *Kerberos Trust Relationship Configuration Guide for Windows Server 2012 and GPO Push*.
...
[]![The Zscaler KDC krbtgt ticket, Your company's KDC krbtgt ticket, and the ZIA Public Service Edge krbtgt ticket in Windows PowerShell](/downloads/zia/authentication-administration/user-management-authentication-settings/kerberos-authentication/troubleshooting-kerberos-authentication/ZIA-KDC-krbtgt-%20and%20-service-ticket.png)
...
[]![Properties... button in the Trusts tab](/downloads/zia/authentication-administration/user-management-authentication-settings/kerberos-authentication/troubleshooting-kerberos-authentication/ZIA-properties..._button_in_the_trusts_tab.png)
[]![The other domain support Kerberos AES Encryption option in the Properties window](/downloads/zia/authentication-administration/user-management-authentication-settings/kerberos-authentication/troubleshooting-kerberos-authentication/ZIA-the_other_domain_support_kerberos_aes_encryption_option_in_the_properties_window.png)
[]![ValueName and Value fields for MitRealms](/downloads/zia/authentication-administration/user-management-authentication-settings/kerberos-authentication/troubleshooting-kerberos-authentication/valuename_and_value_fields_for_mitrealms.png)
[]![ValueName and Value fields for domain_realm](/downloads/zia/authentication-administration/user-management-authentication-settings/kerberos-authentication/troubleshooting-kerberos-authentication/ZIA-valuename_and_value_field_for_domain_realm.png)