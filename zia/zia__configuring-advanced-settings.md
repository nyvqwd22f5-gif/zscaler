...
PDF: https://help.zscaler.com/pdf/com/en/1399146.pdf

On the Advanced Settings page in the ZIA Admin Portal, you can configure settings for a variety of Zscaler service features.
...
- [JWT Authentication Exemptions](#jwt-auth)
...
-
You can modify the following configurations as needed, if you have [enabled the firewall service](/zia/configuring-firewall-policies).
[Watch a video about using custom ports for HTTP, HTTPS, DNS, and FTP traffic.](https://fast.wistia.net/embed/iframe/j4y7rs2uk4)
...
If your organization uses different or additional ports for these types of traffic, you can add custom network services with the appropriate ports. Then, select the network services under the following options to allow the Zscaler service to listen to those ports:
...
To learn more, see [Configuring Custom Ports](/zia/configuring-custom-ports).
-
Configure the following option:
[Auto Proxy Forwarding for Non-Defined Ports](#auto-proxy-forwarding)
- Click **Save** and [activate the change](/zia/saving-and-activating-changes-admin-portal).
[Watch a video about Admin Rank.](https://fast.wistia.net/embed/iframe/zm3n7kynyx)
[]**Enable Admin Ranking**:** **Enable this feature if you want to [rank administrators](/zia/what-admin-rank) and use the ranks when you manage policies.
[Watch a video about Allow Cascading to URL Filtering.](https://fast.wistia.net/embed/iframe/bl5e1nuqeu)
[]**Allow Cascading to URL Filtering**: Enable this if you want the service to apply the [URL Filtering policy](/zia/about-url-filtering) even if it has already applied a [Cloud App Control policy](/zia/about-cloud-app-control) that explicitly allows a transaction. By default, if a user requests a cloud app that you explicitly allow with a Cloud App Control policy rule, the service only applies the Cloud App Control policy and does not apply the URL Filtering policy. For example, if you have a Cloud App Control rule that allows viewing Facebook, but a URL Filtering policy that blocks www.facebook.com, a user is still allowed to view Facebook because, by default, the service doesn't apply the URL Filtering policy if a Cloud App Control rule allows the transaction. However, in the same example, if you allow cascading to URL filtering, the service blocks the user from Facebook because of your URL Filtering policy.
If a user requests a cloud app for which you haven't configured a Cloud App Control policy rule, the service still evaluates and applies the URL filtering policy. To learn more, see [About Policy Enforcement](/zia/about-policy-enforcement).
...
[Watch a video about Admin Session Timeout.](https://fast.wistia.net/embed/iframe/w40e22ctkx)
[]**Session Timeout Duration**: Specify how long admins can be inactive on the ZIA Admin Portal before they must log in again. By default, inactive sessions restart after 30 minutes. You can configure a different time interval, from 5 minutes to 600 minutes (10 hours). All active sessions automatically restart after a continuous login period of 720 minutes (12 hours) that is not configurable.
[Watch a video about Authentication Exemptions.](https://fast.wistia.net/embed/iframe/d20n3y6y10)
[]The service can exempt specific [URL categories](/zia/about-url-categories), URLs, [cloud app categories](/zia/cloud-app-categories), or specific cloud apps from [cookie authentication](/zia/about-zscaler-cookies):
- **Exempted URL Categories**: Select the URL categories you want to exempt from cookie authentication. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
- **Exempted URLs**: Enter the URLs you want to exempt from cookie authentication, and click **Add Items**. You can enter multiple entries. Press `Enter` after each entry. By default, you can add up to 25K custom URLs (across all categories). To learn more, see [Ranges & Limitations](/zia/ranges-limitations). For guidance on entering URLs, see the [URL format guidelines](/zia/url-format-guidelines). For item lists, you can view up to 500 items on a page; filter the list by searching for a word, phrase, or number contained in an item; and remove the first 25K items from the list (**Remove 25K Items**) or only items from a specific page (**Remove Page**). If you select **Remove 25K Items** or **Remove Page**, a confirmation window appears.
- **Exempted Applications**: Select cloud app categories or cloud apps you want to exempt from cookie authentication.
[]The service can exempt specific [URL categories](/zia/about-url-categories), URLs, [cloud app categories](/zia/cloud-app-categories), or specific cloud apps from [Kerberos authentication](/zia/about-kerberos-authentication):
- **Exempted URL Categories**: Select the URL categories you want to exempt from Kerberos authentication. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
...
[]The service can exempt specific [URL categories](/zia/about-url-categories), [cloud app categories](/zia/cloud-app-categories), or specific cloud apps from [Basic authentication](/zia/understanding-basic-authentication):
-
**Exempted URL Categories**: Select the URL categories you want to exempt from Basic authentication. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
If you want to exempt specific URLs, you can add them to a custom category and exempt the category. To learn more, see [Configuring Custom URL Categories](/zia/adding-custom-url-categories).
...
[]The service can exempt specific [URL categories](/zia/about-url-categories), URLs, [cloud app categories](/zia/cloud-app-categories), or specific cloud apps from [digest authentication](/zia/synchronizing-user-data-active-directory-openldap):
- **Exempted URL Categories**: Select the URL categories you want to exempt from digest authentication. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
...
[]The service can exempt specific [URL categories](/zia/about-url-categories), [cloud app categories](/zia/cloud-app-categories), or specific cloud apps from [JSON Web Token (JWT) authentication](/tech-pubs-drafts/understanding-jwt-authentication-draft-doc-56628):
-
**Exempted URL Categories**: Select the URL categories you want to exempt from JWT authentication. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
If you want to exempt specific URLs, you can add them to a custom category and exempt the category. To learn more, see [Configuring Custom URL Categories](/zia/adding-custom-url-categories).
...
[Watch a video about Internal IP Logging.](https://fast.wistia.net/embed/iframe/8rpe2auusc)
[]**Log Internal IPs from XFF Headers**:** **When the Zscaler service logs a transaction, it includes the source IP address, which is always the public IP address of the firewall or edge router that sent the traffic to the service. But if you use [proxy chaining](/zia/configuring-proxy-chaining) to forward traffic to the Zscaler service, a proxy server can insert an X-Forwarded-For (XFF) header in outbound HTTP requests. The XFF header identifies the IP address of the original client that sent the HTTP request through the proxy server.
...
[Watch a video about Surrogate IP Authentication.](https://fast.wistia.net/embed/iframe/gz58hrj5d2)
[]**Enforce Surrogate IP Authentication**:** **Some Windows 8 Metro apps use Internet Explorer as their user agent but do not support cookies or redirects, so the service does not allow traffic to these sites. Enable this option to allow the service to use the user-to-device mappings to apply the appropriate user policies to the traffic of the top Windows 8 Metro apps. The [Surrogate IP](/zia/what-surrogate-ip) feature must be enabled.
[Watch a video about Policy for Unauthenticated Traffic.](https://fast.wistia.net/embed/iframe/ffw255bfx0)
[]**Enable Policy For Unauthenticated Traffic**: For policies where you can specify users and departments in the criteria, the Zscaler service allows you to specify whether you want a rule to apply if the user traffic is unauthenticated. You must turn on this feature here if you want this option to appear when configuring your policy rules. Any rule that applies to unauthenticated traffic must apply to all locations; you cannot apply a rule to unauthenticated traffic and select particular locations. To learn more, see [Configuring Policies for Unauthenticated Traffic](/zia/configuring-policies-for-unauthenticated-traffic).
[Watch a video about HTTP Tunnel Control.](https://fast.wistia.net/embed/iframe/uyujvnk3mm)
...
- **Exempted URL Categories**: Select the URL categories you want to exempt from domain fronting. By default, you can add up to 64 custom categories. To learn more, see [Ranges & Limitations](/zia/ranges-limitations).
- **Exempted Cloud Applications**: Select the cloud app categories or cloud apps you want to exempt from domain fronting.
You can enable the Block CONNECT Host and SNI Mismatch option, but if you have a use case where an upstream proxy puts the IP into the CONNECT host name received by Zscaler, that results in a mismatch (so traffic is blocked). In this case, you can enable the [Prefer SNI over CONNECT Host for DNS](#prefer-sni) option.
[]To learn more about this option, see [About Microsoft One Click Options](/zia/about-microsoft-one-click-options).
[]
**Insert XFF Header for ZPA Traffic**: Inserts the XFF header for traffic that is forwarded from ZIA to Zscaler Private Access (ZPA) through a ZPA gateway ([Source IP Anchored](/zia/understanding-source-ip-anchoring) and [ZIA-inspected ZPA application traffic](/zia/configuring-forwarding-policy)). This field is enabled by default. If you don't want to insert the XFF header for outbound requests, disable this field. To learn more about XFF headers, see [Internal IP Logging](#internal-IP-logging).
This field is only available if you have a subscription to either Source IP Anchoring or ZPA App Inspection.
[See image.](#xff-header-zpa-traffic-image)
[Watch a video about DNS optimization.](https://fast.wistia.net/embed/iframe/3fls2teoxm?quality=1080p)
[]**Settings for DNS Optimization**: You can configure DNS optimization settings to allow Zscaler to perform a DNS lookup and override the externally resolved IP addresses in the outbound HTTP or HTTPS connections to establish a connection to the destination server that is geographically closer to the ZIA Service Edge (Public, Private, or Virtual). DNS optimization ensures reduced latency and increased performance efficiency. Additionally, it prevents DNS spoofing and hence acts as an extra layer of security to customers who are concerned that their client machine's DNS might be compromised.
...
- DNS optimization applies only to Z-Tunnel 2.0 and [transparent](/zia/what-proxy-mode#transparent-mode%20) mode traffic (e.g., traffic via GRE or IPSec tunnels without a PAC file).
...
- **Optimize These URL Categories**: Select the [URL categories](/zia/about-url-categories) to which DNS optimization must apply.
- **Optimize These Cloud Applications**: (Requires Advanced Firewall) Select the [cloud applications](/zia/cloud-app-categories) that the DNS optimization must apply.
-
**Optimize These FQDN**: Enter the IP addresses or FQDNs that the DNS optimization must apply.
To add multiple entries, press `Enter` after each entry and then click **Add Items**. You can add up to 25,000 custom URLs (across all categories). To learn more, see [Ranges & Limitations](/zia/ranges-limitations). For guidance on entering URLs, see the [URL format guidelines](/zia/url-format-guidelines).
For item lists, you can view up to 500 items on a page and filter the list by searching for a word, phrase, or number within an item. To remove items from the list, you can use:
...
- `ecservice.``<cloud>``.net`
- `ecgw.``<cloud>``.net`
To add multiple entries, press `Enter` after each entry and then click **Add Items**. You can add up to custom 25K URLs (across all categories). To learn more, see [Ranges & Limitations](/zia/ranges-limitations). For guidance on entering URLs, see the [URL format guidelines](/zia/url-format-guidelines).
For item lists, you can view up to 500 items on a page and filter the list by searching for a word, phrase, or number within an item. To remove items from the list, you can:
- Remove a single item by clicking the **Remove** icon displayed beside the item.
...
[See image.](#seeimage1)
If you have [IPv6 support enabled](/zia/configuring-ipv6-settings) for your organization, you can additionally configure DNS optimization settings for IPv6 connections to dual-stack or IPv6-only destinations. The Zscaler service performs a DNS lookup and might override the externally resolved IPv6 addresses in the outbound HTTP or HTTPS connections to establish an IPv6 or IPv4 connection to the destination server that is geographically closer to the ZIA Service Edge.
...
- **Optimize All (IPv6)**: Enable this option to apply DNS optimization to all [URL categories](/zia/about-url-categories) and [cloud applications](/zia/cloud-app-categories).
...
After completing your configurations, ensure that you click **Save **and [activate](/zia/saving-and-activating-changes-admin-portal) your changes.
...
To use HTTP/2 as the web protocol for this traffic, you must disable Bandwidth Control for the location it belongs to. Zscaler's Bandwidth Control functionality does not currently support HTTP/2. If a location has Bandwidth Control enabled, its traffic falls back to using HTTP/1.1 even if HTTP/2 is set as the default web protocol and enabled in the SSL Inspection policy.
[]**Enable Real-Time Risk Score Updates**: Enable this option to allow ZIA Public Service Edges to track risky user activities that might increase user risk in real time. When enabled:
- Displays the risk score in the **User Risk Profile** column on the [User Management page](/zia/about-users).
...
- You can view the recent risk score increases in the [User Risk Report](/zia/user-risk-report).
...
[Watch a video about Auto Proxy Forwarding.](https://fast.wistia.net/embed/iframe/xqz6soe7t8)
[]Enable this option to redirect non-standard port traffic, including HTTP, HTTPS, FTP, DNS, RTSP, and PPTP traffic, to the web module for inspection. This option can be used for identifying and redirecting traffic destined to non-standard ports when these ports are not explicitly defined in any [network services](/zia/about-network-services) (custom or predefined).
RTSP and PPTP are applicable only with [transparent mode](/zia/what-proxy-mode#transparent-mode) connectivity to the ZIA Public Service Edge (i.e., GRE or IPSec tunnels with no PAC file).
For the Auto Proxy feature to redirect web traffic, the firewall must be configured to identify web flows that are either HTTP or HTTPS by using an Allow rule with HTTP and HTTPS as [Network Application conditions](/zia/configuring-firewall-filtering-policy) (requires Advanced Firewall). When the firewall identifies a web destination, it registers the IP address and port combination as a web destination in the ZIA Public Service Edge where the traffic landed. The next time a session is directed to that non-standard web destination (from the same user or a user belonging to a different organization) and the traffic arrives on the same Service Edge, the firewall ensures that the traffic is redirected to and inspected by the web module from the first packet of the session. However, if that non-standard web traffic is sent to a different Service Edge, the web destination must be rediscovered and registered in that Service Edge.
[]You can configure the following settings to include the [EDNS Client Subnet (ECS) option](/zia/about-edns-client-subnet-ecs-injection) in DNS queries to obtain geolocated responses for clients from the external DNS service used by your organization:
Before enabling the ECS option, ensure that you have [configured the ECS prefix objects](/zia/adding-edns-client-subnet-ecs-prefixes), so they can be prepopulated for selection in the **ECS Prefix** field.
...
- **ECS Prefix**: This field appears when the ECS option is enabled using the preceding setting. Select the ECS prefix that must be used in DNS queries. To learn how to configure ECS prefixes, see [Adding EDNS Client Subnet (ECS) Prefixes](/zia/adding-edns-client-subnet-ecs-prefixes).
