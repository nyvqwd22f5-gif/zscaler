...
The Elliptic Curve Digital Signature Algorithm (ECDSA) authentication algorithm is supported only on the server-side (Service Edge to Server) SSL connections.
...
- DES-CBC3-SHA
- 3DES-CBC-SHA
Zscaler doesn't perform SSL inspection for websites that only use unsupported protocols.
- [See an example of traffic destined to such a website.](#example)
Zscaler considers traffic from such websites undecryptable. You can [configure the SSL Inspection policy](/zia/configuring-ssl-inspection-policy) to allow or block undecryptable traffic.
