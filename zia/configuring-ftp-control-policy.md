...
PDF: https://help.zscaler.com/pdf/com/en/1400596.pdf

You can configure the [FTP Control](/zia/about-ftp-control) policy to allow access to specific sites. With [Malware Protection](/zia/about-malware-protection), you can also scan FTP over HTTP traffic and FTP traffic in real time.
Configuring the FTP Control Policy
...
[]From the [FTP Control](/zia/about-ftp-control) page, you can allow or block FTP over HTTP traffic. You can also allow or block native FTP traffic by URL categories or specified URLs.
To configure the FTP Control policy:
- Go to **Policy** > **FTP Control**.
...
- **Allow Any URL Category**: Select this to allow FTP traffic for all [URL categories](/zia/about-url-categories). The policy applies to traffic from the known locations of an organization.
...
- **Allowed URL Categories**: You can select [URL super-categories, categories, or both](/zia/about-url-categories). You can select any number of categories. You can also search for categories or add a custom category by clicking the **Add** icon.
- **Allowed URLs**: Enter the URLs for which native FTP traffic will be allowed. You can add up to 25,000 URLs. For guidance on entering URLs, see the [URL format guidelines](/zia/url-format-guidelines). For item lists, you can view up to 500 items on a page; filter the list by searching for a word, phrase, or number contained in an item; and remove the first 25,000 items from the list (**Remove 25K Items**) or only items from a specific page (**Remove Page**). If you select **Remove 25K Items** or **Remove Page**, a confirmation window will appear.
...
[]From the [Malware Protection](/zia/about-malware-protection) page, you can enable or disable scanning for FTP over HTTP traffic and native FTP traffic.
...
You can also apply [Data Loss Prevention (DLP)](/zia/about-data-loss-prevention), [Sandbox](/zia/about-sandbox), and [File Type Control](/zia/about-file-type-control) policies to files extracted from the above protocols.