...
PDF: https://help.zscaler.com/pdf/com/en/1400966.pdf
...
IPS Control enables you to defend your organization against threats from both web traffic and non-web traffic. This includes protection for HTTP, HTTPS, FTP, DNS, TCP, UDP, and IP-based ports and protocols. However, IPS Control based on [custom IPS signature rules](/zia/about-custom-signature-rules) protects only non-web traffic. IPS Control also enables you to create granular rules for specific users, groups, departments, and so on.
You can enable IPS Control policies for specific locations by configuring the appropriate option for the location. To learn more, see [Enabling the Firewall for Locations](/zia/enabling-firewall-locations). If your organization uses Z-Tunnel 1.0 or PAC files to route traffic, ensure that the firewall is enabled for this type of traffic in the [Advanced Settings](/zia/configuring-advanced-settings). Threats detected in your network traffic are displayed under Firewall Insights > Logs. Threats detected in web-only traffic also appear on the Security Dashboard.
- IPS Control is available only with Advanced Firewall. For more information, see [Understanding Firewall Capabilities](/zia/understanding-firewall-capabilities). If you have [Advanced Threat Protection](/zia/configuring-advanced-threat-protection-policy) and IPS Control, rules created for Advanced Threat Protection are evaluated first.
- Only traffic that transits [ZIA Private Service Edge](/zia/about-service-edge) or [ZIA Virtual Service Edge](/zia/about-virtual-service-edges) can be inspected against custom IPS signature rules. To leverage custom IPS signature rules, your organization must have one of these ZIA Service Edge deployments.
About the IPS Control Page
On the IPS Control page (Policy > IPS Control), you can do the following:
- [Configure an IPS Control rule](/zia/configuring-ips-control-policy).
...
- [Edit, duplicate, or delete an IPS Control rule](/zia/how-do-i-edit-delete-or-duplicate-items-admin-portal).
...
- View the [recommended IPS Control policy](/zia/recommended-ips-control-policy).
