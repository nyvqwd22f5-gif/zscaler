...
Source: https://help.zscaler.com/zia/configuring-virtual-service-edge-amazon-web-services-gwlb
PDF: https://help.zscaler.com/pdf/com/en/1467091.pdf

Zscaler supports standalone ZIA Virtual Service Edge for production deployments on Amazon Web Services (AWS) with Gateway Load Balancer (GWLB). An organization can deploy the Virtual Service Edge instance on an EC2 Instance, on AWS with GWLB.
Before you begin deployment, contact Zscaler Support to request a share of the Virtual Service Edge AMI. Provide your AWS account ID and AWS region in which you want the AMI, except for China, as the Virtual Service Edge AMI is unavailable in this region (alternatively, you can consider [China Premium Internet Access](/zia/china-premium-internet-access) for the China region). After deployment, the Virtual Service Edge VM receives automatic software updates from the Zscaler cloud.
...
- [10. Add the service port to both the Virtual Service Edges on the ZIA Admin Portal.](#service-port-vse)
Testing Traffic
To test the traffic:
...
- In the ZIA Admin Portal, go to **Analytics** > **Web Insights** > **Logs**, and retrieve the logs for the last 15 minutes to verify logs for test traffic are present.
After you have verified your deployment, if you face any issues with Virtual Service Edge, you can [troubleshoot Virtual Service Edge](/zia/troubleshooting-virtual-zens).
...
If the swap usage is observed in one of the image types, use the other one.
...
-
Choose the EC2 Instance type recommended in the ZIA Admin Portal.
...
Ensure to repeat the preceding steps to launch a second EC2 instance.
-
...
- Click **Associate**.
Repeat the last three steps for the other Elastic IP address.
...
[]![AWS Create Target Group Window](/downloads/zia/documentation-knowledgebase/traffic-forwarding/zscaler-enforcement-nodes/virtual-service-edge/configuring-virtual-service-edge-amazon-web-services-gwlb/AWS-Create-Target-Groups-Window.png)
...
[-end-of-SME-]`
Ensure to enter the correct health check port (e.g., 5000) configured when [creating a target group](#target-group-vse-service-ip).
-
