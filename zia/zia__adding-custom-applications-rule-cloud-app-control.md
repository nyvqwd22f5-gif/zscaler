...
PDF: https://help.zscaler.com/pdf/com/en/1440851.pdf
...
- [Adding an application](/zia/adding-custom-cloud-applications) if it's not already present in Zscaler for discovery and usage metrics (e.g., number of users, bytes uploaded or downloaded, etc.).
- Associating the application with the Custom Applications for Cloud App Control rule.
You can create rules to control access to specific custom cloud applications. Cloud applications are organized into [categories](/zia/understanding-cloud-app-categories) to facilitate defining rules for similar applications.
Cloud applications that do not belong to any of the predefined categories can be added to the Custom Applications category, so you can create rules specifically for these cloud applications. You can specify which applications your users are allowed to access and define a daily quote by bandwidth or time.
...
-
Click **Add** and select **Custom Applications**.
The **Add Custom Applications Rule** window appears.
- In the **Add Custom Applications Rule** window, enter the **Cloud App Control Rule** attributes:
