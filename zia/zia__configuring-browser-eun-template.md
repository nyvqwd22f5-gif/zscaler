...
PDF: https://help.zscaler.com/pdf/com/en/1487361.pdf

You can configure the browser end user notification (EUN) templates with custom block and caution messages. To display the EUN for a specific Cloud App Control Policy rule, URL Filtering rule, or File Type Control rule, select the respective [browser notification template](/zia/about-browser-eun-template) configured for that particular rule. To learn more, see [Configuring the URL Filtering Policy](/zia/configuring-url-filtering-policy), [Adding Rules to the Cloud App Control Policy](/zia/adding-rules-cloud-app-control-policy), and [Configuring the File Type Control Policy](/zia/configuring-file-type-control-policy).
...
- Go to **Administration** > **End User Notifications** > **Browser** > **Templates**.
-
...
The browser EUN is supported for either the Block or Caution action in the [Cloud App Control policy](/zia/adding-rules-cloud-app-control-policy), [URL Filtering rule](/zia/configuring-url-filtering-policy), or [File Type Control policy](/zia/configuring-file-type-control-policy).
