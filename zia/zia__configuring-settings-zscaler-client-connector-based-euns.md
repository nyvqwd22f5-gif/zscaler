...
- **Do Not Disturb**: Opens the More tab in the Client Connector application where the user can control the notification settings. To learn more, see [Zscaler Endpoint Data Loss Prevention (DLP) Integration with Zscaler Client Connector](/client-connector/zscaler-endpoint-data-loss-prevention-dlp-integration-zscaler-client-connector).
- **Open Corporate Policy**: Opens the URL specified in the text field.
-
**Open Containing Folder**: Opens the folder where files that triggered the block rule are quarantined. To learn more, see [Viewing Information About Zscaler Endpoint DLP on Zscaler Client Connector](/client-connector/viewing-information-about-zscaler-endpoint-dlp-zscaler-client-connector).
This field is applicable only for Endpoint DLP policies that quarantine the original file.
-
**Request Exemption**: Opens the Data Protection tab in the Client Connector application where users can request to be exempted from the block rule application. To learn more, see [Viewing Information About Zscaler Endpoint DLP on Zscaler Client Connector](/client-connector/viewing-information-about-zscaler-endpoint-dlp-zscaler-client-connector).
...
Any changes made within this section are mirrored in the [User Confirmation settings](/zia/configuring-settings-user-confirmation-notification-templates) as these configurations are shared by Client Connector-based EUNs and User Confirmation notifications.
...
- $app_category
- $zcc_user
[]![Zscaler Client Connector EUN Settings](/downloads/zia/authentication-administration/end-user-notifications/client-connector-euns/configuring-settings-client-connector-based-euns/Zscaler-Client-Connector-EUN-Settings.png)