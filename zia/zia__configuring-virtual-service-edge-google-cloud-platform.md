...
Source: https://help.zscaler.com/zia/configuring-virtual-service-edge-google-cloud-platform
PDF: https://help.zscaler.com/pdf/com/en/1415181.pdf

Zscaler supports stand-alone Virtual Service Edge for production deployments on the Google Cloud Platform (GCP).
Requirements
...
[See image.](#cloud-configuration-requirements)
...
-
Virtual machine (VM) specs for a Virtual Service Edge:
- **CPUs**: 4 CPU cores
...
- [2. Add the Virtual Service Edge Instance](/zia/adding-virtual-service-edge-instances)
- [3. Download the Virtual Service Edge Certificate](/zia/downloading-virtual-service-edge-certificates)
- [4. Configure the Virtual Service Edge VM](#Sphere)
- [5. (Optional) Deploy GCP Internal Load Balancer (ILB)](#dual-arm-mode)
After you configure a Virtual Service Edge, you can then forward your internet traffic to it using one of the mechanisms described in [Forwarding Traffic to Virtual Service Edges](/zia/forwarding-traffic-virtual-service-edges).
...
- **9480**
- **9400**
Be sure to allow ports specifically required by your organization.
- Configure the browser proxy as **NIC0:9443**.
-
In the ZIA Admin Portal, configure a [URL Filtering](/zia/configuring-url-filtering-policy) rule and a [Firewall Filtering](/zia/configuring-firewall-filtering-policy) policy rule to block/allow traffic.
...
- Make a request to any domain and verify that the traffic is redirected to the ZIA Admin Portal.
-
Go to **Analytics** > **Web Insights** > **Logs**, and retrieve the logs for the last 15 minutes to verify logs for test traffic are present.
...
| ---------- | ------- | ------------ |
| Management IP Address | This address is used to make an SSH connection to the Virtual Service Edge VM for management. It is also used to download Virtual Service Edge builds from the Zscaler cloud. | In the Virtual Service Edge CLI, configure the IP address to ensure the following:It is accessible via SSH by the Virtual Service Edge administrator during install.It has outbound access to the internet. |
| Proxy IP Address | This address is used for the following purposes:Outbound data connection (proxied traffic)Outbound control connection to the Zscaler cloudHealth monitoring by the load balancerIn a stand-alone Virtual Service Edge, listening for user trafficIn the ZIA Admin Portal, creating a Virtual Service Edge instance |  |
...
[]![Screenshot of the Help menu highlighting the Cloud Configuration Requirements link used to view firewall requirements for Virtual Service Edge clusters.](/downloads/zia/documentation-knowledgebase/traffic-forwarding/zscaler-enforcement-nodes/virtual-service-edge/configuring-virtual-service-edge-clusters/ZIA-Help-Cloud-Configuration-Requirements.png)
...
-
Select the Virtual Service Edge VM.
The Virtual Service Edge VM is started by default. However, if the selected VM is stopped or suspended, click **Start / Resume**.
-
...
-
Install the SSL certificate of the Virtual Service Edge instance. This is the certificate you downloaded from the ZIA Admin Portal. A Virtual Service Edge uses this certificate to authenticate itself to the Zscaler service.
...
Be sure to specify the absolute path to the SSL certificates (e.g., `sudo vzen install-cert /tmp/cert-bundle.zip`).
...
[]![Screenshot of Google Cloud Platform Serial Console window](/downloads/zia/documentation-knowledgebase/traffic-forwarding/zscaler-enforcement-nodes/virtual-service-edge/configuring-virtual-service-edge-google-cloud-platform/SSH-in-browser-password-reset-password.png)
[]To deploy GCP ILB:
...
This avoids parallel upgrade outages as all nodes of GCP Virtual Service Edges are stand-alone.
-
