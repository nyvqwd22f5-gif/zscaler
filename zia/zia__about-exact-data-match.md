...
PDF: https://help.zscaler.com/pdf/com/en/1400851.pdf

[Watch a video on Exact Data Match.](https://fast.wistia.net/embed/iframe/5jnzyl383a)
Using [Exact Data Match (EDM) index templates](/zia/creating-exact-data-match-template) allows the Zscaler service to identify a record from a structured data source that matches predefined criteria. For example, your organization might want to protect personally identifiable information (PII) from being lost or might want to give your employees the ability to share their own PII data using a personal email or file sharing account. In either case, identifying and correlating multiple tokens that contribute to a particular record, to identify ownership of that data, is crucial. To learn more, see [Understanding Exact Data Match Index Templates](/zia/understanding-edm-index-templates).
...
In the [Index Tool](/zia/about-index-tool), creating an EDM template allows you to define these tokens (i.e., criteria) for your data records by importing a CSV file. After the data is defined and submitted, you can then apply the template to a custom DLP dictionary or engine, which uses the criteria to match against your records. The Zscaler service then evaluates the EDM-defined DLP rule with the appropriate action for your outbound traffic.
[See image.](#EDM-Workflow-Diagram)
You can create up to 64 templates for your organization. The cell usage information displayed within the ZIA Admin Portal and the Index Tool can help you determine the size of your data records per template. For example, if you submit a CSV file to create an EDM template that has 40K records and each record includes 3 columns, then it uses 120K cells. Also, the number of records used within a template has an impact on how you determine the amount of memory (RAM) you need to allocate for your [Index Tool VM](/zia/configuring-index-tool).
To learn more, see [About the Index Tool](/zia/about-index-tool) and [Creating an Exact Data Match Template](/zia/creating-exact-data-match-template).
About the Exact Data Match Page
On the Exact Data Match page (Administration > Index Templates > Exact Data Match), you can do the following:
...
- **Template Name**: The name of the template, as specified in the [Index Tool](/zia/creating-exact-data-match-template). You can sort this column.
...
- View EDM index template details, or [delete a template](/zia/creating-exact-data-match-template#DeleteTemplate-AdminPortal). You can also view template details within the [Index Tool](/zia/creating-exact-data-match-template).
- Go to the [Indexed Document Match (IDM)](/zia/about-indexed-document-match) page, where you can view a list of all IDM templates configured for your organization.
