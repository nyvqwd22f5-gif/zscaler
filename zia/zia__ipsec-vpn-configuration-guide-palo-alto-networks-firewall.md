...
PDF: https://help.zscaler.com/pdf/com/en/1399101.pdf
...
This article illustrates how to configure two IPSec VPN tunnels from a Palo Alto Networks firewall to two ZIA Public Service Edges: a primary tunnel from the PAN appliance to a ZIA Public Service Edge in one data center and a secondary tunnel from the PAN appliance to a ZIA Public Service Edge in another data center.
In this article, the IP address of the primary ZIA Public Service Edge is 165.225.80.35 and the IP address of the secondary ZIA Public Service Edge is 185.46.212.35. You can learn how to locate the ZIA Public Service Edge IP addresses for your organization in the Prerequisites section.
...
- [IP addresses of the ZIA Public Service Edges](/zia/locating-the-hostnames-and-ip-addresses-your-zens)
- [Virtual IP addresses of the ZIA Public Service Edges](/zia/locating-virtual-ip-addresses-your-zia-public-service-edges)
-
[Maximum Transmission Unit (MTU) for the tunnels](/zia/determining-the-optimal-mtu-for-gre-or-ipsec-tunnels)
Ensure that the locations of the [ZIA Public Service Edge Virtual IP addresses](/zia/locating-zen-virtual-ip-addresses-your-ipsec-vpn-tunnels) correspond to the locations of the [ZIA Public Service Edge IP addresses](/zia/how-do-i-locate-zen-ip-addresses-my-ipsec-vpn-tunnels).
If you are unable to ping both ZIA Public Service Edge IP addresses, please contact [Zscaler Support](/zia#regional-support).
Configuring the IPSec VPN Tunnels in the ZIA Admin Portal
To configure the IPSec VPN tunnels in the ZIA Admin Portal:
-
[Adding the VPN Credential](/zia/how-do-i-add-individual-vpn-credentials)
Note down the IP address or FQDN and the pre-shared key (PSK) of the added VPN credentials. You need this information to link the VPN credentials to a location and create the IKE gateways.
- [Linking the VPN Credentials to a Location](/zia/how-do-i-add-location)
Configuring the IPSec VPN Tunnels on PAN-OS
This guide covers only the configuration details of IPSec VPN tunnels between the Palo Alto Networks firewall and the ZIA Public Service Edges. For any other specific information about Palo Alto Networks, refer to the [Palo Alto Networks documentation](https://docs.paloaltonetworks.com/).
...
You need to create some additional security policies on the firewall to allow the internet traffic to flow through the IPSec tunnel to ZIA.
Troubleshooting
In the ZIA ZIA Admin Portal, you can go to **Analytics** > **Tunnel Insights** to see data as well as monitor the health and status of your configured IPSec VPN tunnels. To learn more, see [About Insights](/zia/about-insights) and [About Insights Logs](/zia/about-insights-logs).
...
- **Encryption**: Click **Add**, and choose **aes-128-gcm**.
- **Authentication**: Click **Add**, and choose **sha1**.
- **Key Lifetime**: Set it for `24`** Hours**.
- **IKEv2 Authentication Multiple**: Enter `0`.
[See image.](#ikecryptoprofile-8x)
- Click **OK**.
[]Create two IKE gateways, one for each Zscaler IPSec VPN node. In this example, the primary gateway created is named **ZscalerPrimaryTunnel **with the ZIA Public Service Edge IP address **165.225.80.35**. The secondary gateway is named **ZscalerBackupTunnel **with the ZIA Public Service Edge IP address **185.46.212.35**.
...
- **Peer IP Address**: Enter the ZIA Public Service Edge IP address for the primary gateway. In this example, it's `165.225.80.35`.
- **Authentication**: Select **Pre-Shared Key**.
- **Pre-shared Key**: Enter the pre-shared key of the [VPN credentials](/zia/how-do-i-add-individual-vpn-credentials) you created in the ZIA Admin Portal.
- **Confirm Pre-shared Key**: Reenter the pre-shared key.
- **Local Identification**: Enter the FQDN or IP address of the [VPN credentials](/zia/how-do-i-add-individual-vpn-credentials) you created in the ZIA Admin Portal. In this example, it's the IP address** **`99.41.72.25`.
...
- Click **OK**.
- Repeat the procedure to create the secondary IKE gateway (**ZscalerBackupTunnel**) using the ZIA Public Service Edge IP address **185.46.212.35**.
...
-
**Encryption**: Click **Add**, and choose **aes-gcm **based encryption.
...
- Repeat the procedure to create a secondary IPSec VPN tunnel (**ZscalerSecondaryTunnel)** using the secondary tunnel interface (**tunnel.2**), IKE gateway (**ZscalerBackupTunnel**), and ZIA Public Service Edge Virtual IP address (**185.46.212.34**).
...
- Click **OK**.
- Repeat the procedure to define the policy-based forwarding rule for the secondary tunnel (**BTpolicy**) using the secondary tunnel interface (**tunnel.2**) and ZIA Public Service Edge Virtual IP address (**185.46.212.34**).
...
****
[]
[]****![Screenshot of a configured IKE crypto profile in the PAN web interface.](/downloads/zia/traffic-forwarding/ipsec/ipsec-vpn-configuration-guide-palo-alto-networks-firewall/2.configured_ike_crypto_profile_in_the_palo-alto_web_interface_8.0.png)
****
...
****
[]****![Screenshot of a configured IPSec crypto profile in the PAN web interface.](/downloads/zia/traffic-forwarding/ipsec/ipsec-vpn-configuration-guide-palo-alto-networks-firewall/4.configured_ipsec_crypto_profile_in_the_pan_web_interface_8.0phase2.png)
****
...
[]Use the `show vpn gateway` command to view the VPN gateway configuration, no. of gateways configured, their peer address, etc.
