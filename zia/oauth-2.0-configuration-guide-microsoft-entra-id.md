...
PDF: https://help.zscaler.com/pdf/com/en/1417501.pdf

The following guide explains how you can configure [OAuth 2.0 authorization](/zia/securing-zia-apis-oauth-2.0) for the [cloud service API](/zia/understanding-zia-apis) using Okta.
...
- You have configured the necessary [API Roles](/zia/adding-api-roles) in the ZIA Admin Portal.
Configuring Okta
Complete the following steps to set up OAuth 2.0 authorization using Okta:
- [1. Register your application.](#RegisterApplication)
- [2. Create an authorization server.](#CreateAuthorizationServer)
- [3. Request an access token.](#RequestAccessToken)
...
-
Click **Save**.
After your application is registered, the app integration page appears with the General and Okta API Scopes tabs. The General tab of your app integration displays the generated Client ID and Client Secret values that are necessary to implement your authorization flow. The client application must have its client ID and client secret values stored securely.
-
On the **General** tab, under the **General Settings** section, ensure that the **Require Demonstrating Proof of Possession (DPoP) header in token requests** option is disabled for the **Proof of possession** field.
[See image.](#okta-proof-of-possession-option-disabled)
...
[]Configure Scopes
For the [Client Credentials](/zia/securing-zia-apis-oauth-2.0#client-credentials-flow) flow of OAuth 2.0 authorization, Okta requires that you create custom scopes to define the access privileges required for authorization.
...
- `<API Role>` represents the name of API Roles configured on the Role Management page within the ZIA Admin Portal.
...
Zscaler recommends that you do *not* select this option if you have multiple clients accessing the cloud service API using different scopes.
...
Access policies allow you to define rules for client applications that are requesting access tokens from your authorization server. The rules within an access policy assign an expiration time to an access token based on the grant type, scope, and various other parameters in the access token request.
Access policies and rules within each policy are evaluated in priority order (i.e., order of listing), which is automatically assigned on rule and policy creation. You can change the priority of individual policies and rules by reordering them using drag and drop.
...
- **OAuth/OIDC client:** Select your client application from the drop-down menu. To specify a client application, enter the name of the client application and select the application when it appears. This field auto-completes the name of your client applications as you type.
...
- **Scope:** [Custom scopes](#ConfigureScope) configured for your custom authorization server to define the permissions required by the client application to access the cloud service API. Custom scopes can be obtained from within the Scopes tab of the authorization server you created.
...
[]![A screenshot of the Okta client app's general settings with the Proof of Possession option disabled](/downloads/zia/authentication-administration/api-security/oauth-20-configuration-guide-okta/Okta-proof-of-possession-option-disabled.png)