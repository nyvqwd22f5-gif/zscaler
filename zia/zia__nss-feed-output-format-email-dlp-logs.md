...
Source: https://help.zscaler.com/zia/nss-feed-output-format-email-dlp-logs
PDF: https://help.zscaler.com/pdf/com/en/1483226.pdf
...
| %s{username} | The user who sent the email and is provisioned to ZIA | name@company.com |
| %s{extusername} | The user who sent the email but is not provisioned to ZIA. The external user can be within your organization. For example, a new employee joins the company and their company email address is created but their user account is not configured in the ZIA Admin Portal. In this case, the employee is treated as an external or "non-provisioned" user even though they are part of the company and can send email from their official company email address. | name@externalcompany.com |
| %s{owner} | The username or email address of the user who sent the email. This field is an aggregate of %s{username} and %s{extusername}. If the user is provisioned to ZIA, they populate in %s{username} and %s{owner}. If the user is not provisioned to ZIA, they populate in %s{extusername} and %s{owner}. The fields %s{username} and %s{extusername} are not populated at the same time. The field %s{owner} is always populated whether the user is provisioned to ZIA or not. | name@company.com |
| %s{sender} | The username or email address of the user who sent the email. This field is an aggregate of %s{username} and %s{extusername}. If the user is provisioned to ZIA, they populate in %s{username} and %s{sender}. If the user is not provisioned to ZIA, they populate in %s{extusername} and %s{sender}. The fields %s{username} and %s{extusername} are not populated at the same time. The field %s{sender} is always populated whether the user is provisioned to ZIA or not. | name@company.com |
Email Information
| Field | Description | Example |
| ----- | ----------- | ------- |
