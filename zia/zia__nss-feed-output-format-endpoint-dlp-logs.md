...
PDF: https://help.zscaler.com/pdf/com/en/1461946.pdf
...
Fields that support obfuscation are documented in the following tables with the prefix `o` (e.g., `%s{ouser}`). To obfuscate a field, manually add the prefix `o` before the field name in the Feed Output Format in the ZIA Admin Portal.
Date/Time
...
| %s{devicehostname} | The device hostname | Host |
| %s{odevicehostname} | The obfuscated version of the device hostname. This displays a random string. |  |
