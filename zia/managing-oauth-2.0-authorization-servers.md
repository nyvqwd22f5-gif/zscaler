...
PDF: https://help.zscaler.com/pdf/com/en/1415056.pdf

When using OAuth 2.0 for API authentication, the Zscaler service requires you to add your authorization servers to the ZIA Admin Portal in order to authorize client applications. You can add and manage your authorization servers from the OAuth 2.0 Authorization Servers page. You need an admin role with the API Key Management permission enabled to manage your authorization servers. To learn more, see [Adding Admin Roles](/zia/adding-admin-roles).
...
- A valid API subscription is required to add or manage authorization servers in the ZIA admin portal.
...
- Click **Save**.
[]You can edit the OAuth 2.0 authorization servers added to the ZIA Admin Portal. If the authorization server configuration is already in use, any changes to the configuration may lead to disruption in the API service. To avoid this, ensure that the changes are properly configured in both the ZIA Admin Portal and the client applications.
