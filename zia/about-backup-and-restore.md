...
PDF: https://help.zscaler.com/pdf/com/en/1485846.pdf
...
On the Backup and Restore page (Configuration & Control > Administration Control > Backup and Restore), you can do the following:
- View a list of applied filters available from the current and previous user sessions. Applied filters must be saved to the user session before they can be viewed. Use the drop-down menu to select the applied filters to view. To learn more, see [Using Tables](/zpa/using-tables#filterData).
- Hide the filters on the page by clicking **Hide Filters**. Click **Show Filters **to display the filters.
- [Create a scheduled backup configuration.](/zpa/creating-scheduled-backup-configurations)
- Filter the information that appears in the table. By default, no filters are applied. You can also save applied filters to your preferences so that they're visible in future user sessions. To learn more, see [Using Tables](/zpa/using-tables).
- [Add a backup manually](/zpa/adding-backups-manually).
-
View a list of all backups that were configured for your organization. For each backup, you can see:
- **Backup Name**: The name of the backup. Click the backup name to open the **Restore Activities** window and view a list of restore activities applied to the selected backup by the admin, and view the restore reports for each individual backup. To learn more, see [Viewing Restore Activities and Restore Reports](/zpa/viewing-restore-activities-and-restore-reports).
...
- [Edit a backup](/zpa/editing-backups).
-
[Restore stored policies and configurations from a backup.](/zpa/restoring-policies-and-configurations-backup)
You can restore policies and configurations from a backup a maximum of 10 times per day.
- [Delete a backup](/zpa/deleting-backups).
...
- [AppProtection](/zpa/appprotection-private-application-traffic-formerly-inspection)
- [Deception](/deception/what-zscaler-deception)-enabled applications, segment groups, servers, server groups, and access policies.
- [Extranet application support](/zia/about-extranet)
- [Privileged Remote Access (PRA)](/zpa/privileged-remote-access-management)
- Zscaler Internet Access (ZIA)-referenced server groups. Any server groups referenced in the ZIA Admin Portal are unavailable.
