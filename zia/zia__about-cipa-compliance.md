...
Source: https://help.zscaler.com/zia/about-cipa-compliance
PDF: https://help.zscaler.com/pdf/com/en/1401271.pdf
...
To aid schools and organizations to be compliant with the CIPA requirements, Zscaler provides a predefined URL filtering rule ([CIPA Compliance Rule](/zia/about-cipa-compliance#view-CIPA-comp-rule)) and a control ([Enable CIPA Compliance](/zia/about-cipa-compliance#Enable-CIPA-Compliance)) to enable the predefined rule. It also provides a [CIPA Compliance Report](/zia/cipa-compliance-report) which gives insights into the traffic and users that are blocked from accessing the prohibited content based on the CIPA Compliance Rule.
- [Enabling CIPA Compliance](#Enable-CIPA-Compliance)
- [Viewing CIPA Compliance Rule](#view-CIPA-comp-rule)
[]To activate the **CIPA Compliance Rule**, enable the **Enable CIPA Compliance** option from the **Policy **> **URL & Cloud App Control** > **Advanced Policy Settings** tab. To learn more, see [Configuring Advanced URL Policy Settings](/zia/configuring-advanced-url-policy-settings).
...
- The number of URL filtering policy rules exceeds the [limit](/zia/ranges-limitations#Rules).
- A rule with rule order 1 exists with a higher admin rank.
[]When you enable CIPA Compliance:
- The **CIPA Compliance Rule** gets activated under the **Policy **> **URL & Cloud App Control** > **URL Filtering Policy** tab.
- The following **Advanced URL Filtering** **Options** under the **Policy **> **URL & Cloud App Control** > **Advanced Policy Settings **tab get enabled. You cannot disable the following options:
...
- The **CIPA Compliance Report** option gets enabled under the following places:
- **Analytics **> **Standard Reports** > **Interactive Reports **> **Liability Exposure** drop-down.
- **Analytics **> **Schedule Reports** > **Add Scheduled Report** > **Report** drop-down.
[See image.](#reportimg)
To learn more, see [CIPA Compliance Report](/zia/cipa-compliance-report).
[]When you disable CIPA Compliance:
- The following warning message appears:
**If CIPA Compliance is disabled, then CIPA Compliance Rule will be disabled**.
- The **CIPA Compliance Rule** gets disabled under the **Policy **> **URL & Cloud App Control** > **URL Filtering Policy** tab. However, you can view the disabled rule.
- The **Advanced URL Filtering** **Options** under the **Policy **> **URL & Cloud App Control** > **Advanced Policy Settings **tab are set to the recommended policy settings. To learn more, see [Recommended URL & Cloud App Control Policy](/zia/recommended-url-cloud-app-control-policy#adv-url-policy-settings).
[]You can view the **CIPA Compliance Rule** under the **Policy **> **URL & Cloud App Control** > **URL Filtering Policy** tab if you select **Enable CIPA Compliance** option at least once. To learn more, see [Enabling CIPA Compliance](#Enable-CIPA-Compliance).
...
| ---------- | ----- | ----------- |
| Rule Order | 1 | (Editable) Order of precedence of the rule.
When the CIPA Compliance Rule is activated for the first time, it takes the highest precedence (rule order 1) by default. The rule order is automatically incremented by 1 for the other rules.
You can edit the rule order based on your preference. |
| Admin Rank | 0 to 7 | (Editable) Admin rank of the logged-in user.
If the logged-in user does not have an admin rank, then the rule is created with an admin rank 7. |
| Rule Name | CIPA Compliance Rule | (Non-editable) Name of the rule.
You cannot create any custom URL filtering rule with the name CIPA Compliance Rule. |
| Rule Status | Disabled/Enabled | (Non-editable) Status of the rule. |
| URL Categories | Adult Sex Education, Adult Themes, Alcohol/Tobacco, Anonymizer, Computer Hacking, Copyright Infringement, Drugs, Gambling, K-12 Sex Education, Lingerie/Bikini, Mature Humor, Militancy/Hate and Extremism, Newly Registered and Observed Domains, Nudity, Other Adult Material, Other Illegal or Questionable, Pornography, Profanity, Questionable, Body Art, Social Networking Adult, Tasteless, Violence, Weapons/Bombs | (Non-editable) List of all the predefined categories under the URL super category, **Legal Liabilities** with predefined URLs and keywords.
The custom categories under **Legal Liabilities **are excluded.
You cannot add or delete URL categories from the predefined list.
However, you can add or delete custom URLs and keywords (CIPA related) from any predefined category under **Legal Liabilities**. |
