...
PDF: https://help.zscaler.com/pdf/com/en/1503901.pdf

Zscaler’s [Nanolog Streaming Service (NSS)](/zia/understanding-nanolog-streaming-service) supports the configuration and deployment of an NSS virtual machine (VM) on Google Cloud Platform (GCP).
...
Additionally, ensure you have a [subscription](/zia/viewing-subscriptions) to either NSS for Web or NSS for Firewall and review the following specifications and requirements:
- [VM Specs](#vm-specs)
- [Network Specs](#network-specs)
- [Firewall Requirements](#firewall-reqs)
Deploying NSS
To deploy NSS:
- [Step 1: In the ZIA Admin Portal, Add an NSS Server and Download the SSL Certificate](#add-nss-server-ssl-certificate)
- [Step 2: In the ZIA Admin Portal, Add an NSS Feed](#article-adding-nss-feeds)
...
[]You [create a VM instance](/zia/nss-deployment-guide-google-cloud-platform#create-vm-instance) as a step in the deployment procedure. The VM must meet the following requirements:
...
- []Two network interfaces: You [create two network interfaces](/zia/nss-deployment-guide-google-cloud-platform#create-vm-instance) as a step in the deployment procedure.
...
You can find the name of your Zscaler cloud in the URL that you use to log in to the Zscaler service. For example, if you log in to admin.zscaler.net, then go to [https://config.zscaler.com/zscaler.net/nss](https://config.zscaler.com/zscaler.net/nss). To learn more, see [What Is My Cloud Name for ZIA?](/zia/what-my-cloud-name-zia)
...
-
Click **Download** in the **SSL Certificate** column of the newly added NSS server, and then save the SSL certificate. You later [copy the SSL certificate to the VM instance](/zia/nss-deployment-guide-google-cloud-platform#configure-start-nss).
...
When adding the feed, note the SIEM IP address and TCP port for later [verifying the NSS-to-SIEM connection](/zia/nss-deployment-guide-google-cloud-platform#verify-nss-configuration).
...
- **Skip OS adaptation**: Enable this setting.
...
- (Optional) In the **Firewall rules** section, you can select from existing firewall rules if they are appropriate for the VPC network. To learn more, see the [Firewall Requirements](/zia/nss-deployment-guide-google-cloud-platform#firewall-reqs) section. If you want to configure new rules, you can configure a firewall policy and associate it with the VPC network in the [subsequent step](/zia/nss-deployment-guide-google-cloud-platform#create-firewall-policy). For example, if you are creating custom subnets, you can create an appropriate firewall policy to manage them.
- Click **Create**.
...
-
In the **Machine configuration** section, select a machine that meets the requirements in the [VM Specs](/zia/nss-deployment-guide-google-cloud-platform#vm-specs) section. The following image shows an example of a machine that meets the requirements:
...
After creating the network interfaces, the management interface is assigned to `vtnet0` and the service interface is assigned to `vtnet1`, as shown in the configuration file in the [subsequent step](/zia/nss-deployment-guide-google-cloud-platform#verify-network-interfaces).
...
- [c. (Optional) Remove the SSL certificate.](#remove-ssl-certificate)
...
[]
![Create an image in Google Cloud using new Image Import](/downloads/zia/documentation-knowledgebase/analytics/nss/nss-deployment-guides/nss-deployment-guide-google-cloud-platform/create-image-image-import-config.png)
[]
